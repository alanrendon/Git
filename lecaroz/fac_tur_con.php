...
$tpl->assignInclude("body","./plantillas/fac/fac_tur_con.tpl");
$tpl->prepare();
...
if($reg)
{
	for($i=0;$i<count($reg);$i++)
	{
		$tpl->newBlock("rows");
		$tpl->assign("i",$i);
		$tpl->assign('cod_turno',$reg[$i]['cod_turno']);
		$tpl->assign('descripcion',$reg[$i]['descripcion']);
		$hor=obtener_registro("catalogo_horarios",array('cod_horario'),array($reg[$i]['cod_horario']),"","",$dsn);
		$tpl->assign('hor_desc',$hor[0]['descripcion']);
		$tpl->assign('cod_horario',$hor[0]['cod_horario']);
	}
	$tpl->gotoBlock("_ROOT");
	$tpl->assign("count",$i);
}
...
{
	header("location: ./fac_tur_con.php?codigo_error=1");
	die;
}
