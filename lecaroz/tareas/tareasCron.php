<?
include '/opt/lampp/htdocs/lecaroz/includes/class.db.inc.php';
include '/opt/lampp/htdocs/lecaroz/includes/dbstatus.php';
...
if ($flags[0]['actualizar_historico'] == "t") {
	...
	$sql .= " num_cia <= 300 AND fecha = '$fecha_historico' ORDER BY num_cia, controlada DESC, tipo, codmp";
	...
	// Si es para una compañía en específico
	$sql .= " num_cia <= 300 AND fecha BETWEEN '$fecha1' AND '$fecha2' AND descripcion != 'DIFERENCIA INVENTARIO' ORDER BY num_cia, controlada DESC, tipo, codmp, fecha, tipo_mov, cantidad DESC";
	$saldo = $db->query($sql);
	
	$sql = "SELECT num_cia, fecha, codmp, nombre, descripcion, cod_turno, tipo_mov, cantidad, precio_unidad, total_mov FROM mov_inv_real LEFT JOIN catalogo_mat_primas USING (codmp) WHERE";
	// Si es para una compañía en específico
	$sql .= " num_cia <= 300 AND fecha BETWEEN '$fecha1' AND '$fecha2' AND descripcion = 'DIFERENCIA INVENTARIO' ORDER BY num_cia, controlada DESC, tipo, codmp, fecha, tipo_mov";
	$difInv = $db->query($sql);
				...
				if ($saldo[$k]['tipo_mov'] == "t") {
					...
					if ($unidades < 0) {
						...
						if ($arrastre == TRUE)
							...
							// [02-Jul-2009] Si costo es 0 arrastrar costo promedio
//							if ($proximo_costo <= 0)
//								$proximo_costo = $costo_prom;
						}
						
						// Dividir valores de salida
						$val_1 = /*($saldo[$k]['cantidad'] + $unidades) >= 0 ? */($saldo[$k]['cantidad'] + $unidades) * $costo_ant/* : 0*/;
						$val_2 = /*$unidades > 0 ? */abs($unidades) * $proximo_costo/* : 0*/;
						$val_sal = $val_1 + $val_2;
...
$dia = isset($_GET['dia']) && $_GET['dia'] > 0 ? $_GET['dia'] : (int)date("d");
$mes = isset($_GET['mes']) && $_GET['mes'] > 0 ? $_GET['mes'] : (int)date("m");
$anio = isset($_GET['anio']) && $_GET['anio'] > 0 ? $_GET['anio'] : (int)date("Y");
...
$sql .= "INSERT INTO his_sal_ban (num_cia, fecha, cuenta, saldo_libros, saldo_bancos) SELECT num_cia, CURRENT_DATE, cuenta, saldo_libros, saldo_bancos FROM saldos ORDER BY num_cia;\n";

/****** GUARDAR HISTORICO DE SALDO A PROVEEDORES ******/
//$sql .= "INSERT INTO his_sal_pro (num_cia, fecha, saldo) SELECT num_cia, CURRENT_DATE, sum(total) FROM pasivo_proveedores GROUP BY num_cia ORDER BY num_cia;\n";
