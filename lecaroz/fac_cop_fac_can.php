...
$tpl->assignInclude("body","./plantillas/fac/fac_cop_fac_can.tpl");
$tpl->prepare();
...
if (isset($_POST['num_cia'])) {
	...
	for ($i = 0; $i < count($_POST['num_cia']); $i++)
		if ($_POST['num_cia'][$i] > 0 && $_POST['num_pro'][$i] > 0 && $_POST['num_fact'][$i] != '') {
			...
			if ($fact) {
				if ($_POST['num_cia'][$i] == $fact[0]['num_cia'] && $fact[0]['copia_fac'] == 't') {
					$update .= "UPDATE pasivo_proveedores SET copia_fac = 'FALSE' WHERE id = {$fact[0]['id']};\n";
				...
				else if ($_POST['num_cia'][$i] == $fact[0]['num_cia'] && $fact[0]['copia_fac'] == 'f') {
					$facs[$cont]['num_cia_cap'] = $_POST['num_cia'][$i];
					$facs[$cont]['nombre_cia_cap'] = $_POST['nombre_cia'][$i];
					$facs[$cont]['num_cia_fac'] = $fact[0]['num_cia'];
					$facs[$cont]['nombre_cia_fac'] = $fact[0]['nombre_cia'];
					...
					$facs[$cont]['nombre_pro'] = $_POST['nombre_pro'][$i];
					$facs[$cont]['num_fact'] = $_POST['num_fact'][$i];
					$facs[$cont]['estatus'] = 'YA CANCELADA';	// Ya esta cancelada
					$cont++;
				}
				else {
					$facs[$cont]['num_cia_cap'] = $_POST['num_cia'][$i];
	...
	if ($cont == 0) {
		header("location: ./fac_cop_fac_can.php");
		die;
	}
	else {
...
$cias = $db->query("SELECT num_cia, nombre_corto FROM catalogo_companias WHERE num_cia <= 800 ORDER BY num_cia");
foreach ($cias as $cia) {
	$tpl->newBlock("cia");
