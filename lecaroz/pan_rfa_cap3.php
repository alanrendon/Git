...
//SE BUSCAN LAS NOTAS DE PASTEL CUYA FECHA DE ENTREGA ES MENOR A LA FECHA DE CAPTURA
$str_vencidas= "select * from venta_pastel where estado = 0 and fecha_entrega < '".$_POST['fecha']."' and num_cia=".$_POST['num_cia']." and tipo = 0";
$notas_vencidas = ejecutar_script($str_vencidas,$dsn);
$total_vencidas = 0;
if($notas_vencidas){
	$total_vencidas = count($notas_vencidas);
	for($i=0;$i<count($arreglo);$i++){
		for($j=0;$j<count($notas_vencidas);$j++){
			if(@$arreglo[$i][0] == $notas_vencidas[$j]['letra_folio'] and @$arreglo[$i][1] == $notas_vencidas[$j]['num_remi']){
				$total_vencidas--;
				break;
			}
		}
