...
$descripcion_error[3] = "No puedes cancelar facturas de gas en esta pantalla";
...
$tpl->assignInclude("body","./plantillas/fac/fac_fac_can.tpl");
$tpl->prepare();
...
if($factura[0]['concepto']=='FACTURA GAS'){
	header("location: ./fac_fac_can.php?codigo_error=3");
	die;
}
...
//		print_r($prueba);
	...
	for ($j=0;$j<count($desglozada);$j++) 
	{
		$tpl->newBlock("rows");
		$tpl->assign("i",$j);
		$mp = obtener_registro("catalogo_mat_primas",array("codmp"),array($desglozada[$j]['codmp']),"","",$dsn);
		$tpl->assign('nom_mp',$mp[0]['nombre']);
		$tpl->assign('codmp',$desglozada[$j]['codmp']);
		$tpl->assign('contenido',$desglozada[$j]['contenido']);
		$tpl->assign('cantidad',$desglozada[$j]['cantidad']);
//		$tpl->assign('unidad',$desglozada[$j]['unidad']);
		$tpl->assign('precio',number_format($desglozada[$j]['precio'],2,'.',','));
		if($desglozada[$j]['porciento_desc_normal']<=0) $tpl->assign('desc1',"");
		else $tpl->assign('desc1',$desglozada[$j]['porciento_desc_normal']);
		if($desglozada[$j]['porciento_desc_adicional2']<=0) $tpl->assign('desc2',"");
		else $tpl->assign('desc2',$desglozada[$j]['porciento_desc_adicional2']);
		if($desglozada[$j]['porciento_desc_adicional3']<=0) $tpl->assign('desc3',"");
		else $tpl->assign('desc3',$desglozada[$j]['porciento_desc_adicional3']);
		if($desglozada[$j]['porciento_impuesto']<=0) $tpl->assign('iva',"");
		else $tpl->assign('iva',$desglozada[$j]['porciento_impuesto']);
		if($desglozada[$j]['ieps']<=0) $tpl->assign('ieps',"");
		else $tpl->assign('ieps',$desglozada[$j]['ieps']);
		$tpl->assign('total',number_format($desglozada[$j]['costo_unitario'],2,'.',','));
