...
$tpl->assignInclude("body","./plantillas/zap/zap_fac_con.tpl");
$tpl->prepare();
...
if (isset($_GET['num_cia'])) {
	...
	$sql .= $_GET['fecha_ent1'] != '' ? ($_GET['fecha_ent2'] != '' ? " AND f.fecha_rec BETWEEN '$_GET[fecha_ent1]' AND '$_GET[fecha_ent2]'" : " AND f.fecha_rec = '$_GET[fecha_ent1]'") : '';
	$sql .= $_GET['fecha_inv1'] != '' ? ($_GET['fecha_inv2'] != '' ? " AND f.fecha_inv BETWEEN '$_GET[fecha_inv1]' AND '$_GET[fecha_inv2]'" : " AND f.fecha_inv = '$_GET[fecha_inv1]'") : '';
	...
	$sql .= $_GET['tipo'] > 0 ? ($_GET['tipo'] == 1 ? ' AND f.clave = 0' : ' AND f.clave > 0') : '';
	$sql .= " ORDER BY " . ($_GET['orden'] == 1 ? 'num_cia, num_pro, clave' : 'num_pro, clave, num_cia') . ", f.fecha, num_fact";
	$result = $db->query($sql);

	if (!$result)
		die(header('location: ./zap_fac_con.php?codigo_error=1'));
	...
	foreach ($result as $i => $reg) {
		...
		$tpl->assign('ret', $reg['ivaret'] + $reg['isr'] != 0 ? number_format($reg['ivaret'] + $reg['isr'], 2, '.', ',') : '&nbsp;');
		...
		$mega_gran_ret += $reg['ivaret'] + $reg['isr'];
		...
		$tpl->assign('subbloque.ret', number_format($ret, 2, '.', ','));
		$tpl->assign('bloque.ret', number_format($gran_ret, 2, '.', ','));
	...
	$tpl->assign('result.ret', number_format($mega_gran_ret, 2, '.', ','));
