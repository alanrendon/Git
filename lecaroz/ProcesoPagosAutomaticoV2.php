...
if (isset($_REQUEST['accion']))
{
	...
	{
		case 'datos':
			$tpl = new TemplatePower('plantillas/ban/ProcesoPagosAutomaticoV2Bloques.tpl');
			$tpl->prepare();
		...
		case 'buscar':
			...
			if ($cias != '')
			{
				$conditions[] = "num_cia IN (SELECT COALESCE(cm.cia_principal, ccm.num_cia) FROM catalogo_companias ccm LEFT JOIN cuentas_mancomunadas cm ON (cm.cia_secundaria = ccm.num_cia) WHERE num_cia IN ({$cias}))";
			}
			...
			foreach ($result as $r)
			{//echo "CIA:{$r['num_cia']}, " . ($r['saldo'] + ($_REQUEST['cuenta'] < 0 ? ($_REQUEST['prox'] == $r['cuenta'] ? $r['promedio'] : 0) : $r['promedio']) * $dias_deposito) . "<br>";
			...
			if ($pagos_obligados != '')
			{
				...
				$conditions[] = "pp.num_cia IN ({$cias})";
				$conditions[] = "pp.fecha <= '{$_REQUEST['fecha_corte']}'";
				$conditions[] = "pp.total > 0";
				$conditions[] = "(pp.num_proveedor, pp.num_fact) NOT IN (SELECT num_proveedor, num_fact FROM facturas_pendientes WHERE fecha_aclaracion IS NULL)";
				$conditions[] = "(pp.copia_fac = TRUE OR cp.verfac = FALSE)";
				...
				$sql .= "SELECT
					...
					COALESCE((
						...
						FROM
							...
							LEFT JOIN catalogo_companias ccm ON (ccm.num_cia = cm.cia_principal)
						WHERE
							cm.cia_secundaria = pp.num_cia
							AND cm.tsbaja IS NULL
					), pp.num_cia) AS cia_pago,
					COALESCE((
						SELECT
							ccm.nombre
						FROM
					...
					COALESCE((
						...
						WHERE
							cm.cia_secundaria = pp.num_cia
							AND cm.tsbaja IS NULL
							AND LENGTH(clabe_cuenta) = 11
					), (
						CASE
							WHEN LENGTH(TRIM(cc.clabe_cuenta)) = 11 THEN
								pp.num_cia
							ELSE
								NULL
						END
					...
					COALESCE((
						...
						FROM
							cuentas_mancomunadas cm
							LEFT JOIN catalogo_companias ccm ON (cm.num_cia = ccm.cia_principal)
						WHERE
							cm.cia_secundaria = pp.num_cia
							AND cm.tsbaja IS NULL
							AND LENGTH(ccm.clabe_cuenta) = 11
					), (
						CASE
							WHEN LENGTH(TRIM(cc.clabe_cuenta)) = 11 THEN
								cc.nombre_corto
					...
					COALESCE((
						...
						WHERE
							cm.cia_secundaria = pp.num_cia
							AND cm.tsbaja IS NULL
							AND LENGTH(clabe_cuenta2) = 11
					), (
						CASE
							WHEN LENGTH(TRIM(cc.clabe_cuenta2)) = 11 THEN
								pp.num_cia
							ELSE
								NULL
						END
					...
					COALESCE((
						...
						WHERE
							cm.cia_secundaria = pp.num_cia
							AND cm.tsbaja IS NULL
							AND LENGTH(ccm.clabe_cuenta2) = 11
					), (
						CASE
							WHEN LENGTH(TRIM(cc.clabe_cuenta2)) = 11 THEN
								cc.nombre_corto
				...
				FROM
					...
					LEFT JOIN cuentas_mancomunadas cm ON (cm.cia_secundaria = pp.num_cia)
				WHERE
					" . implode(' AND ', $conditions) . "
				UNION\n";
			}
			...
			if ($_REQUEST['tipo_pago'] > 0)
			{
				switch ($_REQUEST['tipo_pago'])
				{
					...
					case 2:
						$conditions[] = "cp.trans = FALSE";
						break;

					case 3:
						$conditions[] = "cp.trans = TRUE";
						break;
				}
			}
			...
			foreach ($facturas as $f)
			{
				if ($num_cia != $f['cia_pago'])
				{
					...
					if (isset($saldos[$num_cia]))
					{
						...
						$tpl->assign('nombre', utf8_encode($f['nombre_cia_pago']));
				...
				if ($f['bloque'] == 1 && $ok)
				{
					...
					$tpl->assign('cia_pago', $f['cia_pago']);
					$tpl->assign('num_cia', $f['num_cia']);
					$tpl->assign('nombre_cia', utf8_encode($f['nombre_cia']));
...
$tpl = new TemplatePower('plantillas/ban/ProcesoPagosAutomaticoV2.tpl');
$tpl->prepare();
