<?php
// ALTA DE SINDICATOS
// Tabla 'catalogo_sindicatos'
// Menu 'Bancos->Catalogos'

define ('IDSCREEN',6618); // ID de pantalla
...
$descripcion_error[1] = "C&oacute;digo de sindicato ya existe en la Base de Datos";
...
$db = DB::connect($dsn);
if (DB::isError($db)) {
	echo "Error al intentar acceder a la Base de Datos. Avisar al administrador. ban_sin_tas.<br>";
	die($db->getMessage());
}
...
if (DB::isError($result)) {
	$db->disconnect();
	echo "Error en script SQL: $sql<br>Avisar al administrador. ban_sin_altas.<br>";
...
$id = nextid($screen->tabla, "idsindicato", $dsn);
$tpl->assign("id",$id);
