...
$sql = 'SELECT * FROM facturas_zap WHERE tspago IS NULL AND copia_fac = \'TRUE\' AND por_aut = \'TRUE\' AND clave > 0';
$sql .= isset($_GET['num_pro']) && $_GET['num_pro'] > 0 ? " AND num_proveedor = $_GET[num_pro]" : '';
$sql .= ' ORDER BY num_proveedor, num_fact';
$result = $db->query($sql);

$query = '';
$html = '<table border="1"><tr><th>ID</th><th>Proveedor</th><th>Remision</th><th>Dif</th><th>Total Remision</th><th>Total Pagos</th></tr>';
foreach ($result as $fac) {
	...
	foreach ($dep as $d) {
		if ($d['num_fact1'] == $fac['num_fact'])
			$total_dep += $d['pag1'] > 0 ? $d['pag1'] : 0;
		else if ($d['num_fact2'] == $fac['num_fact'])
			$total_dep += $d['pag2'] > 0 ? $d['pag2'] : 0;
		else if ($d['num_fact3'] == $fac['num_fact'])
			$total_dep += $d['pag3'] > 0 ? $d['pag3'] : 0;
		else if ($d['num_fact4'] == $fac['num_fact'])
			$total_dep += $d['pag4'] > 0 ? $d['pag4'] : 0;
	...
	$html .= '<tr>';
	$html .= "<td>$fac[id]</td><td>$fac[num_proveedor]</td><td>$fac[num_fact]</td><td>" . round($dif, 2) . "</td><td>$fac[total]</td><td>$total_dep</td></tr>";
