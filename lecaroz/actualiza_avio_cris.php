...
$sql = "SELECT num_cia FROM catalogo_companias WHERE num_cia = 13 ORDER BY num_cia";
...
$sql = "delete from inventario_real where num_cia = 13;";
$sql .= "insert into inventario_real (num_cia,codmp,existencia,precio_unidad) select num_cia,codmp,existencia,precio_unidad from historico_inventario where num_cia = 13 and fecha='2004/12/31';";
...
for ($c=0; $c<count($cia); $c++) {
	...
	$fecha = '01/01/2005';				// Fecha del listado
	...
	for ($j=0; $j<count($tablas); $j++) {
		...
		for ($i=0; $i<count($mp); $i++) {
			...
			if (existe_registro($tabla_inventario,array("num_cia","codmp"),array($num_cia,$codmp),$dsn))
				...
				$sql = "INSERT INTO $tabla_inventario (num_cia,codmp,existencia) VALUES ($num_cia,$codmp,$existencia);
				INSERT INTO historico_inventario (num_cia,codmp,existencia,fecha) VALUES ($num_cia,$codmp,$existencia,'$fecha_his');";
			echo "SQL => $sql<br>";
			ejecutar_script($sql,$dsn);
		}
	}
