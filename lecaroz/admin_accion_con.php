...
$tpl->assignInclude("body","./plantillas/adm/admin_accion_con.tpl");
$tpl->prepare();
...
$sql="SELECT * FROM catalogo_accionistas order by id";
...
if($reg)
{
	for($i=0;$i<count($reg);$i++)
	{
		...
		$tpl->assign("num",$reg[$i]['num']);
		$tpl->assign('nombre',$reg[$i]['nombre_corto']);
	}
}
else
{
	header("location: ./admin_accion_con.php?codigo_error=1");
	die;
}
