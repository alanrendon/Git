...
if (isset($_GET['tabla'])) {
	...
	/*for ($i=0; $i<$_POST['numfilas']; $i++) {
		$_SESSION['dep']['num_cia'.$i] = $_POST['num_cia'.$i];
		$_SESSION['dep']['nombre_cia'.$i] = $_POST['nombre_cia'.$i];
		$_SESSION['dep']['cod_mov'.$i] = $_POST['cod_mov'.$i];
		$_SESSION['dep']['fecha_mov'.$i] = $_POST['fecha_mov'.$i];
		$_SESSION['dep']['importe'.$i] = $_POST['importe'.$i];
		$_SESSION['dep']['concepto'.$i] = $_POST['concepto'.$i];
	}
	
	for ($i=0; $i<$_POST['numfilas']; $i++) {
		if ($_POST['num_cia'.$i] > 0 && $_POST['fecha_mov'.$i] != "" && $_POST['importe'.$i] > 0) {
			$dep['num_cia'] = $_POST['num_cia'.$i];
			$dep['cod_mov'] = $_POST['cod_mov'.$i];
			$dep['fecha_mov'] = $_POST['fecha_mov'.$i];
			$dep['importe'] = $_POST['importe'.$i];
			if ($_POST['concepto'.$i] != "")
				$dep['concepto'] = $_POST['concepto'.$i];
			else {
				$concepto = ejecutar_script("SELECT descripcion FROM catalogo_mov_bancos WHERE cod_mov = ".$_POST['cod_mov'.$i],$dsn);
				$dep['concepto'] = $concepto[0]['descripcion'];
			}
			$dep['fecha_cap'] = date("d/m/Y");
			$dep['manual'] = "TRUE";
			$dep['imprimir'] = "TRUE";
			$dep['ficha'] = (isset($_POST['ficha'.$i])?"TRUE":"FALSE");
			$dep['cuenta'] = 1;
			...
			$db_dep->generar_script_insert("");
			$db_dep->ejecutar_script();
	...
	$sql = "SELECT * FROM depositos WHERE fecha_cap = '".date("d/m/Y")."' AND manual = 'TRUE' AND imprimir = 'TRUE' ORDER BY num_cia,fecha_mov";
	$result = ejecutar_script($sql,$dsn);
	
	// Poner todos los depositos como ya impresos
	ejecutar_script("UPDATE depositos SET imprimir = 'FALSE' WHERE fecha_cap = '".date("d/m/Y")."' AND manual = 'TRUE' AND  imprimir = 'FALSE'",$dsn);
	...
	if ($result) {
		...
		for ($i=0; $i<count($result); $i++) {
			...
			$cia = ejecutar_script("SELECT nombre,clabe_cuenta2 FROM catalogo_companias WHERE num_cia = ".$result[$i]['num_cia'],$dsn);
			$tpl->assign("num_cia",$result[$i]['num_cia']);
			$tpl->assign("cuenta",$cia[0]['clabe_cuenta2']);
...
$mov = ejecutar_script("SELECT DISTINCT ON (cod_mov) cod_mov,descripcion FROM catalogo_mov_bancos WHERE tipo_mov='FALSE' ORDER BY cod_mov",$dsn);
...
for ($i=0; $i<$_GET['numfilas']; $i++) {
	...
	$tpl->assign("num_cia",isset($_SESSION['dep'])?$_SESSION['dep']['num_cia'.$i]:"");
	$tpl->assign("nombre_cia",isset($_SESSION['dep'])?$_SESSION['dep']['nombre_cia'.$i]:"");
	$tpl->assign("fecha_mov",isset($_SESSION['dep'])?$_SESSION['dep']['fecha_mov'.$i]:date("d/m/Y"));
