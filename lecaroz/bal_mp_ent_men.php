...
$tpl->assignInclude("body","./plantillas/bal/bal_mp_ent_men.tpl");
$tpl->prepare();
...
if (isset($_GET['num_cia'])) {
	...
	$sql = "SELECT num_cia_primaria AS num_cia FROM mov_inv_real LEFT JOIN catalogo_companias USING (num_cia) WHERE codmp = $_GET[codmp] AND fecha BETWEEN '$fecha1' AND '$fecha2' AND tipo_mov = 'FALSE'";
	if (count($cias) > 0) {
		$sql .= ' AND num_cia IN (';
	...
	$sql .= " AND descripcion NOT LIKE '%DIFERENCIA%' GROUP BY num_cia_primaria ORDER BY num_cia_primaria";
	$cias = $db->query($sql);
	...
	foreach ($cias as $cia) {
		$sql = "SELECT extract(day from fecha) AS dia, sum(cantidad) AS cantidad FROM mov_inv_real LEFT JOIN catalogo_companias USING (num_cia) WHERE codmp = $_GET[codmp] AND fecha BETWEEN '$fecha1' AND '$fecha2' AND tipo_mov = 'FALSE'";
		$sql .= " AND num_cia_primaria = $cia[num_cia] AND descripcion NOT LIKE '%DIFERENCIA%' GROUP BY dia ORDER BY dia";
		$movs = $db->query($sql);
...
$cods = $db->query('SELECT codmp, nombre FROM catalogo_mat_primas ORDER BY codmp');
foreach ($cods as $cod) {
	$tpl->newBlock('mp');
	$tpl->assign('codmp', $cod['codmp']);
	$tpl->assign('nombre', $cod['nombre']);
}
