...
$tpl->assignInclude("body","./plantillas/ros/ros_fac1_con.tpl");
$tpl->prepare();
...
// -------------------------------- Mostrar listado ---------------------------------------------------------
// Obtener rosticerias
$ros = ejecutar_script("SELECT * FROM catalogo_companias WHERE num_cia BETWEEN 301 AND 599 OR num_cia = 702 ORDER BY num_cia ASC",$dsn);

$sql="SELECT * FROM fact_rosticeria WHERE ";

$cia=" num_cia='".$_GET['num_cia']."' and fecha_mov='".$_GET['fecha_mov']."' AND num_proveedor = $_GET[num_pro]";
$sql=$sql.$cia;
...
if(!$facturas)
{
	header("location: ./ros_fac1_con.php?codigo_error=1");
	die;
}
else
{
	...
	for ($j=0;$j<count($facturas);$j++)
	{
			...
			$tpl->assign('cantidad',number_format($facturas[$j]['cantidad'],2,".",","));
			$tpl->assign('kilos',number_format($facturas[$j]['kilos'],2,".",","));
			$tpl->assign('precio',number_format($facturas[$j]['precio'],2,".",","));
