...
class DBclass {
	...
	var $encoding             = "LATIN1";			// Estilo de la fecha para la Base de Datos
	...
	function DBclass($dsn, $opciones = "") {
		...
		$this->db->query("SET client_encoding TO $this->encoding");
		...
		foreach ($opcion as $key => $value) {
			switch ($key) {
				...
				case "datestyle":
					...
					break;
				case "encoding":
					$result = $this->db->query("SET client_encoding = $value");
					$this->encoding = $result ? $value : $this->encoding;
					break;
			}
		}
	...
	function error($mensaje = "", $sql = "") {
		...
		if ($this->mostrar_errores == TRUE) {
			...
			//
			if (strpos($mensaje, ' at character '))
				$char_error = intval(substr($mensaje, strpos($mensaje, ' at character ') + 14), 10);
			...
			if ($sql != "" && $this->mostrar_script_error) {
				...
				$html .= $char_error > 0 ? (strlen($sql) > 50000 ? substr($sql, $char_error - 100, 200) : $sql) : substr($sql, 0, 50000) . (strlen($sql) > 4096 ? '...' : '');
				$html .= "</textarea></p>\n";
			}
			// Cuerpo de la página de error (continuación...)
			$html .= "</body>\n</html>";
	...
	function query($sql_script, $fetchmode = DB_FETCHMODE_ASSOC) {
		...
		if (stristr($sql_script, "INSERT") || stristr($sql_script, "UPDATE") || stristr($sql_script, "DELETE") || stristr($sql_script, "TRUNCATE")
		 || stristr($sql_script, "BEGIN") || stristr($sql_script, "COMMIT") || stristr($sql_script, "ROLLBACK") || stristr($sql_script, strtoupper("SET DATESTYLE"))) {
			return TRUE;
		}
		else {
			if ($result->numRows() > 0) {
	...
	function obtener_campos($info) {
		...
		foreach ($info as $key=>$value)
			...
			if (!preg_match("/not_null default_nextval/", $info[$key]['flags'])) {
