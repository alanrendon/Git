...
function actualizarMP($db, $num_cia, $codmp, $mes, $anio, $optdif = TRUE) {
	$fecha1 = date('d/m/Y', mktime(0, 0, 0, $mes, 1, $anio));
	$fecha2 = date('d/m/Y', mktime(0, 0, 0, $mes + 1, 0, $anio));
	$fecha_his = date('d/m/Y', mktime(0, 0, 0, $mes, 0, $anio));
	
	$sql = "SELECT num_cia, codmp, nombre, existencia, precio_unidad, controlada, tipo FROM historico_inventario LEFT JOIN catalogo_mat_primas USING (codmp) WHERE num_cia = $num_cia AND fecha = '$fecha_his' AND codmp = $codmp";
	$inv = $db->query($sql);
	
	if (!$inv)
		return FALSE;
	
	$sql = "SELECT codmp, fecha, descripcion, cod_turno, tipo_mov, cantidad, precio_unidad, total_mov, num_proveedor AS num_pro, nombre FROM mov_inv_real LEFT JOIN catalogo_proveedores USING (num_proveedor)";
	$sql .= " WHERE num_cia = $num_cia AND fecha BETWEEN '$fecha1' AND '$fecha2' AND codmp = $codmp AND descripcion != 'DIFERENCIA INVENTARIO' ORDER BY codmp, fecha, tipo_mov, cantidad DESC";
	$movs = $db->query($sql);
	
	if ($optdif) {
		$sql = "SELECT codmp, fecha, descripcion, cod_turno, tipo_mov, cantidad, precio_unidad, total_mov FROM mov_inv_real WHERE num_cia = $num_cia AND fecha = '$fecha2'";
		$sql .= " AND codmp = $codmp AND descripcion = 'DIFERENCIA INVENTARIO' ORDER BY codmp, fecha, tipo_mov, cantidad DESC";
		$difInv = $db->query($sql);
	}
	else
		$difInv = FALSE;
	...
	$costo_dif = costo_dif($codmp, $costo_prom);
			...
			if ($mov['tipo_mov'] == "t") {
				...
				if ($unidades < 0) {
					...
					if ($arrastre == TRUE)
						...
						$proximo_costo = buscar_fac($i, $codmp, $unidades);
	...
	// Opciones extras
	$query = "UPDATE inventario_real SET existencia = $unidades, precio_unidad = " . ($costo_prom != 0 ? $costo_prom : "0.00") . " WHERE num_cia = $num_cia AND codmp = $codmp;\n";
	$query .= "UPDATE inventario_virtual SET existencia = $unidades, precio_unidad = " . ($costo_prom != 0 ? $costo_prom : "0.00") . " WHERE num_cia = $num_cia AND codmp = $codmp;\n";
	$query .= "UPDATE historico_inventario SET existencia = $unidades, precio_unidad = " . ($costo_prom != 0 ? $costo_prom : "0.00") . " WHERE num_cia = $num_cia AND codmp = $codmp AND fecha = '$fecha2';\n";
