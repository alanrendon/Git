...
$tpl->assignInclude("body","./plantillas/ban/ban_cheques_list3.tpl");
$tpl->prepare();
...
$sql="select * from cheques WHERE importe > 0 AND cuenta = $_GET[cuenta] AND fecha between '".$_GET['fecha_inicial']."' and '".$_GET['fecha_final']."'" . (in_array($_SESSION['iduser'], $users) ? " and num_cia between 900 and 950" : " and num_cia between 1 and 800");
//if($_GET['tipo_con']==0){
	if($_GET['cia']!="")
		$sql.=" and num_cia=".$_GET['cia'];
//}
//else if($_GET['tipo_con']==1)
	$sql.= $_GET['proveedor'] > 0 ? " and num_proveedor=".$_GET['proveedor'] : "";
//else if($_GET['tipo_con']==2){
	$sql.= $_GET['gasto'] > 0 ? " and codgastos=".$_GET['gasto'] : "";
//	if($_GET['cia']!="")
//		$sql.=" and num_cia=".$_GET['cia'];
//	}
...
$sql.=" order by num_cia,fecha,folio";

$cheques=ejecutar_script($sql,$dsn);
//----------------------------------------------------------------------------------------------------------
if(!$cheques){
	header("location: ./ban_cheques_list3.php?codigo_error=1");
	die();
}
//print_r($cia);
...
if($_GET['tipo_con']==2){
	...
	$total_general=0;
	$gasto=obtener_registro("catalogo_gastos",array("codgastos"),array($_GET['gasto']),"","",$dsn);
...
//			$tpl->newBlock("cia");
		...
		else
		{
			...
			$tpl->assign("folio",$cheques[$i]['folio']);
			$tpl->assign("fecha",$cheques[$i]['fecha']);
			$tpl->assign("concepto",strtoupper($cheques[$i]['concepto']));
			if($cheques[$i]['fecha_cancelacion']=="")
			{
