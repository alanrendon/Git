...
if (isset($_REQUEST['accion'])) {
	switch ($_REQUEST['accion']) {
		case 'inicio':
			$tpl = new TemplatePower('plantillas/ban/ConciliarComisionesInicio.tpl');
			$tpl->prepare();
		...
		case 'codigos':
			...
			if ($result) {
				...
				foreach ($result as $r) {
					$data[] = '{"value":"' . $r['cod'] . '","text":"[' . $r['cod'] . '] '  . utf8_encode($r['concepto']) . '"}';
				}
		...
		case 'buscar':
			...
			if ($result) {
				...
				$tpl = new TemplatePower('plantillas/ban/ConciliarComisionesResultado.tpl');
				$tpl->prepare();
		...
		case 'conciliar':
			...
			$sql .= '
				UPDATE
					saldos
				SET
						saldo_bancos = saldo_bancos - comisiones.importe,
		...
		case 'reporte':
			...
			if ($result) {
				$tpl = new TemplatePower('plantillas/ban/ConciliarComisionesReporte.tpl');
				$tpl->prepare();
...
$tpl = new TemplatePower('plantillas/ban/ConciliarComisiones.tpl');
$tpl->prepare();
