<?php
// ALTA DE AUDITORES
// Tabla 'catalogo_auditores'
// Menu 'Bancos->Catalogos'

define ('IDSCREEN',6613); // ID de pantalla
...
$descripcion_error[1] = "C&oacute;digo de auditor ya existe en la Base de Datos";
...
$db = DB::connect($dsn);
if (DB::isError($db)) {
	echo "Error al intentar acceder a la Base de Datos. Avisar al administrador. ban_aud_altas.<br>";
	die($db->getMessage());
}
...
if (DB::isError($result)) {
	$db->disconnect();
	echo "Error en script SQL: $sql<br>Avisar al administrador. ban_aud_altas.<br>";
...
$id = nextid($screen->tabla, "idauditor", $dsn);
$tpl->assign("id",$id);
