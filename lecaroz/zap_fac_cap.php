...
if (isset($_GET['num_pro'])) {
	if ($db->query("SELECT id FROM facturas_zap WHERE num_proveedor = $_GET[num_pro] AND num_fact = $_GET[num_fact]"))
		echo 0;
	else
		echo 1;
	die;
...
if (isset($_POST['num_cia'])) {
	...
	header('location: ./zap_fac_cap.php');
	die;
}
...
$tpl->assignInclude("body", "./plantillas/zap/zap_fac_cap.tpl");
$tpl->prepare();
