...
if (isset($_REQUEST['accion'])) {
	switch ($_REQUEST['accion']) {
		...
		case 'inicio':
			$tpl = new TemplatePower('plantillas/zap/FacturasZapateriasConsultaInicio.tpl');
			$tpl->prepare();
		...
		case 'consultar':
			...
			if (isset($_REQUEST['facturas']) && trim($_REQUEST['facturas']) != '') {
				...
				if (count($facturas_between) > 0) {
					$partes1[] = implode(' OR ', $facturas_between);
				}

				if (count($partes1) > 0) {
					$condiciones[] = '(' . implode(' OR ', $partes1) . ')';
				}
			}
			...
			if ($query) {
				$tpl = new TemplatePower('plantillas/zap/FacturasZapateriasConsultaResultado.tpl');
				$tpl->prepare();
...
$tpl = new TemplatePower('plantillas/zap/FacturasZapateriasConsulta.tpl');
$tpl->prepare();
