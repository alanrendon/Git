...
<tr>
<td align="center" valign="middle"><p class="title">Modificaci&oacute;n de Facturas 
  
</p>
  <form action="./zap_fac_mod_v2.php" method="get" name="form">
    ...
    <tr>
      ...
      <td class="vtabla"><input name="num_fact" type="text" class="insert" id="num_fact" onfocus="tmp.value=this.value;this.select()" onchange="this.value=this.value.trim().toUpperCase();" onkeydown="if (event.keyCode == 13) num_pro.select()" size="5" /></td>
      </tr>
  </table>  
  <p>
...
<!--
var f = document.form, pro = new Array();
<!-- START BLOCK : p -->
pro[{num_pro}] = '{nombre}';
<!-- END BLOCK : p -->
...
function cambiaPro() {
	if (f.num_pro.value == '' || f.num_pro.value == '0') {
		...
		f.nombre.value = pro[get_val(f.num_pro)];
	else {
...
<table width="100%"  height="100%" border="0" cellspacing="0" cellpadding="0" align="center">
<tr>
<td align="center" valign="middle"><p class="title">Modificaci&oacute;n de Facturas</p>
  <form action="./zap_fac_mod_v2.php" method="post" name="form">
    <input name="tmp" type="hidden" id="tmp" />
    ...
    <tr>
      ...
      <input name="num_pro" type="text" class="insert" id="num_pro" onfocus="tmp.value=this.value;this.select()" onchange="if (isInt(this,tmp)) cambiaPro()" onkeydown="movCursor(event.keyCode,clave,null,clave,num_cia,fecha)" value="{num_proveedor}" size="3" />
        <input name="nombre_pro" type="text" class="vnombre" id="nombre_pro" style="font-size:12pt;" value="{nombre_pro}" size="45" readonly="true" />
        <input name="clave" type="text" class="insert" id="clave" onfocus="tmp.value=this.value;this.select()" onchange="isInt(this,tmp)" onkeydown="movCursor(event.keyCode,fecha,num_pro,null,num_cia,fecha)" value="{clave}" size="4" maxlength="4" /></td>
    </tr>
    <tr>
      <th colspan="2" class="vtabla" scope="row">Fecha factura</th>
      <td class="vtabla" colspan="3"><input name="fecha" type="text" class="insert" id="fecha" onfocus="tmp.value=this.value;this.select()" onchange="inputDateFormat(this)" onkeydown="movCursor(event.keyCode,num_fact,null,null,num_pro,num_fact)" value="{fecha}" size="10" maxlength="10" /></td>
    </tr>
	<tr>
      <th colspan="2" class="vtabla" scope="row">Factura</th>
      <td class="vtabla"><input name="num_fact" type="text" class="vinsert" id="num_fact" onfocus="tmp.value=this.value;this.select()" onchange="this.value=this.value.trim().toUpperCase();if (this.value.trim() != '') validaFac()" onkeydown="movCursor(event.keyCode,entrada,null,entrada,fecha,fecha_rec)" value="{num_fact}" size="10" />
        <input name="num_fact_old" type="hidden" id="num_fact_old" value="{num_fact}" /></td>
      <th class="vtabla">Entrada no. </th>
      <td class="vtabla"><input name="entrada" type="text" class="rinsert" id="entrada" onfocus="tmp.value=this.value;this.select()" onchange="isInt(this,tmp)" onkeydown="movCursor(event.keyCode,fecha_rec,num_fact,null,fecha,fecha_rec)" value="{entrada}" size="4" maxlength="4" /></td>
    </tr>
    <tr>
      <th colspan="2" class="vtabla" scope="row">Fecha recibido </th>
      <td class="vtabla" colspan="3"><input name="fecha_rec" type="text" class="insert" id="fecha_rec" onfocus="tmp.value=this.value;this.select()" onchange="inputDateFormat(this)" onkeydown="movCursor(event.keyCode,fecha_inv,null,null,num_fact,fecha_inv)" value="{fecha_rec}" size="10" maxlength="10" /></td>
    </tr>
	<tr>
    ...
    <tr>
      ...
      <td class="rtabla" colspan="3"><input name="importe" type="text" class="rinsert" id="importe" onfocus="tmp.value=this.value;this.select()" onchange="if (inputFormat(this,2,true)) calculaTotal()" onkeydown="movCursor(event.keyCode,desc1,null,null,codgastos,desc1)" value="{importe}" /></td>
    </tr>
    <tr>
	  <th colspan="2" class="vtabla" scope="row"><input name="apl_iva" type="checkbox" id="apl_iva" onchange="calculaTotal()" value="1" {apl_iva} />
	    I.V.A.</th>
    ...
    <tr id="faltantes_cell" style="display:none;">
      <td colspan="5" class="vtabla" scope="row"><table width="100%" align="center">
		...
		<tr>
          ...
          <td class="tabla"><input name="talla[]" type="text" class="rinsert" id="talla" onfocus="tmp.value=this.value;this.select()" onchange="isFloat(this,1,tmp)" onkeydown="movCursor(event.keyCode,piezas[{i}],color[{i}],piezas[{i}],talla[{back}],talla[{next}])" value="{talla}" size="4" /></td>
	...
	<tr>
	  ...
	  <td colspan="2" class="tabla"><input name="con_dif_precio" type="text" class="vinsert" id="con_dif_precio" onfocus="this.select()" onkeydown="movCursor(event.keyCode,dif_precio,null,dif_precio,null,cod_desc1)" value="{con_dif_precio}" maxlength="100" /></td>
	  <td class="rtabla"><input name="dif_precio" type="text" class="rinsert" id="dif_precio" onfocus="tmp.value=this.value;this.select()" onchange="if (inputFormat(this,2,true)) calculaTotal()" onkeydown="movCursor(event.keyCode,cod_desc1,con_dif_precio,null,importe,desc1)" value="{dif_precio}" /></td>
	  </tr>
	  <tr>
	  <td colspan="5" class="tabla" scope="row">&nbsp;</td>
	  </tr>
	<tr>
      ...
      <td colspan="2" class="tabla"><input name="cod_desc1" type="text" class="insert" id="cod_desc1" onfocus="tmp.value=this.value;this.select()" onchange="if (isInt(this,tmp)) cambiaCodDesc(1)" onkeydown="movCursor(event.keyCode,pdesc1,null,pdesc1,con_dif_precio,cod_desc2)" value="{cod_desc1}" size="2" />
        <input name="con_desc1" type="text" disabled="disabled" class="vnombre" id="con_desc1" value="{con_desc1}" size="30" />
        <input name="tipo_desc1" type="text" disabled="disabled" class="vnombre" id="tipo_desc1" style="color:#0000CC;" value="{tipo_desc1}" size="5" /></td>
      <td class="rtabla">
        <input name="pdesc1" type="text" class="rinsert" id="pdesc1" style="font-weight:bold;" onfocus="tmp.value=this.value;this.select()" onchange="if (inputFormat(this,2,true)) calculaTotal()" onkeydown="movCursor(event.keyCode,desc1,cod_desc1,desc1,importe,pdesc2)" value="{pdesc1}" size="5" />
    ...
    <tr>
      ...
      <td colspan="2" class="tabla"><input name="cod_desc2" type="text" class="insert" id="cod_desc2" onfocus="tmp.value=this.value;this.select()" onchange="if (isInt(this,tmp)) cambiaCodDesc(2)" onkeydown="movCursor(event.keyCode,pdesc2,null,pdesc2,cod_desc1,cod_desc3)" value="{cod_desc2}" size="2" />
        <input name="con_desc2" type="text" disabled="disabled" class="vnombre" id="con_desc2" value="{con_desc2}" size="30" />
        <input name="tipo_desc2" type="text" disabled="disabled" class="vnombre" id="tipo_desc2" style="color:#0000CC;" value="{tipo_desc2}" size="5" /></td>
      <td class="rtabla">
        <input name="pdesc2" type="text" class="rinsert" id="pdesc2" style="font-weight:bold;" onfocus="tmp.value=this.value;this.select()" onchange="if (inputFormat(this,2,true)) calculaTotal()" onkeydown="movCursor(event.keyCode,desc2,cod_desc2,desc2,pdesc1,pdesc3)" value="{pdesc2}" size="5" />
    ...
    <tr>
      ...
      <td colspan="2" class="tabla"><input name="cod_desc3" type="text" class="insert" id="cod_desc3" onfocus="tmp.value=this.value;this.select()" onchange="if (isInt(this,tmp)) cambiaCodDesc(3)" onkeydown="movCursor(event.keyCode,pdesc3,null,pdesc3,cod_desc2,cod_desc4)" value="{cod_desc3}" size="2" />
        <input name="con_desc3" type="text" disabled="disabled" class="vnombre" id="con_desc3" value="{con_desc3}" size="30" />
        <input name="tipo_desc3" type="text" disabled="disabled" class="vnombre" id="tipo_desc3" style="color:#0000CC;" value="{tipo_desc3}" size="5" /></td>
      <td class="rtabla">
        <input name="pdesc3" type="text" class="rinsert" id="pdesc3" style="font-weight:bold;" onfocus="tmp.value=this.value;this.select()" onchange="if (inputFormat(this,2,true)) calculaTotal()" onkeydown="movCursor(event.keyCode,desc3,cod_desc3,desc3,pdesc2,pdesc4)" value="{pdesc3}" size="5" />
	...
	<tr>
      ...
      <td colspan="2" class="tabla"><input name="cod_desc4" type="text" class="insert" id="cod_desc4" onfocus="tmp.value=this.value;this.select()" onchange="if (isInt(this,tmp)) cambiaCodDesc(4)" onkeydown="movCursor(event.keyCode,pdesc4,null,pdesc4,cod_desc3,null)" value="{cod_desc4}" size="2" />
        <input name="con_desc4" type="text" disabled="disabled" class="vnombre" id="con_desc4" value="{con_desc4}" size="30" />
        <input name="tipo_desc4" type="text" disabled="disabled" class="vnombre" id="tipo_desc4" style="color:#0000CC;" value="{tipo_desc4}" size="5" /></td>
      <td class="rtabla">
        <input name="pdesc4" type="text" class="rinsert" id="pdesc4" style="font-weight:bold;" onfocus="tmp.value=this.value;this.select()" onchange="if (inputFormat(this,2,true)) calculaTotal()" onkeydown="movCursor(event.keyCode,desc4,cod_desc4,desc4,pdesc3,null)" value="{pdesc4}" size="5" />
	...
	<tr>
      <th rowspan="2" class="vtabla" scope="row"><input name="apl_ret" type="checkbox" id="apl_ret" value="1" onclick="calculaTotal()" {apl_ret} /></th>
      <th class="vtabla" scope="row">Retenci&oacute;n I.S.R. </th>
      <th colspan="4" class="rtabla" scope="row"><input name="pisr" type="text" class="rinsert" id="fletes4" onfocus="tmp.value=this.value;this.select()" onchange="if (isFloat(this,2,tmp)) calculaTotal()" onkeydown="movCursor(event.keyCode,pivaret,null,null,desc4,ivaret)" value="{pisr}" size="5" maxlength="5" />
    ...
    <tr>
      ...
      <th colspan="4" class="rtabla" scope="row"><input name="pivaret" type="text" class="rinsert" id="fletes5" onfocus="tmp.value=this.value;this.select()" onchange="if (isFloat(this,8,tmp)) calculaTotal()" onkeydown="movCursor(event.keyCode,fletes,null,null,pisr,fletes)" value="{pivaret}" size="5" maxlength="12" />
    ...
    <tr>
      ...
      <th colspan="4" class="rtabla" scope="row"><input name="con_otros" type="text" class="vinsert" id="con_otros" onfocus="this.select()" onchange="if (inputFormat(this,2,true)) calculaTotal()" onkeydown="movCursor(event.keyCode,null,null,null,fletes,null)" value="{con_otros}" size="30" maxlength="255" />        
      <input name="otros" type="text" class="rinsert" id="otros" onfocus="tmp.value=this.value;this.select()" onchange="if (inputFormat(this,2,true)) calculaTotal()" onkeydown="movCursor(event.keyCode,null,con_otros,null,fletes,null)" value="{otros}" /></th>
    </tr>
    <tr>
      <th colspan="2" class="vtabla" scope="row">Total a Pagar </th>
      <th class="rtabla" colspan="3"><a href="javascript:dec(0.01)" style="color:#00F; text-decoration:none;">[+]</a> <a href="javascript:dec(-0.01)" style="color:#F00; text-decoration:none;">[-]</a><input name="total" type="text" class="rnombre" id="total" style="font-size:14pt; color:#0000CC;" value="{total}" readonly="true" /></th>
    </tr>
  </table>  
  <p>
    <input type="button" class="boton" value="Cancelar" onclick="{close}" />
    &nbsp;&nbsp;
...
<script language="javascript" type="text/javascript">
<!--
var f = document.form, cia = new Array(), pro = new Array(), cod = new Array(), codDesc = new Array(), hidden = true;
<!-- START BLOCK : cia -->
cia[{num_cia}] = '{nombre}';
<!-- END BLOCK : cia -->
<!-- START BLOCK : pro -->
...
function cambiaCodDesc(i) {
	...
	else if (codDesc[get_val(document.getElementById('cod_desc' + i))] != null) {
		var tmp = codDesc[get_val(document.getElementById('cod_desc' + i))].split('|');
		document.getElementById('con_desc' + i).value = tmp[0];
		document.getElementById('tipo_desc' + i).value = tmp[1];
	}
	else {
		alert('El código de descuento no se encuentra en el catálogo');
		document.getElementById('cod_desc' + i).value = f.tmp.value;
		document.getElementById('cod_desc' + i).select();
	}
}
...
function calculaTotal() {
	var total_fac, iva_fac, subtotal, importe, desc1, desc2, desc3, desc4, total_desc, faltantes, iva, pisr, pivaret, isr, ivaret, fletes, otros, total;
	
	importe = get_val(f.importe);
	iva_fac = f.apl_iva.checked ? importe * /*0.15*/0.16 : 0;
	...
	iva = f.apl_iva.checked ? subtotal * /*0.15*/0.16 : 0;
	pisr = get_val(f.pisr);
	...
	/*if (f.num_pro.value == '216' || f.num_pro.value == '231' || f.num_pro.value == '991')
		total -= 0.01;*/
...
function validaFac() {
	if (get_val(f.num_pro) <= 0 || f.num_fact.value.trim() == '')
		return false;
	else if (get_val(f.num_pro) == get_val(f.num_pro_old) && f.num_fact.value.trim() == f.num_fact_old.value.trim())
		return false;
	...
	// Pedir datos
	myConn.connect("./zap_fac_cap_v2.php", "GET", 'num_pro=' + get_val(f.num_pro) + '&num_fact=' + f.num_fact.value.trim() + '&fecha=' + f.fecha.value, resultFac);
}
...
window.onload = function() { showAlert = true; cambiaCodDesc(1); cambiaCodDesc(2); cambiaCodDesc(3); cambiaCodDesc(4); calculaTotal(); f.num_cia.select(); };
//-->
</script>
<!-- END BLOCK : mod -->
<!-- START BLOCK : close -->
