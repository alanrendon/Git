...
<td align="center" valign="middle"><p class="title">Captura de Facturas</p>
  <form action="./zap_fac_cap_v2.php" method="post" name="form">
    ...
    <tr>
      ...
      <input name="num_pro" type="text" class="insert" id="num_pro" onfocus="tmp.value=this.value;this.select()" onchange="if (isInt(this,tmp)) cambiaPro()" onkeydown="movCursor(event.keyCode,clave,null,clave,num_cia,fecha)" value="{num_pro}" size="3" />
    ...
    <tr>
      ...
      <th colspan="4" class="rtabla" scope="row"><input name="pivaret" type="text" class="rinsert" id="fletes5" onfocus="tmp.value=this.value;this.select()" onchange="if (isFloat(this,9,tmp)) calculaTotal()" onkeydown="movCursor(event.keyCode,fletes,null,null,pisr,fletes)" value="10.666666667" size="5" maxlength="12" />
    ...
    <tr id="bloque_suc" style="display:none;">
      <th colspan="2" class="vtabla" scope="row">Sucursales</th>
      <th colspan="3" class="vtabla" id="suc">&nbsp;</th>
    </tr>
    <tr id="bloque_suc" style="display:none;">
      <td colspan="5" class="vtabla" scope="row">&nbsp;</td>
      </tr>
    <tr id="bloque_suc" style="display:none;">
      <th colspan="2" class="vtabla" scope="row">Refacturaci&oacute;n</th>
      <td colspan="3" class="vtabla" id="suc"><input name="refac" type="checkbox" id="refac" onchange="if (this.checked) {
obs.disabled=false;
obs.focus();
}
else obs.disabled=true;" value="1" />
        Si ||| Observaciones
          <textarea name="obs" cols="50" disabled="disabled" class="insert" id="obs"></textarea></td>
    </tr>
  </table>  
  <p>
...
<!-- END BLOCK : cia -->
<!-- START BLOCK : pro -->
pro[{num_pro}] = new Array('{nombre}', {cod_desc1}, {desc1}, {cod_desc2}, {desc2}, {cod_desc3}, {desc3}, {cod_desc4}, {desc4}, {clave});
<!-- END BLOCK : pro -->
<!-- START BLOCK : cod -->
cod[{cod}] = '{desc}';
<!-- END BLOCK : cod -->
...
var resultFac = function (oXML) {
	...
	if (parseInt(result) == 0) {
		alert('Ya existe una factura con el mismo proveedor, año y número, favor de verificar.');
...
function sucursales() {
	if (get_val(f.num_cia) <= 0) {
		...
		document.getElementById('bloque_suc').style.display = 'none';
		return false;
	}
	...
	// Pedir datos
	myConn.connect("./zap_fac_cap_v2.php", "GET", 'matriz=' + get_val(f.num_cia), resultSuc);
}

var resultSuc = function (oXML) {
	var result = oXML.responseText;
	...
	document.getElementById('suc').innerHTML = result;
	document.getElementById('bloque_suc').style.display = 'table-row';
}
