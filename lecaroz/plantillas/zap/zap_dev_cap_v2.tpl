...
<td align="center" valign="middle"><p class="title">Captura de Devoluciones</p>
  <form action="./zap_dev_cap.php" method="post" name="form">
    ...
    <tr>
      ...
      <td class="vtabla"><input name="folio" type="text" class="nombre" id="folio" value="{folio}" size="6" readonly="true" /></td>
    </tr>
  </table>  
  <br />
  <table class="tabla">
    <tr>
      ...
      <th colspan="2" class="tabla">Observaciones</th>
      </tr>
    <!-- START BLOCK : fila_cap -->
	<tr onMouseOver="overTR(this,'#ACD2DD');" onMouseOut="outTR(this,'');">
      <td class="tabla"><input name="modelo[]" type="text" class="insert" id="modelo" style="width:100%;" onfocus="tmp.value=this.value;this.select()" onchange="isInt(this,tmp)" onkeydown="movCursor(event.keyCode,color[{i}],null,color[{i}],modelo[{back}],modelo[{next}])" value="{modelo}" size="6" /></td>
      ...
      <td colspan="2" class="tabla"><input name="obs[]" type="text" class="vinsert" id="obs" onkeydown="movCursor(event.keyCode,modelo[{next}],precio[{i}],null,obs[{back}],obs[{next}])" value="{obs}" size="30" maxlength="100" /></td>
      </tr>
	<!-- END BLOCK : fila_cap -->
	<!-- START BLOCK : fila_reg -->
    <tr onMouseOver="overTR(this,'#ACD2DD');" onMouseOut="outTR(this,'');">
      <td class="tabla">{modelo}</td>
    ...
    <tr>
      ...
      <th class="tabla"><input name="total_tmp" type="hidden" id="total_tmp" value="{total_tmp}" />
        <input name="total" type="text" class="rnombre" id="total" style="font-size:14pt;" value="{total}" size="10" /></th>
      <th colspan="2" class="tabla">&nbsp;</th>
      </tr>
  </table>
  <p>
    <input name="" type="button" class="boton" onclick="cancelar()" value="Cancelar" />
&nbsp;&nbsp;
<input type="button" class="boton" onclick="borrar()" value="Borrar" />
&nbsp;&nbsp;
<input name="" type="button" class="boton" onclick="validar('terminar')" value="Terminar" />
  &nbsp;&nbsp;
...
function cambiaCia() {
	if (f.num_cia.value == '' || f.num_cia.value == '0') {
		...
		f.num_cia.value = f.num_cia.value;
	}
}
...
function cambiaPro() {
	if (f.num_pro.value == '' || f.num_pro.value == '0') {
		...
		alert("La compañía no se encuentra en el catálogo");
		f.num_pro.value = f.num_pro.value;
	}
}
...
function validar(action) {
	...
	if (action == 'agregar' && confirm("¿Desea agregar más datos?")) {
		f.action = "./zap_dev_cap.php?action=append";
		f.submit();
	}
	else if (action == 'terminar' && confirm("¿Desea terminar la captura?")) {
		f.action = "./zap_dev_cap.php?action=finish";
		f.submit();
	}
}

function cancelar() {
	if (confirm("¿Desea cancelar la captura?"))
		document.location='./zap_dev_cap.php?action=cancel';
	else
		f.modelo[0].select();
}
...
function borrar() {
	...
	if (cont == 0) {
		...
		f.action = "./zap_dev_cap.php?action=delete";
		f.submit();
	}
}
