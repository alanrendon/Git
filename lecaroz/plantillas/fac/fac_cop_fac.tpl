...
<tr>
<td align="center" valign="middle"><p class="title">Validaci&oacute;n de Facturas </p>
  <form action="./fac_cop_fac.php" method="post" name="form">
	...
	<tr>
      ...
      <td class="tabla"><input name="num_pro[]" type="text" class="insert" id="num_pro" onFocus="tmp.value=this.value;this.select()" onChange="if (isInt(this,tmp)) cambiaPro({i})" onKeyDown="if (event.keyCode == 13) num_fact[{i}].focus()" size="3" maxlength="4">
        <input name="nombre_pro[]" type="text" class="vnombre" id="nombre_pro" size="30" readonly="true"></td>
      <td class="tabla"><input name="num_fact[]" type="text" class="insert" id="num_fact" onFocus="tmp.value=this.value;this.focus()" onChange="this.value=this.value.replace(/[^a-zA-Z0-9ñÑ]/g,'');this.value=this.value.toUpperCase();" onKeyDown="if (event.keyCode == 13) num_cia[{next}].select()" size="8"></td>
    </tr>
	<!-- END BLOCK : fila -->
  </table>  <p>
...
function cambiaCia(i) {
	if (f.num_cia[i].value == "" || f.num_cia[i].value == "0") {
		...
		if (i > 0) {
			...
			f.num_fact[i].value = f.num_fact[i - 1].value.replace(/[^a-zA-Z]/g, '').toUpperCase();
			f.num_fact[i].setSelectionRange(f.num_fact[i].value.length, f.num_fact[i].value.length);
		}
	}
	else {
...
<td align="center" valign="middle"><!--<p style="font-family: Arial, Helvetica, sans-serif; font-size: 12pt;">Las siguientes facturas no corresponden a la compa&ntilde;&iacute;a capturada </p>-->
  ...
  <p>
    <input type="button" class="boton" value="Siguiente" onClick="document.location='./fac_cop_fac.php'">
  </p></td>
</tr>
</table>
<!-- END BLOCK : errores -->
