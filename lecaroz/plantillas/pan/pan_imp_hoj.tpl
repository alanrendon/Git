...
<tr>
<td align="center" valign="middle"><p class="title">Impresi&oacute;n de Hoja Diaria de Panaderias</p>
  <form action="hoja_diaria.php" method="get" name="form" target="hoja">
    ...
    <tr>
      <th class="vtabla" scope="row">Nuevos</th>
      <td class="vtabla"><input name="nuevos" type="checkbox" id="nuevos" value="1" checked="checked" /></td>
    </tr>
  </table>  
    <p>
...
function validar() {
	if (f.fecha.value.length < 8) {
		...
		var win = window.open("","hoja","toolbar=no,location=no,directories=no,status=no,menubar=yes,scrollbars=yes,resizable=yes,width=800,height=600");
		f.submit();
		win.focus();
	}
}
