...
<form name="form" method="get" action="./ban_che_imp2.php">
...
<script language="javascript" type="text/javascript">
	function valida_registro() {
		if (document.form.tipo[1].checked) {
			...
			else if (document.form.ultimo_folio.value <= 0) {
			...
			if (document.form.ultimo_folio.value <= 0) {
				alert("Debe especificar el último folio de los cheques");
				document.form.ultimo_folio.select();
				return false;
			}
...
<script language="javascript" type="text/javascript">
	function imprimir() {
		...
		if (confirm(mensaje)) {
			...
			window.open("","cheques2","toolbar=no,location=no,directories=no,status=no,menubar=yes,scrollbars=yes,resizable=no,width=800,height=600");
			// Cambiar propiedades del formulario
			document.form.target = "cheques2";
			document.form.method = "post";
			document.form.action = "./ban_gen_che2.php";
			document.form.submit();
