...
<td align="center" valign="middle"><p class="title">Captura de Otros Dep&oacute;sitos</p>
  <form action="./ban_dep_otros_v3.php" method="get" name="form">
...
<tr>
<td align="center" valign="middle"><p class="title">Captura de Otros Dep&oacute;sitos</p>
  <form action="./ban_dep_otros_v3.php" method="post" name="form">
    <input name="tmp" type="hidden" id="tmp" />
  ...
  <table class="tabla">
    ...
    <tr>
      ...
      <td class="tabla"><input name="num_fact1[]" type="text" class="insert" id="num_fact1" onfocus="tmp.value=this.value;this.select()" onchange="if (isInt(this,tmp)) getRem({i},1)" onkeydown="movCursor(event.keyCode,num_fact2{index},concepto{index},num_fact2{index},num_fact1{back},num_fact1{next})" value="{num_fact1}" size="5" />
        <input name="imp1[]" type="text" id="imp1" value="{imp1}" />
        <input name="pag1[]" type="text" id="pag1" value="{pag1}" />
        <input name="sal1[]" type="text" id="sal1" value="{sal1}" />
        ...
        <input name="num_fact4[]" type="text" class="insert" id="num_fact4" onfocus="tmp.value=this.value;this.select()" onchange="if (isInt(this,tmp)) getRem({i},4)" onkeydown="movCursor(event.keyCode,num_cia{next},num_fact3{index},null,num_fact4{back},num_fact4{next})" value="{num_fact4}" size="5" />
...
function getRem(i, r) {
	...
	myConn.connect("./ban_dep_otros_v3.php", "GET", 'num=' + num + '&rem=' + rem + '&i=' + i + '&r=' + r, procRem);
}
...
var procRem = function(oXML) {
	...
	if (get_val2(data[0]) < 0) {
		...
		f.eval('num_fact' + data[2])[get_val2(data[1])].value = f.tmp.value;
		f.eval('num_fact' + data[2])[get_val2(data[1])].select();
		return false;
	}
	
	// Importe de la remisión e importe de lo pagado con anterioridad
	f.eval('imp' + data[1])[get_val2(data[0])].value = get_val2(data[2]);
	f.eval('pag' + data[1])[get_val2(data[0])].value = get_val2(data[3]);
...
function descRem(i) {
	...
	var imp = new Array(), pag = new Array(), sal = new Array(), rem = new Array();
	...
	// Arreglo con los importes pagados por remision
	for (j = 1; j <= 4; j++)
		if (get_val(f.eval('num_fact' + j)[i]) > 0) {
			if (rem[get_val(f.eval('num_fact' + j)[i])] != null)
				rem[get_val(f.eval('num_fact' + j)[i])] += Round(get_val(f.eval('importe')[i]), 2);
			else
				rem[get_val(f.eval('num_fact' + j)[i])] = Round(get_val(f.eval('importe')[i]), 2);
			alert(rem[get_val(f.eval('num_fact' + j)[i])]);
		}
	
	// Obtener totales pagados en la captura de las remisiones del proveedor a revisar
	if (f.id.length == undefined) {
		
	}
	for (var k = 0; k < f.id.length; k++)
		if (k != i && get_val(f.id[k]) > 0 && get_val(f.id[k]) == get_val(f.id[i]))
			for (j = 1; j <= 4; j++)
				if (rem[get_val(f.eval('num_fact' + j)[k])] != null)
					rem[get_val(f.eval('num_fact' + j)[k])] += Round(get_val(f.eval('importe')[k]), 2);
	...
	for (j = 1; j <= 4; j++)
		...
		else if (importe > 0 && imp[j] - pag[j] - (rem[get_val(f.eval('num_fact' + j)[i])] != null ? rem[get_val(f.eval('num_fact' + j)[i])] : 0) <= importe) {
			importe -= imp[j] - (pag[j] + (rem[get_val(f.eval('num_fact' + j)[i])] != null ? rem[get_val(f.eval('num_fact' + j)[i])] : 0));
		...
		// Si el importe del depósito no cubre la remisión, descontarlo y calcular el remanente de la remisión
			...
			sal[j] = Round(imp[j] - pag[j] - (rem[get_val(f.eval('num_fact' + j)[i])] != null ? rem[get_val(f.eval('num_fact' + j)[i])] : 0) - importe, 2);
			importe = 0;
...
function validar() {
	...
	for (var i = 0; i < f.num_cia.length; i++)
		if (get_val(f.num_cia[i]) > 0 && get_val(f.dia[i]) > 0 && get_val(f.importe[i])) {
			...
			// Validar en caso de que nombre sea mayor a 0 que la suma total de las remisiones sean igual o mayor al import del depósito
			else if (get_val(f.num[i]) > 0 && get_val(f.importe[i]) > Round(Round(get_val(f.imp1[i]) - get_val(f.pag1[i]), 2) + Round(get_val(f.imp2[i]) - get_val(f.pag2[i]), 2) + Round(get_val(f.imp3[i]) - get_val(f.pag3[i]), 2) + Round(get_val(f.imp4[i]) - get_val(f.pag4[i]), 2), 2)) {
