...
<td align="center" valign="middle"><p class="title">Pago Manual de Proveedores</p>
  <form action="./ban_pma_pro_v3.php" method="get" name="form">
...
<td align="center" valign="middle"><p class="title">Pago Manual de Proveedores </p>
  ...
  <br>
  <form action="./ban_pma_pro_v3.php" method="post" name="form">
  <input type="hidden" name="temp">
  ...
  <table width="80%" class="tabla">
	...
	<tr onMouseOver="overTR(this,'#ACD2DD');" onMouseOut="outTR(this,'');">
      <td class="tabla"><input name="id[]" type="checkbox" id="id" value="{id}" onChange="validar_importe({i},{block})" {checked}></td>
...
<script language="javascript" type="text/javascript">
	...
	function check_block(block, check) {
		if (form.tipo.value == "0" || !check.checked) {
			if (form.id.length == undefined) {
				...
				for (var i = parseInt(form.ini.length == undefined ? form.ini.value : form.ini[block].value); i <= parseInt(form.fin.length == undefined ? form.fin.value : form.fin[block].value); i++) {
					form.id[i].checked = check.checked == true ? true : false;
				}
			}
		}
	...
	function total_block(block) {
		...
		if (form.id.length == undefined) {
			...
			for (i = parseInt(form.ini.length == undefined ? form.ini.value : form.ini[block].value); i <= parseInt(form.fin.length == undefined ? form.fin.value : form.fin[block].value); i++) {
	...
	function cancelar() {
		if (confirm("¿Desea cancelar el pago a proveedores?"))
			document.location = "./ban_pma_pro_v3.php?cancelar=1";
		else
			return false;
	}
	...
	function siguiente() {
		...
		if (confirm("¿Desea generar los pagos y pasar al siguiente proveedor?")) {
			...
			if (count < 1) {
				...
				form.action = "./ban_pma_pro_v3.php?generar=1&siguiente=1";
				form.submit();
			}
		}
		else if (confirm("¿Desea pasar al siguiente proveedor?")) {
			form.action = "./ban_pma_pro_v3.php?siguiente=1";
			form.submit();
		}
		else {
			return false;
	...
	function terminar() {
		if (confirm("¿Desea generar los pagos y terminar el proceso?")) {
			...
			form.action = "./ban_pma_pro_v3.php?generar=1&terminar=1";
			form.submit();
		}
		else {
			return false;
...
<tr>
  ...
  <tr>
    <th class="tabla">Compa&ntilde;&iacute;a que pagara</th>
    <th class="tabla"><!-- <input name="num_cia_pago" type="text" class="insert" id="num_cia_pago" onFocus="temp.value=this.value;this.select()" onChange="if (isInt(this,temp)) cambiaCiaPago(this,nombre_cia_pago)" onKeyDown="if (event.keyCode == 13) this.blur()" size="3">
      <input name="nombre_cia_pago" type="text" disabled class="vnombre" id="nombre_cia_pago" size="30"> -->
      <select name="num_cia_pago" id="num_cia_pago" class="insert"{cia_pago_disabled}>
      	<option value="" selected=""></option>
      	<!-- START BLOCK : otra_cia -->
      	<option value="{num_cia}">{num_cia} {nombre_cia} - {saldo}</option>
      	<!-- END BLOCK : otra_cia -->
      </select>
    </th>
  </tr>
...
<script language="javascript" type="text/javascript">
	var form = document.form;
	var OK = true;
	var cia = new Array(), cia_pago = [];
	<!-- START BLOCK : cia -->
	cia[{num_cia}] = "{nombre}";
	<!-- END BLOCK : cia -->
	<!-- START BLOCK : cia_pago -->
	cia_pago[{num_cia}] = "{nombre}";
	<!-- END BLOCK : cia_pago -->
	...
	function cambiaCiaPago(num, nombre) {
		if (num.value == "") {
			nombre.value = "";
		}
		else if (cia_pago[num.value] != null) {
			nombre.value = cia_pago[num.value];
		}
		else {
	...
	function valid_check(i) {
		...
		if (total > saldo && form.num_cia_pago.value == '') {
			alert("No puede pagar mas del saldo disponible");
			if (form.id.length == undefined)
				form.id.checked = false;
			else
