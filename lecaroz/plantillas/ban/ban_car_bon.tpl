...
<tr>
<td align="center" valign="middle"><p class="title">Carta de Bonificaciones</p>
  <form action="./ban_car_bon.php" method="post" name="form" target="carta">
...
function cambiaCiaDep(i) {
	if (f.num_cia_dep[i].value == '' || f.num_cia_dep[i].value == '0') {
		...
		// Pedir datos
		myConn.connect('./ban_car_bon.php', 'GET', 'c=' + get_val(f.num_cia_dep[i]) + '&i=' + i, obtenerCiaDep);
	}
}
...
function cambiaCiaDes(i) {
	if (f.num_cia_des[i].value == '' || f.num_cia_des[i].value == '0') {
		...
		// Pedir datos
		myConn.connect('./ban_car_bon.php', 'GET', 'c=' + get_val(f.num_cia_des[i]) + '&cp=' + get_val(f.num_cia_dep[i]) + '&i=' + i, obtenerCiaDes);
	}
}
