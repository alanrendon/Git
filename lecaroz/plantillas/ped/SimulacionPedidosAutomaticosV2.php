			...
			foreach ($inventario as $cia => $datos) {
				foreach ($datos['productos'] as $codmp => $pro) {
					...
					if ($_REQUEST['existencia'] > 1) {
						...
						$consumo_mes = $con2['consumo'] > 0 ? $con2['consumo'] : $con1['consumo'];
						...
						$consumo_dia = $con2['consumo'] > 0 ? $consumo_mes / 30 : ($con_dia > 0 ? $consumo_mes / intval($con_dia, 10) : 0);
						
						$consumo_fecha = $con2['consumo'] > 0 ? $con2['fecha'] : $con1['fecha'];
