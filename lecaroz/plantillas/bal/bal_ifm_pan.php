...
if (isset($_GET['go'])) {
	...
	$sql .= $_GET['num_cia'] > 0 ? "= $_GET[num_cia]" : '< 100';
	$sql .= " AND ((num_cia, codmp) IN (SELECT num_cia, codmp FROM mov_inv_real WHERE fecha >= cast('$fecha' as date) - interval '2 months' AND num_cia < 100 GROUP BY num_cia, codmp) OR existencia != 0) ORDER BY num_cia, tipo, cm.nombre";
	$result = $db->query($sql);
	...
	foreach ($result as $reg) {
		if ($num_cia != $reg['num_cia']) {
			if ($num_cia != NULL) {
				...
				$data .= "\$sql = \"DELETE FROM `inv_fin_mes` WHERE `num_cia` = '\$num_cia'\";\r\n";
				$data .= "\$db->query(\$sql);\r\n\r\n";
				
				$data .= "foreach (\$avio as \$cod => \$exi) {\r\n";
				$data .= "\t\$sql = \"INSERT INTO `inventario` (`num_cia`, `IdCodMP`, `Existencia`, `fecha`) VALUES ('\$num_cia', '\$cod',";
				$data .= " '\$exi', '\$fecha')\";\r\n";
				$data .= "\t\$db->query(\$sql);\r\n";
				$data .= "}\n";
				$data .= "?>\n";
		...
		$data .= ($cont > 0 ? ",\r\n" : '') . "$reg[codmp] => array('nombre' => $reg[nombre], 'fecha' => '$reg[fecha]', 'unidad' => '$reg[unidad]')";
		$cont++;
	}
	if ($num_cia != NULL) {
		$data .= ");\n\n";
