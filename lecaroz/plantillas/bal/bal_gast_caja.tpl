...
<form name="form" action="./bal_gast_caja.php" method="get">
...
<form name="form" action="./bal_gast_caja.php" method="post">
<input name="temp" type="hidden">
...
<input name="contador" type="hidden" class="insert" id="contador" size="13" maxlength="5" value="{contador}">
    <tr class="tabla" align="center">
      <td>
	  	<p>
	  	  <input name="num_cia{i}" type="text" class="insert" id="num_cia{i}" size="3" maxlength="3" onKeyDown="if(event.keyCode == 13){document.form.importe{i}.select();}" onChange="valor=isInt(this,form.temp); if (valor==false) this.select(); actualiza_compania(this,form.nombre_cia{i})" onFocus="form.temporal2.value=this.value">
          <input name="nombre_cia{i}" type="text" class="vnombre" id="nombre_cia{i}" size="28" readonly>
		</p>
  	  </td>
      <td>
        <input name="importe{i}" type="text" class="insert" id="importe{i}" size="8" maxlength="15" onChange="valor=isFloat(this,2,form.temporal); if (valor==false) this.select(); totales1(this,form.operador{i});" onKeyDown="if(event.keyCode == 13){document.form.num_cia{next}.select();}" onFocus="form.temp.value=this.value; form.temporal.value=this.value;">
		</td>

      <td>
	  <select name="concepto{i}" size="1" class="insert" id="select">
        <!-- START BLOCK : codigo -->
	    <option value="{num_gasto}" {checked}>{descripcion}</option>
        <!-- END BLOCK : codigo -->
      </select></td>
	  <td>
	  <input type="text" size="25" class="insert" name="comentario{i}">
	  </td>
      <td>
      <select name="balance{i}" size="1" class="insert" id="balance{i}">
        <option value="true" {selec}>Si Balances</option>
        <option value="false"  {selec1}>No balance</option>
      </select>
	  </td>
      <td>
	  	<input name="fecha{i}" type="hidden" class="insert" id="fecha{i}" value="{fecha}" size="10" maxlength="10">
	  	<input name="operador{i}" type="hidden" class="insert" id="operador{i}" value="{operador}" size="10" maxlength="10">
    
          <label>
          <input type="radio" name="tipo_mov{i}" value="0" {sel} onChange="document.form.operador{i}.value=0; totales2(form.importe{i},form.operador{i})">
  E</label>
          <label>
          <input name="tipo_mov{i}" type="radio" value="1"{sel1} onChange="document.form.operador{i}.value=1; totales2(form.importe{i},form.operador{i})" >
			I</label>
</td>
    </tr>
<!-- END BLOCK : rows -->
...
<form name="form" action="./insert_bal_gast_caja.php" method="post">
<input name="contador1" type="hidden" value="{contador}">
<table class="print">
  <tr class="print">
  ...
  <tr class="print">
    <td class="print">{num_cia} <input name="num_cia{i}" type="hidden" value="{num_cia}"></td>
    <td class="vprint">{nombre_cia}</td>
    <td class="vprint">{concepto1} <input name="concepto{i}" type="hidden" size="5" value="{concepto}">
	<input name="comentario{i}" type="hidden" size="5" value="{comentario}">
	</td>
	
    <td class="print">{balance1} <input name="balance{i}" type="hidden" size="5" value="{balance}"></td>
    <td class="vprint">{tipo1}<input name="tipo_mov{i}" type="hidden" id="tipo_mov{i}" value="{tipo}" size="5"></td>
    <td class="print">{importe1}<input name="importe{i}" type="hidden" id="importe{i}" value="{importe}" size="5">	<input name="fecha{i}" type="hidden" id="fecha{i}" value="{fecha}" size="5"></td>
  </tr>
