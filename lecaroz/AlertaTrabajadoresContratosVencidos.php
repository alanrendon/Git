	...
	switch ($_REQUEST['accion']) {
		case 'verificar':
			$sql = '
				...
				WHERE
					...
					AND fecha_termino_contrato <= NOW()::DATE
		...
		case 'listado':
			...
			if ($result && in_array($_SESSION['iduser'], array(1, 4, 28, 34, 40, 52, 53, 54, 59, 58, 61))) {
				$tpl = new TemplatePower('plantillas/AlertaTrabajadoresContratosVencidos.tpl');
				$tpl->prepare();
				...
				foreach ($result as $rec) {
					if ($num_cia != $rec['num_cia']) {
						if ($num_cia != NULL) {
							...
							$mail->Subject = '[' . $num_cia . ' ' . $nombre_cia . '] Contratos de trabajadores vencidos [' . date('d/m/Y H:i:s') . ']';
						...
						$tpl = new TemplatePower('plantillas/nom/EmailContratosVencidos.tpl');
						$tpl->prepare();
