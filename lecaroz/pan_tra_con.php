...
$tpl->assignInclude("body","./plantillas/pan/pan_tra_con.tpl");
$tpl->prepare();
...
if($_GET['tipo_con']==0){
	if($_GET['cia']=="")
		$sql = "SELECT * FROM catalogo_trabajadores ct LEFT JOIN catalogo_companias cc USING (num_cia) LEFT JOIN catalogo_operadoras co USING (idoperadora) WHERE fecha_baja IS NULL AND co.iduser = $_SESSION[iduser] AND num_cia_emp <= 300 ORDER BY num_cia, nombre_completo";
	else
		$sql="SELECT * FROM catalogo_trabajadores ct LEFT JOIN catalogo_companias cc USING (num_cia) LEFT JOIN catalogo_operadoras co USING (idoperadora) WHERE num_cia=".$_GET['cia']." AND fecha_baja IS NULL AND co.iduser = $_SESSION[iduser] AND num_cia_emp <= 300 ORDER BY nombre_completo";
}
else{
	$sql="select * from catalogo_trabajadores ct LEFT JOIN catalogo_companias cc USING (num_cia) LEFT JOIN catalogo_operadoras co USING (idoperadora) where nombre_completo like '%".strtoupper($_GET['nombre'])."%' and fecha_baja is null AND co.iduser = $_SESSION[iduser] AND num_cia_emp <= 300 order by ap_paterno, ap_materno, nombre";
}
