...
if (isset($_GET['num_cia'])) {
	...
	$result = $db->query($sql);//echo "<pre>";print_r($result);echo "</pre>";die;
	...
	for ($i = 0; $i < count($result); $i++) {
		...
		$temp = $db->query("SELECT importe, tipo FROM aguinaldos WHERE id_empleado = {$result[$i]['id']} AND fecha >= '$fecha_ini' AND tipo > 3 ORDER BY fecha DESC LIMIT 1");
	...
	$tpl->assign('leyenda', ' (con incidencias)');
	...
	for ($i = 0; $i < count($result); $i++) {
		if ($orden[0] == 'cia_aguinaldos' && $_GET['cod_puestos'] == "" && $_GET['cod_turno'] == "") {
			...
			$agu_act = $db->query("SELECT importe, tipo FROM aguinaldos WHERE id_empleado = {$result[$i]['id']} AND fecha >= '$fecha_ini' AND tipo > 3 ORDER BY fecha DESC LIMIT 1");
			if ($agu_act && $agu_act[0]['importe'] >= 20) {
				...
				if (isset($_GET['agu_act'])) {
					...
					if (isset($_GET['status'])) {
						...
						$tpl->assign("status", isset($status) ? $status : "&nbsp;");
					}
					$numfilas++;
				}
