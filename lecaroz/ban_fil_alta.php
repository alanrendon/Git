...
$tpl->assignInclude("body", "./plantillas/ban/ban_fil_alta.tpl");
$tpl->prepare();
...
if (isset($_POST['num_cia_primaria'])) {
	...
	for ($i = 0; $i < $numfilas; $i++)
		if ($_POST['num_cia_primaria'][$i] > 0 && $_POST['num_cia'][$i] > 0) {
			if ($db->query("SELECT id FROM catalogo_filiales WHERE num_cia_primaria = {$_POST['num_cia_primaria'][$i]} AND num_cia = {$_POST['num_cia'][$i]}")) {
				$tpl->newBlock("valid");
			...
			$first = $_POST['num_cia_primaria'][$i] == $_POST['num_cia'][$i] ? 'TRUE' : 'FALSE';
			$sql .= "INSERT INTO catalogo_filiales (num_cia_primaria, num_cia, first) VALUES ({$_POST['num_cia_primaria'][$i]}, {$_POST['num_cia'][$i]}, '$first');\n";
		}
