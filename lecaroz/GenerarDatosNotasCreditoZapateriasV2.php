...
{
	echo "GenerarDatosNotasCreditoZapateriasV2.php Ver 2.15.9.8";
	echo "\nCopyright (c) 2015, Lecaroz";
	echo "\n\nModo de empleo: php GenerarDatosNotasCreditoZapateriasV2.php [OPCIONES]";
...
if (isset($params['ids']) && trim($params['ids']) != '')
{
	...
	if (count($ids) > 0)
	{
		$condiciones[] = 'nc.id IN (' . implode(', ', $ids) . ')';
	}
}

if (isset($params['fecha1']) || isset($params['fecha2']))
{
	$condiciones[] = "nc.fecha BETWEEN '{$params['fecha1']}' AND '{$params['fecha2']}'";
}
else
{
	$condiciones[] = "nc.fecha >= NOW()::DATE - INTERVAL '3 MONTHS'";
}
...
if (isset($params['cias']) && trim($params['cias']) != '')
{
	...
	if (count($cias) > 0)
	{
		$condiciones[] = 'nc.num_cia_apl IN (' . implode(', ', $cias) . ')';
	}
}
...
if (isset($params['pros']) && trim($params['pros']) != '')
{
	...
	if (count($pros) > 0)
	{
		$condiciones[] = 'nc.num_proveedor IN (' . implode(', ', $pros) . ')';
	}
}
...
if ( ! $nvcf)
{
	$condiciones[] = 'nc.ts_carga_conta IS NULL';
}

$condiciones_string = implode(' AND ', $condiciones);

$sql = "SELECT
	COUNT(nc.id) AS num_rows
FROM
	notas_credito_zap nc
WHERE
	{$condiciones_string}";
...
while ($datos_facturas = $db->query("SELECT
	nc.id,
	nc.num_cia_apl AS num_cia,
	cc.razon_social AS nombre_cia,
	nc.num_proveedor AS num_pro,
	cp.nombre AS nombre_pro,
	cp.rfc AS rfc_pro,
	'NCZ' || nc.folio AS num_fact,
	nc.fecha,
	cp.calle,
	cp.no_exterior,
	cp.no_interior,
...
FROM
	notas_credito_zap nc
	LEFT JOIN catalogo_proveedores cp ON (cp.num_proveedor = nc.num_proveedor)
	LEFT JOIN catalogo_companias cc ON (cc.num_cia = nc.num_cia_apl)
WHERE
	{$condiciones_string}
ORDER BY
	nc.id
LIMIT {$rows_per_query} OFFSET {$offset}"))
{
	...
	foreach ($datos_facturas as $i => $df)
	{
		...
		$iva = ($df['total'] / (1 + (16 - 4) / 100)) * 0.16;
		$ret_4 = ($df['total'] / (1 + (16 - 4) / 100)) * 0.04;
		$subtotal = $df['total'] - $iva + $ret_4;
		$total = $df['total'];
		...
		if ($response === FALSE)
		{
			$db->query("UPDATE notas_credito_zap SET ts_carga_conta_error = 'Webservice no disponible' WHERE id = {$df['id']}");
			...
			if ( ! $nderr && $ob_err >= $max_ob_err)
			{
				...
				if ($ids)
				{
					...
					$db->query("UPDATE notas_credito_zap SET ts_carga_conta = NOW() WHERE id IN (" . implode(', ', $ids) . ")");
				}
		...
		else
		{
			...
			{
				$db->query("UPDATE notas_credito_zap SET ts_carga_conta_error = 'Imposible decodificar respuesta de openbravo: {$response}' WHERE id = {$df['id']}");
			...
			else if ($ob_data->status < 0)
			{
				...
				$db->query("UPDATE notas_credito_zap SET ts_carga_conta_error = '{$msg}' WHERE id = {$df['id']}");
...
function generar_xml_datos($datos, $guardar_xml = FALSE)
{
	...
	if ($guardar_xml)
	{
		...
		$nombre_xml = "carga-notas-credito-zap-{$datos['cabecera']['id']}-{$datos['cabecera']['num_cia']}-{$datos['cabecera']['clave_pro']}-{$datos['cabecera']['num_fact']}.xml";
