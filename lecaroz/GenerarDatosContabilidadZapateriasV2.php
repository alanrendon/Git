...
{
	echo "GenerarDatosContabilidadZapateriasV2.php Ver 2.15.9.8";
	echo "\nCopyright (c) 2015, Lecaroz";
	echo "\n\nModo de empleo: php GenerarDatosContabilidadZapateriasV2.php [OPCIONES]";
...
{
	$condiciones[] = "f.tscap::DATE BETWEEN '{$params['fecha1']}' AND '{$params['fecha2']}'";
}
else if ( ! isset($ids))
{
	$condiciones[] = "f.tscap::DATE >= NOW()::DATE - INTERVAL '3 MONTHS'";
}
...
LIMIT {$rows_per_query} OFFSET {$offset}"))
{
	...
	foreach ($datos_facturas as $i => $df)
	{
		...
		if ($response === FALSE)
		{
			$db->query("UPDATE facturas_zap SET ts_carga_conta_error = 'Webservice no disponible', ts_carga_conta = NULL WHERE id = {$df['id']}");
			...
			if ( ! $nderr && $ob_err >= $max_ob_err)
			{
				...
				if ($ids)
				{
					...
					$db->query("UPDATE facturas_zap SET ts_carga_conta = NOW(), ts_carga_conta_error = NULL WHERE id IN (" . implode(', ', $ids) . ")");
				}
		...
		else
		{
			...
			{
				$db->query("UPDATE facturas_zap SET ts_carga_conta_error = 'Imposible decodificar respuesta de openbravo: {$response}', ts_carga_conta = NULL WHERE id = {$df['id']}");
			...
			else if ($ob_data->status < 0)
			{
				...
				$db->query("UPDATE facturas_zap SET ts_carga_conta_error = '{$msg}', ts_carga_conta = NULL WHERE id = {$df['id']}");
