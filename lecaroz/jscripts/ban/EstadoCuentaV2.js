...
var consultar = function() {
	...
	if (!document.id('pendientes').get('checked') && !document.id('conciliados').get('checked')) {
		alert('Debe seleccionar \'Pendientes\', \'Conciliados\' o ambos');
		return false;
	}
	
	if (document.id('fecha1').get('value').trim() == '' && document.id('fecha2').get('value').trim() == ''
		&& document.id('conciliado1').get('value').trim() == '' && document.id('conciliado2').get('value').trim() == '') {
