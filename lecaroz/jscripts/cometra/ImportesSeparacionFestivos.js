...
window.addEvent('domready', function() {
	...
	$('dia_festivo').addEvent('change', ObtenerDia).fireEvent('change');
...
var ObtenerDia = function() {
	var fecha = JSON.decode($('dia_festivo').get('value'));
	
	new Request({
		'url': 'ImportesSeparacionFestivos.php',
		'data': 'accion=obtenerDia&mes=' + fecha.mes + '&dia=' + fecha.dia,
		'onRequest': function() {
			new Element('img', {
				'id': 'loading',
				'src': 'imagenes/_loading.gif'
			}).inject($('dia_festivo'), 'after');
		},
		'onSuccess': function(result) {
			...
			if (result != '') {
				...
				data.each(function(el, i) {
					$$('input[id=importe]')[i].set('value', el.importe > 0 ? el.importe.numberFormat(2, '.', ',') : '');
					$$('input[id=porcentaje]')[i].set('value', el.porcentaje > 0 ? el.porcentaje.numberFormat(2, '.', ',') : '');
				});
			}
		}
	}).send();
