...
var inicio = function () {
	new Request({
		url: 'TiposBajaTrabajadorCatalogo.php',
		data: 'accion=inicio',
		onRequest: function() {
			boxProcessing.open();
...
var alta = function() {
	new Request({
		...
		onSuccess: function(result) {
			...
			new FormValidator(document.id('alta_tipo'), {
				showErrors: true,
				selectOnFocus: true
			});
...
var do_alta = function() {
	if (document.id('descripcion').get('value') == '') {
		alert('Debe especificar la descripción del tipo');
		
		document.id('descripcion').focus();
	} else if (confirm('¿Son correctos todos los datos?')) {
		new Request({
			url: 'TiposBajaTrabajadorCatalogo.php',
			data: 'accion=do_alta&' + document.id('alta_tipo').toQueryString(),
			onRequest: function() {
				boxProcessing.open();
...
var modificar = function(id) {
	new Request({
		...
		onSuccess: function(result) {
			...
			new FormValidator(document.id('modificar_tipo'), {
				showErrors: true,
				selectOnFocus: true
			});
...
var do_modificar = function() {
	if (document.id('descripcion').get('value') == '') {
		...
		new Request({
			url: 'TiposBajaTrabajadorCatalogo.php',
			data: 'accion=do_modificar&' + document.id('modificar_tipo').toQueryString(),
			onRequest: function() {
				boxProcessing.open();
