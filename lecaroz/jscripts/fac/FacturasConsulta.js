...
{
	boxViewCFD.setContent('<iframe frameborder="0" width="800" height="400" src="FacturasConsulta.php?accion=visualizar_cfd&id=' + id + '"></iframe>').open();
}
...
var cargar_conta = function() {
	if ( ! ($$('input[id^=id_][data-tipo=1]:checked').length > 0 || $$('input[id^=id_][data-tipo=2]:checked').length > 0))
	{
	...
	new Request({
		...
		'data': {
			accion: 'cargar_conta',
			ids: $$('input[id^=id_][data-tipo=1]:checked').length > 0 ? $$('input[id^=id_][data-tipo=1]:checked').get('value') : null,
			ids_ros: $$('input[id^=id_][data-tipo=2]:checked').length > 0 ? $$('input[id^=id_][data-tipo=2]:checked').get('value') : null
		},
		'onRequest': function() {
			boxProcessing.open();
		},
