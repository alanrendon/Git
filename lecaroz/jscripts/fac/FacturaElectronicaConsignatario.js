...
var cambiaCia = function() {
	if ($('num_cia').get('value').getNumericValue() > 0) {
		new Request({
			'url': 'FacturaElectronicaConsignatario.php',
			...
			'onSuccess': function(result) {
				if (result != '') {
					...
					if ($('num_cia').get('value').getNumericValue() == 900 || $('num_cia').get('value').getNumericValue() == 992) {
...
var calcularTotal = function() {
	...
	$$('input[id=importe]').each(function(el, i) {
		...
		$$('input[id=descuento_row]')[i].set('value', el.get('value').getNumericValue() * porcentaje_descuento / 100);

		descuento += el.get('value').getNumericValue() * porcentaje_descuento / 100;
	});
