...
var Generar = function() {
	if ($('num_cia').get('value').getNumericValue() == 0) {
		...
		alert('Debe especificar la fecha de ingreso');
		$('fecha_inicio').select();
	}
	else if ($('fecha_termino').get('value').clean() == '') {
		...
		var url = 'ReciboFiniquito.php',
			arg = '?accion=recibo&' + $('Datos').toQueryString(),
			opt = 'toolbar=no,location=no,directories=no,status=no,menubar=yes,scrollbars=yes,resizable=yes,width=1024,height=768',
			win;
		
		win = window.open(url + arg, 'recibo', opt);
		win.focus();
	}
}
