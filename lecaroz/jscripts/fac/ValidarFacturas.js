...
var iniciar = function ()
{
	...
	document.id('registrar').addEvent('click', registrar_facturas);
}
...
var new_row = function(i)
{
	...
	var num_pro = new Element('input',
	{
		...
		class: 'validate focus toPosInt right',
		size: 3,
		value: i > 0 ? $$('input[id=num_pro]')[i - 1].get('value') : ''
	}).inject(td2);
	var nombre_pro = new Element('input',
	{
		...
		type: 'text',
		size: 30,
		value: i > 0 ? $$('input[id=nombre_pro]')[i - 1].get('value') : ''
	}).inject(td2);
	...
	var num_fact = new Element('input',
	{
		...
		size: 10,
		value: i > 0 ? $$('input[id=num_fact]')[i - 1].get('value').replace(/[^a-zA-Z]/g, '').toUpperCase() : ''
	}).inject(td3);
		...
		new Request({
			...
			onSuccess: function(result)
			{
				...
				else if (data.status == -1)
				{
					...
					document.id('status_' + i).set({
						html: 'No existe la factura, se guardara temporalmente',
						class: 'orange'
					});
				}
				...
				else if (data.status == -3)
				{
					...
					document.id('status_' + i).set({
						html: 'La factura ya esta pendiente para validaci&oacute;n desde el d&iacute;a ' + data.fecha_alta,
						class: 'orange'
					});
				}
				else if (data.status == -4)
				{
					...
					document.id('status_' + i).set({
						html: 'La factura ya fue validada ',
						class: 'red'
					});
				}
				else if (data.status == -5)
				{
					...
					document.id('status_' + i).set({
						html: 'La factura no corresponde a la compa&ntilde;&iacute;a, est&aacute; capturada en ' + data.num_cia + ' ' + data.nombre_cia,
						class: 'red'
					});
				}
			}
...
var registrar_facturas = function()
{
	if (confirm('¿Son correctos todos los datos?'))
	{
		new Request({
			url: 'ValidarFacturas.php',
			data: 'accion=registrar_facturas&' + document.id('datos_form').toQueryString(),
			onRequest: function()
			{
				boxProcessing.open();
			},
