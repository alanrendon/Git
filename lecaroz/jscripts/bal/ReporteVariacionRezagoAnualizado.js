window.addEvent('domready', function()
{
	...
	boxReporte = new mBox.Modal(
	{
		...
		{
			document.id('reporte_frame').set('src', 'ReporteVariacionRezagoAnualizado.php?accion=reporte&' + param);
		},
...
var inicio = function ()
{
	new Request(
	{
		url: 'ReporteVariacionRezagoAnualizado.php',
		data: 'accion=inicio',
		onRequest: function()
		{
			boxProcessing.open();
	...
	new Request(
	{
		...
		onSuccess: function(result)
		{
			if (result != '')
			{
				...
				{
					el.store('tip:title', '<img src="/lecaroz/iconos/info.png" /> Proveedores de gas');
					el.store('tip:text', el.get('data-info'));
				});

				tips_info = new Tips($$('a[id=info]'),
				{
					'fixed': true,
					'className': 'Tip',
					'showDelay': 50,
...
var exportar = function() {
	var url = 'ReporteVariacionRezagoAnualizado.php',
