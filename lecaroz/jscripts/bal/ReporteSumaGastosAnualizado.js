window.addEvent('domready', function()
{
	...
	boxReporte = new mBox.Modal(
	{
		...
		{
			document.id('reporte_frame').set('src', 'ReporteSumaGastosAnualizado.php?accion=reporte&' + param);
		},
...
var inicio = function()
{
	new Request(
	{
		url: 'ReporteSumaGastosAnualizado.php',
		data: 'accion=inicio',
		onRequest: function()
		{
			boxProcessing.open();
		...
		document.id('gastos').addEvents(
		{
			keydown: function(e) {
				if (e.key == 'enter') {
					...
					document.id('gastos_caja').select();
				}
			}
		});

		document.id('gastos_caja').addEvents(
		{
			keydown: function(e) {
				if (e.key == 'enter') {
					e.stop();
...
{
	var url = 'ReporteSumaGastosAnualizado.php',
