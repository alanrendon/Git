	...
	new Request({
		...
		'onSuccess': function(result) {
			if (result) {
				...
				$('pedido').addEvent('click', /*realizarPedido*/Anotaciones);
...
var calcularEntrega = function() {
	...
	if ($('cantidad' + i).get('value').getNumericValue() > 0 && $('presentacion' + i).get('value') != '') {
		...
		entregar = $('tomar_consumo' + i).get('checked') ? Number.ceil($('cantidad' + i).get('value').getNumericValue() / data[1].getNumericValue()) : $('cantidad' + i).get('value').getNumericValue();
...
var Anotaciones = function() {
	...
	new Request({
		...
		'data': 'accion=anotaciones&' + $$('select[id^=num_pro]').get('value').getNumericValue().filter(function(val, i) { return $('id' + i).get('checked') && val > 0; }).map(function(val) { return 'num_pro[]=' + val; }).join('&'),
		'onRequest': function() {
		},
		'onSuccess': function(result) { 
...
var realizarPedido = function() {
	...
	new Request({
		'url': 'PedidosPanaderias.php',
		'data': 'accion=realizarPedido&' + $('Datos').toQueryString() + '&' + queryString.join('&'),
		'onRequest': function() {
			popup.Close();
