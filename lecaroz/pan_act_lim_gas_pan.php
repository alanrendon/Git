...
$tpl->assignInclude("body", "./plantillas/pan/pan_act_lim_gas_pan.tpl");
$tpl->prepare();
...
if (isset($_GET['num_cia'])) {
	...
	// Obtener control
	$sql = "SELECT num_cia, codgastos, descripcion, limite FROM catalogo_limite_gasto LEFT JOIN catalogo_gastos USING (codgastos) LEFT JOIN catalogo_companias USING (num_cia) LEFT JOIN catalogo_operadoras USING (idoperadora) WHERE";
	...
	if (!$result)
		die(header('location: ./pan_act_lim_gas_pan.php?codigo_error=1'));
	...
	$ftp = @ftp_connect($ftp_server) or die(header('location: ./pan_act_lim_gas_pan.php?codigo_error=2'));
	
	// Iniciar sesión en el servidor FTP
	if (!@ftp_login($ftp, $ftp_user, $ftp_pass))
		die(header('location: ./pan_act_lim_gas_pan.php?codigo_error=3'));
	...
	if (!ftp_chdir($ftp, $rdir))
		die(header('location: ./pan_act_lim_gas_pan.php?codigo_error=4'));
	...
	foreach ($result as $reg) {
		if ($num_cia != $reg['num_cia']) {
			if ($num_cia != NULL) {
				...
				$data .= "\$sql = \"TRUNCATE TABLE `catgastos`\";\r\n";
				$data .= "\$db->query(\$sql);\r\n\r\n";
			...
			$fp = fopen("$ldir/$num_cia.php", 'wb+') or die(header('location: ./pan_act_lim_gas_pan.php?codigo_error=5'));
...
$result = $db->query("SELECT num_cia, nombre_corto AS nombre FROM catalogo_companias WHERE num_cia < 100");
foreach ($result as $reg) {
	$tpl->newBlock('c');
