...
*/
$fecha1 = date('d/m/Y', mktime(0, 0, 0, 1, 1, $anyo));
/*
@@@ Fecha de fin de mes
*/
$fecha2 = date('d/m/Y', mktime(0, 0, 0, 12, 31, $anyo));
...
function hoja2($num_cia, $anyo, $tipo, $new) {
	...
	$tpl->assign('anyo3', $anyo - 2);
}

function hoja4($num_cia, $anyo) {
	global $MonthName, $tpl;
...
$sql = '
	SELECT
		...
		(
			...
			AS
				inv_ant_ant,
		SUM(compras + desc_compras + desc_pagos)
			AS
				compras,
		SUM(desc_compras)
			AS
		...
		SUM(dev_otros_meses)
			AS
				dev_otros_meses,
		SUM(dev_otras_tiendas)
			AS
				dev_otras_tiendas,
		SUM(dev_por_otras_tiendas)
			AS
		...
		SUM(utilidad_bruta)
			AS
				utilidad_bruta,
		SUM(gastos_operacion)
			AS
				gastos_operacion,
		SUM(gastos_generales)
			AS
				gastos_generales,
		SUM(gastos_caja)
			AS
		...
		SUM(reservas)
			AS
				reservas,
		SUM(pagos_anticipados)
			AS
				pagos_anticipados,
		SUM(gastos_otras_cias)
			AS
				gastos_otras_cias,
		SUM(total_gastos)
			AS
...
$tpl = new TemplatePower('./plantillas/bal/balance_zap_anual.tpl');
$tpl->prepare();
...
foreach ($balances as $bal) {
	...
	*/
	...
	$tpl->assign('un_c', $un_act != $un_ant && $un_c <= 100 ? '<span class="' . ($un_act > $un_ant ? 'Blue">MBR ' : 'Red">EBR ') . number_format($un_c, 2, '.', ',') . '%</span>' : '&nbsp;');
	
	/*
	@ 
	*/
	$tpl->assign('utilidad_neta_inv', round($bal['utilidad_neta'] + ($bal['inv_act'] - $bal['inv_ant']), 2) != 0 ? '<span class="' . ($bal['utilidad_neta'] + ($bal['inv_act'] - $bal['inv_ant']) > 0 ? 'Blue' : 'Red') . '">' . number_format($bal['utilidad_neta'] + ($bal['inv_act'] - $bal['inv_ant']), 2, '.', ',') . '</span>' : '&nbsp;');
	
	$uni_act = $bal['utilidad_neta'] - $bal['ingresos_ext'] + ($bal['inv_act'] - $bal['inv_ant']);
	$uni_ant = $bal['utilidad_neta_ant'] - $bal['ingresos_ext_ant'] + ($bal['inv_act_ant'] - $bal['inv_ant_ant']);
	$uni_c = $uni_ant > 0 ? abs(($uni_act) * 100 / ($uni_ant) - 100) : 0;
	$tpl->assign('uni_c', $uni_act != $uni_ant && $uni_c <= 100 ? '<span class="' . ($uni_act > $uni_ant ? 'Blue">MBR+INV ' : 'Red">EMB+INV ') . number_format($uni_c, 2, '.', ',') . '%</span>' : '&nbsp;');
	...
	*/
	if ($anyo >= 2009) {
		...
		$tpl->assign('porcentaje', $anyo >= 2010 ? '3' : '2');
		$porcentaje = $anyo >= 2010 ? 0.03 : 0.02;
		
		$impuesto_ide = round(($efectivo[0]['efectivo'] - 25000) * $porcentaje, 2);
		
		if ($impuesto_ide > 0) {
			$tpl->assign('efe_fijo', number_format($efectivo[0]['efectivo'], 2, '.', ','));
			$tpl->assign('efe_2por', number_format($impuesto_ide, 2, '.', ','));
		}
		else
					...
					(
							...
							WHERE
								...
								AND
									fecha
										BETWEEN
												\'' . date('d/m/Y', mktime(0, 0, 0, 1, 1, $anyo - 2)) . '\'
											AND
												\'' . $fecha2 . '\'
								AND
									codigo_edo_resultados
		...
		*/
		...
		foreach ($gastos as $g) {
			if ($tipo != $g['tipo']) {
				...
				$size += 7.5;
				if ($size > $page_size) {
					hoja2($bal['num_cia'], $anyo, $tipo, FALSE);
					$size = 0;
					/*
					@ Incrementar el número de hojas +1
					*/
			...
			if ($cod != $g['cod']) {
				...
				if ($size > $page_size) {
					hoja2($bal['num_cia'], $anyo, $tipo, TRUE);
					$size = 0;
					/*
					@ Incrementar el número de hojas +1
					*/
				...
				if ($anyo >= 2007
					...
					for ($col = 1; $col <= 3; $col++) {
						...
						if ($anyo_gasto >= 2009 && !$imp_ide_ins[$col]) {
							/*
							...
							$sql = '
								...
								WHERE
									...
									AND
										fecha_con
											BETWEEN
													\'' . date('d/m/Y', mktime(0, 0, 0, 1, 1, $anyo_gasto)) . '\'
												AND
													\'' . date('d/m/Y', mktime(0, 0, 0, 12, 31, $anyo_gasto)) . '\'
									AND
										cod_mov
											IN
												(
				...
				$sql = '
					...
					WHERE
						...
						AND
							fecha_con
								BETWEEN
										\'' . date('d/m/Y', mktime(0, 0, 0, 1, 1, $g['anyo'])) . '\'
									AND
										\'' . date('d/m/Y', mktime(0, 0, 0, 12, 31, $g['anyo'])) . '\'
						AND
							cod_mov
								IN
									(
	...
	$sql = '
		SELECT
			...
			CASE
				WHEN anio = ' . $anyo . ' THEN
					1
				WHEN anio = ' . ($anyo - 1) . ' THEN
					2
				WHEN anio = ' . ($anyo - 2) . ' THEN
					3
			END
				AS
					col
