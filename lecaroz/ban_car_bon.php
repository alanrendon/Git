...
if (isset($_GET['c'])) {
	...
	$sql .= isset($_GET['cp']) && $_GET['cp'] > 0 && $_SESSION['iduser'] < 28 ? " AND num_cia_primaria = $_GET[cp]" : '';
	$sql .= ' AND num_cia BETWEEN ' . ($_SESSION['tipo_usuario'] == 2 ? '900 AND 998' : '1 AND 899');
	$result = $db->query($sql);
	
	if (!$result && isset($_GET['cp'])) {
		$sql = "SELECT nombre_corto AS nombre FROM catalogo_companias WHERE num_cia = $_GET[cp]";
		$sql .= isset($_GET['c']) && $_GET['c'] > 0 && $_SESSION['tipo_usuario'] == 2 ? " AND num_cia_primaria = $_GET[c]" : '';
		$sql .= ' AND num_cia BETWEEN ' . ($_SESSION['tipo_usuario'] == 2 ? '900 AND 998' : '1 AND 899');
		$result = $db->query($sql);
	}
...
if (isset($_POST['cuenta'])) {
	...
	for ($i = 0; $i < 10; $i++)
		...
		if (count($reg) > 0) {
			...
			$tpl->assignInclude("body","./plantillas/ban/ban_car_bon.tpl");
			$tpl->prepare();
