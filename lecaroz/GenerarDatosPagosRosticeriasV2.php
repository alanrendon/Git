...
{
	echo "GenerarDatosPagosRosticeriaV2.php Ver 2.15.9.8";
	echo "\nCopyright (c) 2015, Lecaroz";
	echo "\n\nModo de empleo: php GenerarDatosPagosRosticeriaV2.php [OPCIONES]";
	...
	foreach ($datos_pago as $i => $dp)
	{
		if ($id_pago != $dp['id_pago'])
		{
			if ($id_pago != NULL)
			{
				...
				if ( ! $no_cargar)
				{
					...
					if ($response === FALSE)
					{
						$db->query("UPDATE total_fac_ros SET ts_carga_pago_conta_error = 'Webservice no disponible', ts_carga_pago_conta = NULL WHERE id IN (" . implode(', ', $facs_id) . ")");
						...
						if ( ! $nderr && $ob_err >= $max_ob_err)
						{
							...
							if ($ids)
							{
								...
								$db->query("UPDATE total_fac_ros SET ts_carga_pago_conta = NOW(), ts_carga_pago_conta_error = NULL WHERE id IN (" . implode(', ', $ids) . ")");
							}
					...
					else
					{
						...
						{
							$db->query("UPDATE total_fac_ros SET ts_carga_pago_conta_error = 'Imposible decodificar respuesta de openbravo: {$response}', ts_carga_pago_conta = NULL WHERE id IN (" . implode(', ', $facs_id) . ")");
						...
						else if ($ob_data->status < 0)
						{
							...
							$db->query("UPDATE total_fac_ros SET ts_carga_pago_conta_error = '{$msg}', ts_carga_pago_conta = NULL WHERE id IN (" . implode(', ', $facs_id) . ")");
		...
		if ($dp['carga_factura'] == 'f' || $forzar_carga)
		{
			...
			$query_fac = $db->query("SELECT
				...
				cp.codigo_postal,
				cp.email1 AS email,
				'COMPRA DE POLLO' AS descripcion,
				33 AS codigo,
			...
			if ( ! $no_cargar)
			{
				...
				if ($response === FALSE)
				{
					$db->query("UPDATE total_fac_ros SET ts_carga_conta_error = 'Webservice no disponible', ts_carga_conta = NULL WHERE id = {$dp['id_factura']}");
				...
				else
				{
					...
					if ( ! isset($ob_data->status))
					{
						$db->query("UPDATE total_fac_ros SET ts_carga_conta_error = 'Imposible decodificar respuesta de openbravo: {$response}', ts_carga_conta = NULL WHERE id = {$dp['id_factura']}");
					...
					else if ($ob_data->status < 0)
					{
						...
						$db->query("UPDATE total_fac_ros SET ts_carga_conta_error = '{$msg}', ts_carga_conta = NULL WHERE id = {$dp['id_factura']}");
					...
					else
					{
						...
						$db->query("UPDATE total_fac_ros SET ts_carga_conta = NOW(), ts_carga_conta_error = NULL WHERE id = {$dp['id_factura']}");
					}
				}
			}
			else
