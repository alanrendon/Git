...
$tpl->assignInclude("body","./plantillas/pan/pan_rev_sol_v2.tpl");
$tpl->prepare();
...
*/	
	$sql="select num_cia, idoperadora, catalogo_operadoras.nombre from catalogo_companias join catalogo_operadoras using(idoperadora) where num_cia in (select distinct(num_cia) from modificacion_pastel where fecha_autorizacion is null union select distinct(num_cia) from revision_efectivos where estado = false) group by num_cia,idoperadora,catalogo_operadoras.nombre order by idoperadora";
	$usuarios=ejecutar_script($sql,$dsn);
...
$sql="select * from modificacion_pastel where num_cia in (select num_cia from catalogo_companias where idoperadora=".$_GET['capturistas']." and num_cia <= 300 or num_cia=999 order by num_cia) and estado = false order by num_cia";
$facturas=ejecutar_script($sql,$dsn);
$sql="select * from revision_efectivos where num_cia in (select num_cia from catalogo_companias where idoperadora=".$_GET['capturistas']." and num_cia <= 300 order by num_cia) and estado = false order by num_cia";
$efectivos=ejecutar_script($sql,$dsn);


if(!$facturas and !$efectivos)
{
	header("location: ./pan_rev_sol_v2.php?codigo_error=1");
	die();
}
...
$tpl->assign("cont",0);
$tpl->assign("cont_efe",0);


if($facturas){
	$tpl->assign("cont",count($facturas));

	$tpl->newBlock("pastel_reg");
