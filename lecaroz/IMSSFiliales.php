...
if (isset($_REQUEST['accion']))
{
	switch ($_REQUEST['accion'])
	{
		...
		case 'inicio':
			$tpl = new TemplatePower('plantillas/ban/IMSSFilialesInicio.tpl');
			$tpl->prepare();
		...
		case 'consultar':
			...
			$condiciones[] = "f.num_cia_primaria IN (SELECT num_cia_primaria FROM catalogo_filiales_imss WHERE num_cia_primaria = num_cia)";
			...
			if (isset($_REQUEST['filiales']) && trim($_REQUEST['filiales']) != '')
			{
				...
				if (count($filiales) > 0)
				{
					$condiciones[] = 'f.num_cia_primaria IN (SELECT num_cia_primaria FROM catalogo_filiales_imss WHERE num_cia IN (' . implode(', ', $filiales) . '))';
				}
			}
			...
			$tpl = new TemplatePower('plantillas/ban/IMSSFilialesConsulta.tpl');
			$tpl->prepare();
		...
		case 'do_alta_matriz':
			$db->query("INSERT INTO catalogo_filiales_imss (num_cia_primaria, num_cia) VALUES ({$_REQUEST['num_cia']}, {$_REQUEST['num_cia']})");

			break;

		case 'do_alta_filial':
			$db->query("INSERT INTO catalogo_filiales_imss (num_cia_primaria, num_cia) VALUES ({$_REQUEST['matriz']}, {$_REQUEST['filial']})");

			break;

		case 'do_baja_matriz':
			$db->query("DELETE FROM catalogo_filiales_imss WHERE num_cia_primaria = {$_REQUEST['matriz']}");

			break;

		case 'do_baja_filial':
			$db->query("DELETE FROM catalogo_filiales_imss WHERE id = {$_REQUEST['id_filial']}");
...
$tpl = new TemplatePower('plantillas/ban/IMSSFiliales.tpl');
$tpl->prepare();
