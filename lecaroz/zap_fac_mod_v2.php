...
$tpl->assignInclude("body","./plantillas/zap/zap_fac_mod_v2.tpl");
$tpl->prepare();

if (isset($_POST['id'])) {
	// [01-Oct-2014] Obtener datos anteriores de la factura
	$sql = "
		SELECT
			*
		FROM
	...
	$sql .= " desc4 = $data[desc4], cod_desc1 = $data[cod_desc1], cod_desc2 = $data[cod_desc2], cod_desc3 = $data[cod_desc3],";
	$sql .= " cod_desc4 = $data[cod_desc4], faltantes = $data[faltantes], iva = $data[iva], pisr = $data[pisr], isr = $data[isr], pivaret = $data[pivaret], ivaret = $data[ivaret],";
	$sql .= " fletes = $data[fletes], otros = $data[otros], con_otros = '$data[con_otros]', total = $data[total], clave = $data[clave], iduser_mod = $_SESSION[iduser], tsmod = now() WHERE id = $data[id];\n";
	...
	if ($old[0]['num_proveedor'] != $_POST['num_pro'] || $old[0]['num_fact'] != $_POST['num_fact'])
	{
		$sql .= "UPDATE devoluciones_zap SET num_proveedor = {$_POST['num_pro']}, num_fact = '{$_POST['num_fact']}' WHERE num_proveedor = {$old[0]['num_proveedor']} AND num_fact = '{$old[0]['num_fact']}';\n";
	}
	
	$db->query($sql);
	
	if ($_POST['action'] == 1)
		header('location: ./zap_fac_mod_v2.php');
	else {
		$tpl->newBlock('close');
		$tpl->printToScreen();
	}
...
if (isset($_GET['num_pro']) || isset($_GET['id'])) {
	...
	$sql .= isset($_GET['num_pro']) ? " num_proveedor = $_GET[num_pro] AND num_fact = '$_GET[num_fact]'/* AND fecha >= CURRENT_DATE - interval '6 months'*/ ORDER BY id DESC LIMIT 1" : " id = $_GET[id]";
	$result = $db->query($sql);
	
	if (!$result) {
		header('location: ./zap_fac_mod_v2.php?codigo_error=1');
		die;
	}
	else if ($result[0]['por_aut'] == 't' && !in_array($_SESSION['iduser'], $admin_users)) {
		header('location: ./zap_fac_mod_v2.php?codigo_error=2');
		die;
	}
	else if ($result[0]['folio'] > 0 && !in_array($_SESSION['iduser'], $admin_users)) {
		header('location: ./zap_fac_mod_v2.php?codigo_error=3');
		die;
	}
	...
	$tpl->newBlock('mod');
	$tpl->assign('close', isset($_GET['id']) ? 'self.close()' : "document.location='./zap_fac_mod_v2.php'");
	...
	foreach ($reg as $k => $v)
		...
		else if (in_array($k, array('id', 'num_fact', 'fecha', 'fecha_rec', 'fecha_inv', 'concepto', 'con_dif_precio', 'con_otros')))
			$tpl->assign($k, $v);
		else if (in_array($k, array('entrada', 'clave', 'cod_desc1', 'cod_desc2', 'cod_desc3', 'cod_desc4')))
			$tpl->assign($k, $v != 0 ? $v : '');
		else if (in_array($k, array('importe', 'pdesc1', 'pdesc2', 'pdesc3', 'pdesc4', 'desc1', 'desc2', 'desc3', 'desc4', 'dif_precio', 'faltantes', 'iva', 'pisr', 'isr', 'ivaret', 'fletes', 'otros', 'total')))
			$tpl->assign($k, $v != 0 ? number_format($v, 2, '.', ',') : '');
		else if (in_array($k, array('pivaret')))
			$tpl->assign($k, $v != 0 ? number_format($v, 7, '.', ',') : '');
		else
			continue;
