...
if (isset($_REQUEST['accion'])) {
	if ($_REQUEST['accion']=="buscar") {
		...
		if (!empty($_REQUEST["date_range1"])) {
			$sql.=" AND date_ref BETWEEN  '".$_REQUEST["date_range1"]."' AND '".$_REQUEST["date_range2"]."'  ";
		}
	...
	foreach ($result as $key) {
		...
		$cad.='
			<tr id="row4" class="'.$line.'">
				...
				<td align="center" valign="top" nowrap="nowrap"><b>'.$key["price"].'</b></td>
				<td align="center" valign="top" nowrap="nowrap">'.( (!empty($key["date_ref"])) ? date('d/m/Y',strtotime($key["date_ref"])) : "" ).'</td>
				<td align="center" valign="top" nowrap="nowrap">'.$key["description"].'</td>
				<td align="center" valign="top" nowrap="nowrap">'.$key["observations"].'</td>
