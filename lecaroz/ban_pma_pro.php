		...
		if ($count > 0) {
			...
			for ($i=0; $i<count($factura); $i++) {
				...
				if ($factura[$i]['num_cia'] != $num_cia || $factura[$i]['num_proveedor'] != $num_proveedor || $num_fac == $fac_x_cheque) {
					...
					if ($num_cia != NULL && $num_proveedor != NULL && $num_fac > 0 && $num_fac <= $fac_x_cheque) {
						...
						if ($importe_cheque >= $monto_min) {
							...
							$nombre_gasto = ejecutar_script("SELECT descripcion FROM catalogo_gastos WHERE codgastos = $codgastos", $dsn);
							$cheque['concepto'.$che_count]          = $nombre_gasto[0]['descripcion'];	// El concepto de cheque son todas facturas que son pagadas con el mismo
							$cheque['facturas'.$che_count]          = $facturas;
							$cheque['fecha'.$che_count]             = $_SESSION['fac']['fecha_cheque'];
							...
							$cheque['proceso'.$che_count]           = "FALSE";
							//$cheque['cuenta'.$che_count]            = $_SESSION['fac']['cuenta'];
							
							// Datos para la tabla de 'estado_cuenta'
							$cuenta['num_cia'.$che_count] = $num_cia;
							$cuenta['fecha'.$che_count] = $_SESSION['fac']['fecha_cheque'];
							...
							//$cuenta['cuenta'.$che_count]            = $_SESSION['fac']['cuenta'];
							...
							$folio['utilizado'.$che_count] = "TRUE";
							$folio['fecha'.$che_count] = $_SESSION['fac']['fecha_cheque'];
							//$folio['cuenta'.$che_count]            = $_SESSION['fac']['cuenta'];
							
							$gasto['codgastos'.$che_count] = $codgastos;
							$gasto['num_cia'.$che_count]   = $num_cia;
							$gasto['fecha'.$che_count]     = $_SESSION['fac']['fecha_cheque'];
							$gasto['importe'.$che_count]   = number_format($importe_cheque,2,".","");
							$gasto['concepto'.$che_count]  = "PAGO PROVEEDOR: $num_proveedor FAC.: $facturas";
							$gasto['captura'.$che_count]   = "TRUE";
							$gasto['factura'.$che_count]   = "";
							$gasto['folio'.$che_count]     = $folio_cheque;
							...
							ejecutar_script("UPDATE saldos SET saldo_libros = saldo_libros - $importe_cheque WHERE num_cia = $num_cia AND cuenta = {$_SESSION['fac']['cuenta']}",$dsn);
				...
				if ($num_fac < $fac_x_cheque) {
					...
					$fac_pag['fecha_cheque'.$fac_count]  = $_SESSION['fac']['fecha_cheque'];
					$fac_pag['folio_cheque'.$fac_count]  = $folio_cheque;
					$fac_pag['proceso'.$fac_count]       = "FALSE";	// Proceso manual
					$fac_pag['imp'.$fac_count]           = "TRUE";
					
					/*$gasto['codgastos'.$fac_count] = $factura[$i]['codgastos'];
					$gasto['num_cia'.$fac_count]   = $num_cia;
					$gasto['fecha'.$fac_count]     = $_SESSION['fac']['fecha_cheque'];
					$gasto['importe'.$fac_count]   = $factura[$i]['importe'];
			...
			if ($che_count > 0) {
				...
				$db->xinsertar();
				unset($db);
				// Almacenar datos en la tabla de 'movimiento_gastos'
				$db = new DBclass($dsn,"movimiento_gastos",$gasto);
				$db->xinsertar();
				unset($db);
			}
...
// Inicializar ciclo de pantallas (para pantallas por proveedor)
if (isset($_GET['tipo']) && !isset($_SESSION['fac']) && $_GET['fac'] == "proveedor") {
	$_SESSION['fac']['tipo'] = $_GET['tipo'];
	$_SESSION['fac']['fecha_corte'] = $_GET['fecha_corte'];
	$_SESSION['fac']['fecha_cheque'] = $_GET['fecha_cheque'];
	$_SESSION['fac']['cuenta'] = $_GET['cuenta'];
	...
	$sql = "SELECT DISTINCT ON (num_proveedor) num_proveedor,nombre AS nombre_proveedor FROM pasivo_proveedores LEFT JOIN catalogo_proveedores USING(num_proveedor) WHERE";
	...
	if (!$result) {
		header("location: ./ban_pma_pro.php?cancelar=1&codigo_error=1");
		die;
	}
...
else if (isset($_GET['tipo']) && !isset($_SESSION['fac']) && $_GET['fac'] == "antiguedad") {
	$_SESSION['fac']['fecha_corte'] = $_GET['fecha_corte'];
	$_SESSION['fac']['fecha_cheque'] = $_GET['fecha_cheque'];
	$_SESSION['fac']['cuenta'] = $_GET['cuenta'];
	
	//$clabe_cuenta = $_SESSION['fac']['cuenta'] == 1 ? "clabe_cuenta" : "clabe_cuenta2";
	
	// Construir script sql
	$sql = "SELECT id,num_cia,pasivo_proveedores.num_proveedor AS num_proveedor,fecha_mov,num_fact,descripcion AS concepto,total AS importe FROM pasivo_proveedores LEFT JOIN catalogo_companias USING (num_cia) WHERE";
	if ($_GET['tipo'] == "cia")
		$sql .= " num_cia = $_GET[num_cia] AND";
	$sql .= " fecha_pago <= '$_GET[fecha_corte]' AND clabe_cuenta IS NOT NULL ORDER BY num_cia,num_proveedor,fecha_pago ASC";
	$result = ejecutar_script($sql,$dsn);
	...
	for ($i=0; $i<count($result); $i++) {
		if ($num_cia != $result[$i]['num_cia'] || $num_proveedor != $result[$i]['num_proveedor']) {
			...
			$tpl->newBlock("bloque_proveedor");
			$tpl->assign("num_proveedor",$num_proveedor);
			$nombre_proveedor = ejecutar_script("SELECT nombre FROM catalogo_proveedores WHERE num_proveedor = $num_proveedor",$dsn);
			$tpl->assign("nombre_proveedor",$nombre_proveedor[0]['nombre']);
		...
		$tpl->assign("bloque_proveedor.total",number_format($total,2,".",","));
	}
	$tpl->printToScreen();
	die;
}
...
// *************************************************************************************************************************************************
	// Construir script sql
	$sql  = "SELECT id,num_cia,fecha_mov,num_fact,descripcion AS concepto,total AS importe FROM pasivo_proveedores LEFT JOIN catalogo_companias USING (num_cia)";
	$sql .= " WHERE pasivo_proveedores.num_proveedor = ".$_SESSION['fac'][$_SESSION['fac']['next']];
	if ($_SESSION['fac']['tipo'] == "cia")
		$sql .= " AND num_cia = ".$_SESSION['fac']['num_cia'];
	/******************** PROVISIONAL, NO TOMAR EN CUENTA LAS FACTURAS DE PANADERIAS **************************/
	/*else
		$sql .= "AND num_cia > 100";*/
	$sql .= " AND clabe_cuenta IS NOT NULL AND fecha_pago <= '".$_SESSION['fac']['fecha_corte']."' ORDER BY num_cia,fecha_pago ASC";
	// Obtener facturas
	$fac = ejecutar_script($sql,$dsn);
	//echo $sql;print_r($_SESSION['fac']);
	if ($fac) {
