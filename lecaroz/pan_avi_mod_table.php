...
function buscar($array, $codmp, $cod_turno, $tipo_mov) {
	...
	for ($i=0; $i<$numfilas; $i++)
		if ($codmp == $array[$i]['codmp'] && $cod_turno == $array[$i]['cod_turno'] && $tipo_mov == $array[$i]['tipo_mov'])
...
for ($i=0; $i<count($avio); $i++) {
	...
	if ($avio[$i]['codmp'] != $mp) {
		...
		$entrada = buscar($mov,$mp,"","f");
		$entrada = $mp == 1 ? $entrada / 44 : $entrada;
		$tpl->assign("entrada",$entrada != 0 ? number_format($entrada,2,".","") : "");
		
		$tpl->assign("next",($fila<$numfilas)?$i:0);
		$tpl->assign("back",($fila>0)?$i-1:count($avio)-1);
		
		$tpl->assign("bottom",($fila<$numfilas-1)?$fila+1:0);
		$tpl->assign("top",($fila>0)?$fila-1:$numfilas-1);
		
		// Buscar todos los movimiento de entrada despues de la fecha dada
		$sql = "SELECT sum(cantidad) FROM mov_inv_virtual WHERE num_cia = $_GET[num_cia] AND codmp = $mp AND tipo_mov = 'FALSE' AND fecha >= '$fecha'";
		$mov_ent = $db->query($sql);
		// Buscar todos los movimiento de salida despues de la fecha dada
		$sql = "SELECT sum(cantidad) FROM mov_inv_virtual WHERE num_cia = $_GET[num_cia] AND codmp = $mp AND tipo_mov = 'TRUE' AND fecha >= '$fecha'";
		$mov_sal = $db->query($sql);
		
		$existencia = $avio[$i]['existencia'] - $mov_ent[0]['sum'] + $mov_sal[0]['sum'];
		
		if ($avio[$i]['codmp'] == 1)
			$existencia = $existencia / 44;
		
		$existencia_final = $existencia + $entrada;
		
		$tpl->assign("existencia",round($existencia,2));
	...
	switch ($avio[$i]['cod_turno']) {
		case 1:
			...
			$tpl->assign("consumo",$consumo != 0 ? number_format($mp == 1 ? $consumo / 44 : $consumo,2,".","") : "");
			
			$existencia_final -= $mp == 1 ? $consumo / 44 : $consumo;
	...
	$tpl->assign("mp.existencia_final",$existencia_final != 0 ? number_format($existencia_final,2,".","") : "&nbsp;");
