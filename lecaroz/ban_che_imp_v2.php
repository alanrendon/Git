...
//if ($_SESSION['iduser'] > 27) die(header('location: offline.htm'));

$users = array(28, 29, 30, 31, 32);

define ("strIni",    "&%STHPASSWORD$");				// Cadena que inicia el modo MICR de la impresora
define ("strImpIni", "&%STP12500$&%1B$(12500X$"/*ESC . "(12500X$"*/);	// Cadena de inicio de impresión de importe con protección especial
...
define ("AJUSTE", 1.5/*-4.5*/- 3.5);
define ("AJUSTE_H", 2.0);

function chequePCL($datos, $cuenta, $poliza = FALSE) {
	$pcl = "";
	
	// [14-Abril-2008] Ajuste para la nueva impresora de la oficina sin que afecte a la de zapaterias :S
	$ajuste_mollendo = $datos['num_cia'] <= 800 ? 1.50 - 4.00 : 0.00;
	...
	if ($fac[0] != "") {
		...
		for ($i = 0; $i < count($fac); $i++) {
			...
			if ($num_facs >= $num_facs_x_rows) {
				$pcl .= MoveCursorH(0.00);
				$pcl .= MoveCursorV(4.00 - AJUSTE, TRUE);
				$num_facs = 0;
			}
		}
		$pcl .= MoveCursorH(0.00);
	...
	if ($datos['num_cia'] >= 900) {
		...
		if (count($fac) > 0) {
			//$sql = "SELECT num_fact, importe, faltantes, desc1, desc2, desc3, desc4, iva, ivaret, isr, total FROM facturas_zap WHERE num_proveedor = $datos[num_proveedor] AND num_fact IN (";
			// [29-Jun-2007] Modificación de script para que incluya las devoluciones, fletes y otros de cada factura
			$sql = "SELECT num_fact, fz.importe, faltantes, sum(dz.importe) AS dev, pdesc1, pdesc2, pdesc3, pdesc4, desc1, desc2, desc3, desc4, iva, ivaret, isr, fletes, otros, total, sucursal FROM";
			...
			$sql .= " GROUP BY num_fact, fz.importe, faltantes, pdesc1, pdesc2, pdesc3, pdesc4, desc1, desc2, desc3, desc4, iva, ivaret, isr, fletes, otros, total, sucursal ORDER BY num_fact, sucursal DESC";
			...
			if ($des) {
				...
				foreach ($des as $i => $d) {
					if ($d['dev'] > 0 && $d['sucursal'] != 't') {
						...
						if ($d['sucursal'] == 't') $imp_suc += $d['total'];
					...
					$pcl .= MoveCursorH(10.00);
					$pcl .= /*$d['importe']*/$importe != 0 ? number_format(/*$d['importe']*/$importe, 2, '.', ',') : '';
					$pcl .= MoveCursorH(20.00);
					$pcl .= $d['faltantes'] != 0 ? number_format($d['faltantes'], 2, '.', ',') : '';
					$pcl .= MoveCursorH(30.00);
					$pcl .= $d['dev'] != 0 ? number_format($d['dev'], 2, '.', ',') : '';
					$pcl .= MoveCursorH(40.00);
					$pcl .= /*$d['desc1'] + $d['desc2'] + $d['desc3'] + $d['desc4']*/$desc1 + $desc2 + $desc3 + $desc4 != 0 ? number_format(/*$d['desc1'] + $d['desc2'] + $d['desc3'] + $d['desc4']*/$desc1 + $desc2 + $desc3 + $desc4, 2, '.', ',') : '';
					$pcl .= MoveCursorH(50.00);
					$pcl .= /*$d['iva']*/$iva != 0 ? number_format(/*$d['iva']*/$iva, 2, '.', ',') : '';
					$pcl .= MoveCursorH(60.00);
					...
					$pcl .= MoveCursorH(100.00);
					$pcl .= /*$d['total']*/$total_fac != 0 ? number_format(/*$d['total']*/$total_fac, 2, '.', ',') : '';
				}
			}
		}
		
		// Devoluciones hechas al proveedor
		/*$sql = "SELECT sum(importe) AS importe FROM devoluciones_zap WHERE num_cia = $datos[num_cia] AND folio_cheque = $datos[folio] AND cuenta = $cuenta";
		$dev = $GLOBALS['db']->query($sql);
		if ($dev[0]['importe'] > 0) {
			$pcl .= MoveCursorH(100.00);
			$pcl .= MoveCursorV(135.00);
			$pcl .= SetFontStrokeWeight(BOLD);
			...
			$pcl .= "DEVOLUCIONES";
			$pcl .= MoveCursorH(100.00);
			$pcl .= MoveCursorV(138.50);
			$pcl .= SetFontStrokeWeight(MEDIUM);
			$pcl .= number_format($dev[0]['importe'], 2, '.', ',');
		}*/
	}
	...
	// Nombre de la compañía
	$pcl .= MoveCursorH(2.00 - AJUSTE_H);
	$pcl .= MoveCursorV(187.00 - AJUSTE + $ajuste_mollendo);
	$pcl .= SetFontStrokeWeight(BOLD);
	$pcl .= SetFontPointSize(10.00);
	$pcl .= strtoupper($datos['nombre_cia']);
	// Leyenda G.OAM
	$pcl .= MoveCursorH(2.00 - AJUSTE_H);
	$pcl .= MoveCursorV(191.00 - AJUSTE + $ajuste_mollendo);
	$pcl .= SetFontPointSize(8.00);
	$pcl .= "G.OAM";
	// RFC
	$pcl .= MoveCursorH(22.00 - AJUSTE_H);
	...
	if ($datos['para_abono'] == "t") {
		...
		$pcl .= MoveCursorV(197.00 - AJUSTE + $ajuste_mollendo);
		$pcl .= SetFontStrokeWeight(MEDIUM);
		$pcl .= SetFontPointSize(10.00);
		$pcl .= "PARA ABONO EN CUENTA BANCARIA DE:";
	}
	// Leyenda LUGAR DE EXPEDICION
	$pcl .= MoveCursorH(84.00 - AJUSTE_H);	// [30/11/2005] Se modifico de 74mm a 84mm
	$pcl .= MoveCursorV(203.00 - AJUSTE + $ajuste_mollendo);
	$pcl .= $datos['para_abono'] == "f" ? SetFontStrokeWeight(MEDIUM): "";
	$pcl .= SetFontPointSize(6.00);
	...
	// Beneficiario
	$pcl .= MoveCursorH(2.00 - AJUSTE_H);
	$pcl .= MoveCursorV(214.00 - AJUSTE + $ajuste_mollendo);
	$pcl .= $datos['a_nombre'];
	// Importe formateado
	$pcl .= MoveCursorH(100.00);
	$pcl .= (!$poliza ? ESC . ($_SESSION['iduser'] >= 28 ? strImpIni : strImpIniE330) : "*") . number_format($datos['importe'], 2, ".", ",") . (!$poliza ?  ($_SESSION['iduser'] >= 28 ? strImpFin : strImpFinE330) : "*");
	// Importe escrito
	$pcl .= MoveCursorH(2.00 - AJUSTE_H);
	$pcl .= MoveCursorV(218.00 - AJUSTE + $ajuste_mollendo);
	$pcl .= DEFAULT_FONT;
	$pcl .= SetFontPointSize(8.00);
	$pcl .= "*" . num2string($datos['importe']) . "*";
	// Leyendas de Banorte
	$pcl .= MoveCursorH(2.00 - AJUSTE_H);
	$pcl .= MoveCursorV(225.00 - AJUSTE + $ajuste_mollendo);	// [30/11/2005] Modificado de 222mm a 225mm
	$pcl .= SetFontStrokeWeight(BOLD);
	$pcl .= SetFontPointSize(10.00);
	$pcl .= $cuenta == 1 ? "BANCO MERCANTIL DEL NORTE, S.A." : "";
	$pcl .= MoveCursorH(2.00 - AJUSTE_H);
	$pcl .= MoveCursorV(227.00 - AJUSTE + $ajuste_mollendo);	// [30/11/2005] Modificado de 225mm a 227mm
	$pcl .= SetFontStrokeWeight(MEDIUM);
	$pcl .= SetFontPointSize(5.00);	// [30/11/2005] Modificado de 6Pts a 5Pts
	$pcl .= $cuenta == 1 ? "INSTITUCION DE BANCA MULTIPLE." : "";
	$pcl .= MoveCursorH(2.00 - AJUSTE_H);
	$pcl .= MoveCursorV(229.00 - AJUSTE + $ajuste_mollendo);	// [30/11/2005] Modificado de 227mm a 229mm
	$pcl .= $cuenta == 1 ? "GRUPO FINANCIERO BANORTE." : "";
	$pcl .= MoveCursorH(2.00 - AJUSTE_H);
	$pcl .= MoveCursorV(232.00 - AJUSTE + $ajuste_mollendo);
	...
	$pcl .= MoveCursorH(2.00 - AJUSTE_H);
	$pcl .= MoveCursorV(235.00 - AJUSTE + $ajuste_mollendo);
	$pcl .= SetFontStrokeWeight(MEDIUM);
	$pcl .= "MEXICO, D.F.";
	// Banda MICR
	$pcl .= MoveCursorH(2.00 - AJUSTE_H);
	$pcl .= MoveCursorV(244.00 - AJUSTE + $ajuste_mollendo);
	...
	// Facturas pagadas por el cheque
	$pcl .= MoveCursorH(0.00);
	$pcl .= MoveCursorV(260.00 - AJUSTE + $ajuste_mollendo);
	$pcl .= SetFontStrokeWeight(BOLD);
	...
	if ($fac[0] != "") {
		...
		for ($i = 0; $i < count($fac); $i++) {
			...
			if ($num_facs >= $num_facs_x_rows) {
				$pcl .= MoveCursorH(0.00);
				$pcl .= MoveCursorV(4.00 - AJUSTE + $ajuste_mollendo, TRUE);
				$num_facs = 0;
			}
		}
		$pcl .= MoveCursorH(0.00);
...
$tpl->assignInclude("body", "./plantillas/ban/ban_che_imp_v2.tpl");
$tpl->prepare();
...
if (isset($_GET['pol'])) {
	...
	$sql .= " LEFT JOIN catalogo_proveedores ON (catalogo_proveedores.num_proveedor = cheques.num_proveedor) WHERE num_cia BETWEEN " . (in_array($_SESSION['iduser'], $users) ? '900 AND 998' : '1 AND 800') . " AND imp = 'FALSE'/* AND cheques.cuenta = 2*/ AND poliza = 'TRUE'";
	...
	if (in_array($_SESSION['iduser'], $users))
		...
		shell_exec("lp -d S1855 pcl/cheques.pcl");//shell_exec("lp -d T622 pcl/cheques.pcl");
...
if (isset($_POST['imp'])) {
	...
	//shell_exec("lp -d T622 pcl/cheques.pcl");
	// [12/09/2006] Cambio provisional
	if (in_array($_SESSION['iduser'], $users))//shell_exec("lp -d " . ($_POST['ultimo_folio'] > 0 ? "T622" : "S1855") . " pcl/cheques_zap.pcl");
		shell_exec("lp -d elite pcl/cheques_zap.pcl");
	else
...
if (isset($_GET['ultimo_folio'])) {
	...
	$sql = "SELECT num_cheque FROM cheques WHERE num_cheque IS NOT NULL AND fecha >= '31/10/2005' AND num_cia BETWEEN " . (in_array($_SESSION['iduser'], $users) ? '900 AND 998' : '1 AND 800') . " AND cuenta = $_GET[cuenta] ORDER BY num_cheque DESC LIMIT 1";
	...
	$sql .= " FROM cheques LEFT JOIN catalogo_companias USING (num_cia) WHERE num_cia BETWEEN " . (in_array($_SESSION['iduser'], $users) ? '900 AND 998' : '1 AND 800') . " AND imp = 'FALSE' AND cuenta = $_GET[cuenta] " . (!isset($_GET['poliza']) ? "AND poliza != 'TRUE'" : '') . " AND fecha_cancelacion IS NULL ORDER BY a_nombre, num_cia, folio ASC";
	$result = $db->query($sql);
...
$result = $db->query("SELECT id FROM cheques WHERE num_cia BETWEEN " . (in_array($_SESSION['iduser'], $users) ? '900 AND 998' : '1 AND 800') . " AND imp = 'FALSE'/* AND cuenta = 2*/ AND poliza = 'TRUE' AND fecha_cancelacion IS NULL LIMIT 1");
...
$result = $db->query("SELECT id FROM cheques WHERE num_cia BETWEEN " . (in_array($_SESSION['iduser'], $users) ? '900 AND 998' : '1 AND 800') . " AND imp = 'FALSE' AND codgastos IN (140, 141) AND fecha_cancelacion IS NULL LIMIT 1");
