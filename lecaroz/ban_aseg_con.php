...
$tpl->assignInclude("body","./plantillas/ban/ban_aseg_con.tpl");
$tpl->prepare();
...
if($reg)
{
	for($i=0;$i<count($reg);$i++)
	{
		$tpl->newBlock("rows");
		$tpl->assign("i",$i);
		$tpl->assign('idaseguradora',$reg[$i]['idaseguradora']);
		$cia = obtener_registro("catalogo_aseguradoras",array("idaseguradora"),array($reg[$i]['idaseguradora']),"","",$dsn);
		$tpl->assign('nombre_aseguradora',$cia[0]['nombre_aseguradora']);
	}
}
else
{
	header("location: ./ban_aseg_con.php?codigo_error=1");
	die;
}
