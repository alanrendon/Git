...
if (isset($_POST['num_cia'])) {
	...
	for ($i=0; $i<$numfilas; $i++)
		if ($_POST['num_cia'][$i] > 0 && $_POST['importe'][$i] > 0)
			if ($id = $db->query("SELECT id FROM importes_talleres WHERE num_cia = {$_POST['num_cia'][$i]} AND mes = $_POST[mes] AND anio = $_POST[anio]"))
				$sql .= "UPDATE importes_talleres SET importe = {$_POST['importe'][$i]} WHERE id = {$id[0]['id']};\n";
			else
				$sql .= "INSERT INTO importes_talleres (num_cia,importe,mes,anio) VALUES ({$_POST['num_cia'][$i]},{$_POST['importe'][$i]},$_POST[mes],$_POST[anio]);\n";
		else if ($_POST['num_cia'][$i] > 0 && ($_POST['importe'][$i] == 0 || $_POST['importe'][$i] == ""))
			$sql .= "DELETE FROM importes_talleres WHERE num_cia = {$_POST['num_cia'][$i]} AND mes = $_POST[mes] AND anio = $_POST[anio];\n";
	
	$db->query($sql);
	$db->desconectar();
	
	header("location: ./ban_imp_tal.php");
	die;
}
...
$tpl->assignInclude("body","./plantillas/ban/ban_imp_tal.tpl");
$tpl->prepare();
