...
{
	echo "GenerarDatosDepositosV3.php Ver 2.15.10.8";
	echo "\nCopyright (c) 2015, Lecaroz";
	echo "\n\nModo de empleo: php GenerarDatosDepositosV3.php [OPCIONES]";
...
$condiciones[] = 'sec.cod_mov IN (1, 2, 16, 20, 44, 99)';
		...
		if ($facturas)
		{
			...
			foreach ($facturas as $f)
			{
					...
					fe_id = {$f['id']}"))
				{
					$datos['facturas'][] = array(
						'id'		=> intval($nc[0]['id']),
						'num_cia'	=> intval($nc[0]['num_cia']),
						'cuenta'	=> $cuenta[0]['num_cuenta'],
						'fecha'		=> $nc[0]['fecha'],
						'folio'		=> $nc[0]['folio'],
						'importe'	=> floatval($nc[0]['importe'])
					);
			...
			else
			{
				...
				foreach ($datos['facturas'] as $row)
				{
					$xml_carga = generar_xml_datos($row, $guardar_xml);
					...
					if ( ! $no_cargar)
					{
						...
						if ($response === FALSE)
						{
							...
							if ( ! $nderr && $ob_err >= $max_ob_err)
							{
								...
								if ($records)
								{
									...
									WHERE
										...
										AND cod_mov IN (1, 2, 16, 44, 99)");

									$db->query("UPDATE facturas_electronicas SET ts_carga_dep = NOW() WHERE id IN (" . implode(', ', $facs) . ")");
								}
							...
							echo "\n[" . date('Y-m-d H:i:s.') . substr(microtime(), 2, 6) . "](EE) Se cancela carga de depósitos del día {$dd['fecha']} para la compañía {$dd['num_cia']}";
...
function generar_xml_datos($datos, $guardar_xml = FALSE)
{
	...
	$xml_documentNo = $xml->createElement('documentNo', $datos['id']);
	$xml_payment->appendChild($xml_documentNo);

	$xml_financialAccountValue = $xml->createElement('financialAccountValue', "{$datos['cuenta']}");
	$xml_payment->appendChild($xml_financialAccountValue);

	$xml_paymentAmount = $xml->createElement('paymentAmount', $datos['importe']);
	$xml_payment->appendChild($xml_paymentAmount);

	$xml_paymentDate = $xml->createElement('paymentDate', dmy_to_ymd($datos['fecha']));
	$xml_payment->appendChild($xml_paymentDate);

	$xml_organization = $xml->createElement('organization', $datos['num_cia']);
	...
	$xml_invoice_amount = $xml->createElement('amount', $datos['importe']);
	$xml_invoice->appendChild($xml_invoice_amount);

	$xml_invoice_documentNo = $xml->createElement('documentNo', $datos['folio']);
	$xml_invoice->appendChild($xml_invoice_documentNo);

	$xml_invoice_organization = $xml->createElement('organization', $datos['num_cia']);
