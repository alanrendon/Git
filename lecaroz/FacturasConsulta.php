...
if (isset($_REQUEST['accion'])) {
	switch ($_REQUEST['accion']) {
		...
		case 'inicio':
			$tpl = new TemplatePower('plantillas/fac/FacturasConsultaInicio.tpl');
			$tpl->prepare();
		...
		case 'consultar':
			...
			if ($query) {
				$tpl = new TemplatePower('plantillas/fac/FacturasConsultaResultado.tpl');
				$tpl->prepare();
				...
				foreach ($query as $row) {
					...
					$tpl->assign('tipo', $row['tipo'] == 3 ? 2 : 1);
					$tpl->assign('num_pro', $row['num_pro']);
					...
					$tpl->assign('cancelar_disabled', (in_array($_SESSION['iduser'], array(1, 4, 48)) && ($row['fecha_pago'] != '' || $row['tipo'] == 3)) || ( ! in_array($_SESSION['iduser'], array(1, 4, 48)) && ($row['fecha_pago'] != '' || $row['balance_generado'] == 't' || $row['tipo'] == 3)) ? '_gray' : '');
					$tpl->assign('icono_cancelar_class', (in_array($_SESSION['iduser'], array(1, 4, 48)) && ($row['fecha_pago'] != '' || $row['tipo'] == 3)) || ( ! in_array($_SESSION['iduser'], array(1, 4, 48)) && ($row['fecha_pago'] != '' || $row['balance_generado'] == 't' || $row['tipo'] == 3)) ? '' : ' class="icono"');
		...
		case 'cancelar_factura':
			...
			else if ($_REQUEST['tipo'] == 1)
			{
				...
				WHERE
					tipo_mov = FALSE
					AND descripcion NOT LIKE 'REMISION%'
		...
		case 'cargar_conta':
			...
			if ($_REQUEST['ids'] && count($_REQUEST['ids']) > 0)
			{
				$sql[] = "SELECT
					f.id,
					f.num_cia,
			...
			if ($_REQUEST['ids_ros'] && count($_REQUEST['ids_ros']) > 0)
			{
				$sql[] = "SELECT
					f.id,
					f.num_cia,
				...
				FROM
					...
					LEFT JOIN catalogo_companias cc ON (cc.num_cia = f.num_cia)
				WHERE
					f.id IN (" . implode(', ', $_REQUEST['ids_ros']) . ")";
			}

			$result = $db->query(implode(' UNION ', $sql) . ' ORDER BY tipo_fac, id');

			if ( ! $result)
			{
				echo "\nNo hay registros por cargar\n";
			...
			foreach ($result as $i => $df)
			{
				...
				if ($response === FALSE)
				{
					if ($df['tipo_fac'] == 2)
					{
						$db->query("UPDATE total_fac_ros SET ts_carga_conta_error = 'Webservice no disponible', ts_carga_conta = NULL WHERE id = {$df['id']}");
					}
					else
					{
					...
					if ( ! $nderr && $ob_err >= $max_ob_err)
					{
						...
						if ($ids_ros)
						{
							echo "\n[" . date('Y-m-d H:i:s.') . substr(microtime(), 2, 6) . "](PP) Actualizando marcas de tiempo de " . count($ids_ros) . " registro(s)";

							$db->query("UPDATE total_fac_ros SET ts_carga_conta = NOW(), ts_carga_conta_error = NULL WHERE id IN (" . implode(', ', $ids_ros) . ")");
						}
				...
				else
				{
					...
					if ( ! isset($ob_data->status))
					{
						if ($df['tipo_fac'] == 2)
						{
							$db->query("UPDATE total_fac_ros SET ts_carga_conta_error = 'Imposible decodificar respuesta de openbravo: {$response}', ts_carga_conta = NULL WHERE id = {$df['id']}");
						}
						else
						{
					...
					else if ($ob_data->status < 0)
					{
						...
						if ($df['tipo_fac'] == 2)
						{
							$db->query("UPDATE total_fac_ros SET ts_carga_conta_error = '{$msg}', ts_carga_conta = NULL WHERE id = {$df['id']}");
						}
						else
						{
...
$tpl = new TemplatePower('plantillas/fac/FacturasConsulta.tpl');
$tpl->prepare();
