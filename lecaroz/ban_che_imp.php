...
$descripcion_error[3] = "El ultimo folio capturado no es el consecutivo del ultimo cheque generado.";
...
$tpl->assignInclude("body","./plantillas/ban/ban_che_imp.tpl");
$tpl->prepare();
...
// -------------------------------- Tipo de listado -------------------------------------------------------
if (!isset($_GET['tipo'])) {
	// Verificar si la ultima ronda de cheques fue impresa correctamente
	$ok = ejecutar_script("SELECT * FROM status_cheques",$dsn);
	if ($ok[0]['ok'] == "f") {
...
// Verificar que el ultimo número de cheque introducido sea el consecutivo
/*$ultimo_cheque = ejecutar_script("SELECT num_cheque FROM cheques WHERE num_cheque IS NOT NULL ORDER BY num_cheque DESC LIMIT 1",$dsn);
if (isset($_GET['ultimo_folio']) && $_GET['ultimo_folio'] != $ultimo_cheque[0]['num_cheque'] + 1) {
	header("location: ./ban_che_imp.php?codigo_error=3");
	die;
}*/
...
// Por folio
if ($_GET['tipo'] == "folio")
	$sql .= " AND folio BETWEEN $_GET[folio1] AND $_GET[folio2] AND num_cia = $_GET[num_cia_folio]";
// Por compañía
...
// Por fecha
	...
	$sql .= " AND fecha BETWEEN '$_GET[fecha1]' AND '$_GET[fecha2]'";
// Organizar registros
...
$tpl->assign("ultimo_cheque", isset($_GET['ultimo_folio']) ? $_GET['ultimo_folio'] : "");
