...
$tpl->assignInclude("body","./plantillas/pan/pan_con_acu_v4.tpl");
$tpl->prepare();
...
for ($j = 0; $j < $nummp; $j++) {
	...
	// [13-Ago-2009] Arrastre de ultimo costo con existencia positiva
	$arrastre_costo = $costo_promedio;
	...
	if (round($consumo_total_mp, 2) > 0) {
		...
		if ($saldo_ant[$j]['controlada'] == 'TRUE') {
			...
			if ($saldo_ant[$j]['tipo'] == 1) {
				...
				foreach ($no_con as $key => $value) {
					$tpl->assign($key, $value != 0 ? number_format($value, 2, '.', ',') : '&nbsp;');
					$no_controlado[$key] += $value;
				}
				$tpl->assign("consumo", number_format(array_sum($no_con), 2, ".", ","));
			}
			// Material de empaque
			else if ($saldo_ant[$j]['tipo'] == 2) {
				$no_con = array();
