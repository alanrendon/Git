...
if (isset($_GET['accion']) && $_GET['accion'] == "cancel") {
	...
	header("location: ./ban_con_aut_san_no_bloqueo_v2.php");
	die;
}
...
$tpl->assignInclude("body", "./plantillas/ban/ban_con_aut_san_no_bloqueo_v2.tpl");
$tpl->prepare();
...
if (isset($_FILES['userfile'])) {
	...
	/*if ($_FILES['userfile']['tmp_name'] != "" && !stristr(mime_content_type($_FILES['userfile']['tmp_name']), "gzip")) {
		header("location: ./ban_con_aut_san_no_bloqueo_v2.php?codigo_error=1");
		die();
	}*/
	...
	if ($_FILES['userfile']['tmp_name'] != "" && $db->query("SELECT hash FROM mov_santander WHERE hash = '$hash' LIMIT 1")) {
		header("location: ./ban_con_aut_san_no_bloqueo_v2.php?codigo_error=3&archivo={$_FILES['userfile']['name']}");
		die;
	}
	...
	header("location: ./ban_con_aut_san_no_bloqueo_v2.php?accion=imp&hash=$hash");
	die;
}
...
if (isset($_GET['accion']) && $_GET['accion'] == "imp") {
	...
	if (!$result) {
		header("location: ./ban_con_aut_san_no_bloqueo_v2.php?accion=codsob&hash=$_GET[hash]");
		die;
	}
...
if (isset($_GET['accion']) && $_GET['accion'] == "codsob") {
	...
	if (!$result) {
		header("location: ./ban_con_aut_san_no_bloqueo_v2.php?accion=con&hash=$_GET[hash]");
		die;
	}
...
if (isset($_GET['accion']) && $_GET['accion'] == "con") {
	...
	// if ($tar)
	// 	foreach ($tar as $reg)
	// 		if ($reg['num_documento'] > 0 && $com = $db->query("SELECT * FROM mov_santander WHERE num_cia = $reg[num_cia] AND cod_banco IN (864, 898, 571, 2571) AND fecha_con IS NULL AND num_documento IN ($reg[num_documento], $reg[num_documento] + 1)")) {
	// 			// Conciliar tarjeta
	...
	// 			foreach ($com as $c) {
	// 				if (!in_array($c['id'], $id)) {
	// 					$cod_mov = $c['cod_banco'] == 864 ? 46 : 10;
	// 					$concepto = $c['cod_banco'] == 864 ? "COM. TARJETA DE CREDITO" : "IVA POR COMISIONES";
	// 					$sql .= "INSERT INTO estado_cuenta (num_cia, fecha, fecha_con, tipo_mov, importe, cod_mov, concepto, cuenta, iduser, timestamp, num_doc) ";
	...
	// [15-Sep-2016] Nueva forma para conciliar tarjetas de crédito, basado en la compañía, fecha y número de documento
	...
	header("location: ./ban_con_aut_san_no_bloqueo_v2.php?accion=saldos");
	die;
}
...
if (isset($_GET['accion']) && $_GET['accion'] == 'saldos') {
	...
	if (!$result)
		die(header('location: ./ban_con_aut_san_no_bloqueo_v2.php?accion=res'));
