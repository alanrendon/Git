...
$session->validar_sesion($_SESSION['authlevel']);
...
$sql = "SELECT permiso FROM permisos WHERE id_user = $_SESSION[iduser] AND authlevel = $_SESSION[authlevel] AND idscreen = $_GET[id_screen]";
$result = $db->query($sql);
...
if ($result->numRows() > 0 && $ver_pantalla->permiso == TRUE) {
	...
	$tpl->assignInclude("body","./plantillas/$menu->path/$screen->plantilla.tpl");
	$tpl->prepare();
