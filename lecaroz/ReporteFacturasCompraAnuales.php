...
if (isset($_REQUEST['accion']))
{
	switch ($_REQUEST['accion'])
	{
		...
		case 'reporte':
			...
			$condiciones3[] = "f.tscap::DATE BETWEEN '{$fecha1}' AND '{$fecha2}'";
			...
			FROM
				(
					...
					SELECT
						f.num_cia,
						cc.nombre_corto AS nombre_cia,
						EXTRACT(MONTH FROM f.tscap::DATE) AS mes,
						SUM(factura_zap_total(
							f.importe,
							f.faltantes,
							f.dif_precio,
							CASE
			...
			$tpl = new TemplatePower('plantillas/bal/ReporteFacturasCompraAnualesListado.tpl');
			$tpl->prepare();
...
$tpl = new TemplatePower('plantillas/bal/ReporteFacturasCompraAnualesInicio.tpl');
$tpl->prepare();
