...
{
	echo "GenerarDatosContabilidadEspecialV2.php Ver 2.15.9.8";
	echo "\nCopyright (c) 2015, Lecaroz";
	echo "\n\nModo de empleo: php GenerarDatosContabilidadEspecialV2.php [OPCIONES]";
...
FROM
	facturas f
WHERE
	f.fecha BETWEEN '2016-01-01' AND '2016-11-30'
	AND f.num_proveedor IN (2324, 1757)
	AND f.codgastos = 23";
...
LIMIT {$rows_per_query} OFFSET {$offset}"))
{
	...
	foreach ($datos_facturas as $i => $df)
	{
		...
		// {
			...
			$tax = $df['iva'] != 0 && $df['ieps'] != 0 ? ($df['codigo'] == 87 ? 'IMP-RON' : 'PIEPSIVA') : ($df['iva'] != 0 ? 'IVA' : ($df['ieps'] != 0 ? 'PIEPS' : 'IVA 0'));

			$datos['detalle'][] = array(
				'clave'			=> /*"G{$df['codigo']}"*/'MP' . ($df['num_pro'] == 2324 ? 1183 : 302),
				'descripcion'	=> /*$df['descripcion']*/'PAPAS',
				'cantidad'		=> 1,
