...
{
	echo "GenerarDatosContabilidadRosticeriasV2.php Ver 2.15.9.8";
	echo "\nCopyright (c) 2015, Lecaroz";
	echo "\n\nModo de empleo: php GenerarDatosContabilidadRosticeriasV2.php [OPCIONES]";
		...
		if ($response === FALSE)
		{
			...
			if ( ! $nderr && $ob_err >= $max_ob_err)
			{
				...
				if ($ids)
				{
					...
					$db->query("UPDATE total_fac_ros SET ts_carga_conta = NOW() WHERE id IN (" . implode(', ', $ids) . ")");
				}
...
function generar_xml_datos($datos, $guardar_xml = FALSE)
{
	...
	if ($guardar_xml)
	{
		$nombre_xml = "carga-conta-{$datos['cabecera']['num_cia']}-{$datos['cabecera']['clave_pro']}-{$datos['cabecera']['num_fact']}.xml";

		$xml->save("tmp/{$nombre_xml}");
