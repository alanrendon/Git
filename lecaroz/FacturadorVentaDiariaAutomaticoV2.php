...
{
	echo "FacturadorVentaDiariaAutomaticoV2.php";
	echo "\nCopyright (c) 2016, Lecaroz";
	echo "\n\nModo de empleo: php FacturadorVentaDiariaAutomaticoV2.php [OPCIONES]";
...
if (isset($options['iduser']) || isset($_REQUEST['iduser']))
{
	...
	echo "\n[" . date('Y-m-d H:i:s.') . substr(microtime(), 2, 6) . "](II) iduser={$params['iduser']}";
}
...
foreach ($result as $cia)
{
	...
	$datos[$cia['num_cia']] = array(
		...
		'desgloses'		=> array_fill(1, $dia_corte, NULL),
	...
	// Obtener depósitos detallados por día de la compañía
	$sql = "SELECT
		COALESCE(ec.num_cia_sec, ec.num_cia) AS num_cia,
		fecha,
		EXTRACT(DAY from fecha) AS dia,
	...
	$desgloses = $db->query($sql);

	foreach ($desgloses as $desglose)
	{
		$datos[$cia['num_cia']]['desgloses'][$desglose['dia']][$desglose['cod_mov']] = floatval($desglose['importe']);
	}
	...
	foreach ($datos[$cia['num_cia']]['efectivos'] as $dia => $efectivo)
	{
		...
		// Distribuir importe de clientes entre los diferentes depósitos del día
		else
		{
			$clientes = $datos[$cia['num_cia']]['clientes'][$dia];

			foreach ($datos[$cia['num_cia']]['desgloses'][$dia] as $codigo => $deposito)
			{
				if ($deposito - $clientes > 0)
				{
					$datos[$cia['num_cia']]['desgloses'][$dia][$codigo] -= $clientes;
				...
				else
				{
					...
					unset($datos[$cia['num_cia']]['desgloses'][$dia][$codigo]);
				}
			}
		}
	}
