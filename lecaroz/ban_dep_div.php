...
$tpl->assignInclude("body","./plantillas/ban/ban_dep_div.tpl");
$tpl->prepare();
...
if (isset($_GET['tabla'])) {
	...
	// Restar importe del saldo en libros
	ejecutar_script("UPDATE saldos SET saldo_libros = saldo_libros - ".$dep[0]['importe']." WHERE cuenta = {$dep[0]['cuenta']} AND num_cia = ".$dep[0]['num_cia'],$dsn);
	...
	for ($i=1; $i<=5; $i++) {
		if ($_POST['dep'.$i] > 0) {
			...
			if ($_POST['efe'] != "1") $_SESSION['check']['id'.count($_SESSION['check'])] = $id;
			$id++;
			$count++;
			
			// Actualizar saldo en libros
			ejecutar_script("UPDATE saldos SET saldo_libros = saldo_libros + ".$_POST['dep'.$i]." WHERE cuenta = {$dep[0]['cuenta']} AND num_cia = ".$dep[0]['num_cia'],$dsn);
		}
	}
...
if (empty($_GET['efe']))
	$cia = ejecutar_script("SELECT num_cia,nombre_corto FROM catalogo_companias WHERE num_cia = ".$_SESSION['con']['num_cia'.$_SESSION['con']['next']],$dsn);
else
	$cia = ejecutar_script("SELECT num_cia,nombre_corto FROM catalogo_companias WHERE num_cia IN (SELECT num_cia FROM estado_cuenta WHERE id = $_GET[id])", $dsn);
$dep = ejecutar_script("SELECT * FROM estado_cuenta WHERE id = $_GET[id]",$dsn);

$tpl->newBlock("dividir");
$tpl->assign("efe", isset($_GET['efe']) ? "1" : "");
$tpl->assign("id",$_GET['id']);
$tpl->assign("num_cia",isset($_GET['efe']) ? $cia[0]['num_cia'] : $_SESSION['con']['num_cia'.$_SESSION['con']['next']]);
