...
if (isset($_POST['mes'])) {
	...
	for ($i = 0; $i < count($_POST['num_cia']); $i++)
		if ($_POST['num_cia'][$i] > 0 && $_POST['dia'][$i] > 0 && get_val($_POST['importe'][$i]) > 0) {
			...
			$data['pag1'] = $_POST['num_fact1'][$i] > 0 ? ($_POST['sal1'][$i] == 0 ? 0 : round(get_val($_POST['imp1'][$i]) - get_val($_POST['pag1'][$i]) - get_val($_POST['sal1'][$i]), 2)) : '';
			if ($_POST['num_fact1'][$i] > 0 && $_POST['sal1'][$i] == 0) $sql .= "UPDATE facturas_zap SET folio = 0, cuenta = 0, tspago = now() WHERE clave = {$_POST['num'][$i]} AND num_fact = {$_POST['num_fact1'][$i]};\n";
			$data['num_fact2'] = $_POST['num_fact2'][$i];
			$data['pag2'] = $_POST['num_fact2'][$i] > 0 ? ($_POST['sal2'][$i] == 0 ? 0 : round(get_val($_POST['imp2'][$i]) - get_val($_POST['pag2'][$i]) - get_val($_POST['sal2'][$i]), 2)) : '';
			if ($_POST['num_fact2'][$i] > 0 && $_POST['sal2'][$i] == 0) $sql .= "UPDATE facturas_zap SET folio = 0, cuenta = 0, tspago = now() WHERE clave = {$_POST['num'][$i]} AND num_fact = {$_POST['num_fact2'][$i]};\n";
			$data['num_fact3'] = $_POST['num_fact3'][$i];
			$data['pag3'] = $_POST['num_fact3'][$i] > 0 ? ($_POST['sal3'][$i] == 0 ? 0 : round(get_val($_POST['imp3'][$i]) - get_val($_POST['pag3'][$i]) - get_val($_POST['sal3'][$i]), 2)) : '';
			if ($_POST['num_fact3'][$i] > 0 && $_POST['sal3'][$i] == 0) $sql .= "UPDATE facturas_zap SET folio = 0, cuenta = 0, tspago = now() WHERE clave = {$_POST['num'][$i]} AND num_fact = {$_POST['num_fact3'][$i]};\n";
			$data['num_fact4'] = $_POST['num_fact4'][$i];
			$data['pag4'] = $_POST['num_fact4'][$i] > 0 ? ($_POST['sal4'][$i] == 0 ? 0 : round(get_val($_POST['imp4'][$i]) - get_val($_POST['pag4'][$i]) - get_val($_POST['sal4'][$i]), 2)) : '';
			if ($_POST['num_fact4'][$i] > 0 && $_POST['sal4'][$i] == 0) $sql .= "UPDATE facturas_zap SET folio = 0, cuenta = 0, tspago = now() WHERE clave = {$_POST['num'][$i]} AND num_fact = {$_POST['num_fact4'][$i]};\n";
			
			$sql .= $db->preparar_insert('otros_depositos', $data) . ";\n";
		}
		if ($sql != '') $db->query($sql);
		die(header('location: ./ban_dep_otros_v3.php'));
}
...
$tpl->assignInclude("body","./plantillas/ban/ban_dep_otros_v3.tpl");
$tpl->prepare();
...
if (isset($_GET['mes'])) {
	...
	if ($_GET['tipo'] == 'listado') {
		...
		$sql .= " USING(num_cia) WHERE acumulado = 'TRUE'" . ($_SESSION['iduser'] != 1 ? " AND iduser = $_SESSION[iduser]" : '') . " AND num_cia BETWEEN " . ($_SESSION['iduser'] >= 28 ? "900 AND 950" : "1 AND 800");
		$sql .= $_GET['idnombre'] > 0 ? " AND idnombre = $_GET[idnombre]" : "";
		$sql .= " ORDER BY num_cia_primaria, fecha, num_cia";
		$result = $db->query($sql);
		
		if (!$result) die(header('location: ./ban_dep_otros_v3.php'));
		...
		$total_mes = $db->query("SELECT sum(importe) FROM otros_depositos WHERE fecha >= '1/$_GET[mes]/$_GET[anio]' AND fecha <= '" . date("d/m/Y", mktime(0, 0, 0, $_GET['mes'] + 1, 0, $_GET['anio'])) . "' AND num_cia BETWEEN " . (in_array($_SESSION['iduser'], $users) ? "900 AND 950" : "1 AND 800") . ($_GET['idnombre'] > 0 ? " AND idnombre = $_GET[idnombre]" : ''));
		$total_mes_ant = $db->query("SELECT sum(importe) FROM otros_depositos WHERE fecha >= '" . date("d/m/Y", mktime(0, 0, 0, $_GET['mes'] - 1, 1, $_GET['anio'])) . "' AND fecha <= '".date("d/m/Y",mktime(0,0,0,$_GET['mes'],0,$_GET['anio']))."' AND num_cia BETWEEN " . (in_array($_SESSION['iduser'], $users) ? "900 AND 950" : "1 AND 800") . ($_GET['idnombre'] > 0 ? " AND idnombre = $_GET[idnombre]" : ""));
	...
	$tpl->assign('cancelar', isset($_GET['con']) ? './ban_con_dep_v2.php' : './ban_dep_otros_v3.php');
	...
	$cias = $db->query("SELECT num_cia, nombre_corto AS nombre FROM catalogo_companias" . ($_SESSION['iduser'] != 1 ? ' WHERE num_cia BETWEEN ' . ($_SESSION['iduser'] >= 28 ? '900 AND 950' : '1 AND 800') : ''));
	foreach ($cias as $cia) {
		$tpl->newBlock('cia');
