...
if (isset($_REQUEST['accion']))
{
	switch ($_REQUEST['accion'])
	{
		...
		case 'consultar':
			...
			$tpl = new TemplatePower('plantillas/ban/EstadoCuentaReporteV2.tpl');
			$tpl->prepare();
			...
			if ($query)
			{
				...
				foreach ($query as $row)
				{
					if ($num_cia != $row['num_cia'])
					{
						...
						if (isset($_REQUEST['conciliados'])
							&& isset($_REQUEST['depositos'])
							&& isset($_REQUEST['cargos'])
							&& (isset($_REQUEST['fecha1']) && $_REQUEST['fecha1'] != '')
							&& (isset($_REQUEST['fecha2']) && $_REQUEST['fecha2'] != '')
					...
					$depositos_con += $row['conciliado'] != '' && $row['deposito'] > 0 ? $row['deposito'] : 0;
					$cargos_con += $row['conciliado'] != '' && $row['cargo'] > 0 ? abs($row['cargo']) : 0;
...
$tpl = new TemplatePower('plantillas/ban/EstadoCuentaV2.tpl');
$tpl->prepare();
