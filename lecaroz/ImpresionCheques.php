...
$layout = array(
	...
	*/
	2 => array(
		...
		'cheques' => array(
			1 => 'XML/ChequeBanorteEliteNuevo.xml',
			2 => 'XML/ChequeSantanderEliteNuevo.xml'
		)
	)
);
...
function LayoutToPDF($layout, $data = NULL)
{
	global $PDFfunctions, $pdf;
	...
	foreach ($layout as $k => $v)
	{
		...
		if ($k == 'ParaAbono' && $data['para_abono'] != 't')
			continue;
		else if (in_array($k, $PDFfunctions))
		{
			...
			switch ($k)
			{
				...
				case 'Campo':
					if (isset($data[$v['data']]))
					{
						...
						if (isset($v['explode']) && strlen($v['explode']) > 0)
						{
							...
							// Número de piezas por fila, predeterminado: 8 piezas
							$piecesPerRow = isset($v['piecesPerRow']) && intval($v['piecesPerRow']) > 0 ? intval($v['piecesPerRow']) : 8;
							// Contador de piezas
							$cont = 1;
						...
						if (isset($v['multicell']))
						{
							$pdf->MultiCell(floatval($v['width']), floatval($v['height']), $campo);
						}
						else
						{
							$pdf->Cell(floatval($v['width']), 0, $campo);
						}
					}
				break;
				...
				// Leyenda
				case 'Leyenda':
					$pdf->Cell(floatval($v['width']), 0, $v['data']);
				break;

				// Margen superior de la página
				case 'TopMargin':
					$pdf->SetTopMargin(floatval($v['data']));
				break;

				// Margen izquierdo de la página
				case 'LeftMargin':
					$pdf->SetLeftMargin(floatval($v['data']));
				break;

				// Movimiento horizontal
				case 'X':
					$pdf->SetX(floatval($v['data']));
				break;

				// Movimiento vertical
				case 'Y':
					$pdf->SetY(floatval($v['data']));
				break;

				// [15-May-2009] Tipo de fuente
				case 'Font':
					$pdf->SetFont($v['data'], $v['style'], floatval($v['size']));
				break;
			}
		}
		else if (is_array($v))
			...
			switch ($k)
			{
				...
				case 'Campo':
					if (isset($data[$v['data']]))
					{
						...
						else if (isset($v['charsPerLine']))
						{
							$chunks = str_split($campo, $v['charsPerLine']);

							$pcl .= MoveCursorV(-3 * (count($chunks) - 1), TRUE);

							foreach ($chunks as $i => $chunk)
							{
								$pcl .= MoveCursorH($x);
								$pcl .= MoveCursorV(3 * $i, TRUE);
				...
				case 'Desglose':
					$pcl .= desglosePagos($data['num_cia'], $data['clave_banco'], $data['folio'], $data['fecha']);
				break;
...
function chequePDF($data, $cuenta = NULL, $layout)
{
	global $datosBanco;
...
function desglosePagos($num_cia, $cuenta, $folio, $fecha)
{
	if ($num_cia < 900)
	{
		$sql = "SELECT
		...
		WHERE
			fp.num_cia = {$num_cia}
			AND fp.cuenta = {$cuenta}
			AND fp.folio_cheque = {$folio}
			AND fp.fecha_cheque = '{$fecha}'
		ORDER BY
			num_fact";
	}
	else
	...
	$xml = new XMLClass('XML/Desglose' . ($num_cia < 900 ? 'Mollendo' : 'Elite') . '.xml', 'file');
	$xml->parse();
...
if ((isset($_POST['id']) && count($_POST['id']) > 0) || (isset($_GET['polizas']) && $_GET['polizas'] > 0))
{
	if (isset($_GET['polizas']))
	{
		$sql = '
			...
			WHERE
					c.num_cia
						BETWEEN
								' . ($_SESSION['tipo_usuario'] == 2 ? 900 : 1) . '
							AND
								' . ($_SESSION['tipo_usuario'] == 2 ? 998 : 899) . '
				AND
					imp = \'FALSE\'
				AND
	...
	@
	*/
	$oficina = $_SESSION['tipo_usuario'] == 2 ? 2 : 1;
	...
	if (isset($xml->data['Documento']['FileType']['data']) && $xml->data['Documento']['FileType']['data'] == 'PDF')
	{
		...
		class PDF extends FPDF {
			function Header()
			{
				...
				if (isset($xml->data['Documento']['Cabecera']['ImageHeader']))
				{
					$this->Image(
						$xml->data['Documento']['Cabecera']['ImageHeader']['data'],
						$xml->data['Documento']['Cabecera']['ImageHeader']['x'],
						$xml->data['Documento']['Cabecera']['ImageHeader']['y'],
						$xml->data['Documento']['Cabecera']['ImageHeader']['width']
					);
				}
			}
		}
		...
		$pdf = new PDF(
			$xml->data['Documento']['Cabecera']['Orientation']['data'],
			$xml->data['Documento']['Cabecera']['Units']['data'],
			$xml->data['Documento']['Cabecera']['Format']['data'] == 'UNIVERSAL'
				? array(
					$xml->data['Documento']['Cabecera']['Format']['width'],
					$xml->data['Documento']['Cabecera']['Format']['height']
				)
				: $xml->data['Documento']['Cabecera']['Format']['data']
		);
	}
	else
	{
	...
	foreach ($result as $i => $reg)
	{
		...
		else
		{
			...
			{
				$pdf->AddPage();
				chequePDF($reg, $cuenta, $xml->data['Documento']['PseudoPoliza']);
			}
			else
			{
	...
	else
	{
		if (isset($xml->data['Documento']['FileType']['data']) && $xml->data['Documento']['FileType']['data'] == 'PDF')
		{
			$pdf->Output('pcl/MollendoPolizas.pdf', 'F');
		}
		else
		{
		...
		shell_exec('lp -d ' . ($folio > 0 ? 'cheques' : 'polizas') . ' pcl/Mollendo' . ($folio > 0 ? 'Cheques' : 'Polizas') . (isset($xml->data['Documento']['FileType']['data']) && $xml->data['Documento']['FileType']['data'] == 'PDF' ? '.pdf' : '.pcl'));
	}
	shell_exec("chmod ugo=r pcl");
...
if (isset($_GET['idstatus']))
{
	...
	if ($result)
	{
		...
		$tpl->assign('firma', $estatus[0]['oficina'] == 2 ? 'MARCOS VEGA ANGELES' : 'JULIAN EUGENIO LARRACHEA ECHENIQUE');
	}
...
$sql = '
	...
	WHERE
			estatus = 0
		AND
			tipo_usuario =' . ($_SESSION['tipo_usuario'] == 2 ? '2' : '1') . '
';
$estatus = $db->query($sql);
