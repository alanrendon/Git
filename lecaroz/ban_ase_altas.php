...
// Tabla 'catalogo_aseguradoras'
// Menu 'Bancos->Catalogos'

define ('IDSCREEN',6614); // ID de pantalla
...
$descripcion_error[1] = "C&oacute;digo de aseguradora ya existe en la Base de Datos";
...
$db = DB::connect($dsn);
if (DB::isError($db)) {
	echo "Error al intentar acceder a la Base de Datos. Avisar al administrador. ban_ase_tas.<br>";
	die($db->getMessage());
}
...
if (DB::isError($result)) {
	$db->disconnect();
	echo "Error en script SQL: $sql<br>Avisar al administrador. ban_ase_altas.<br>";
...
$id = nextid($screen->tabla, "idaseguradora", $dsn);
$tpl->assign("id",$id);
