<?php
// ALTA DE SUBDELEGACIONES DEL IMSS
// Tabla 'catalogo_subdel_imss'
// Menu 'Bancos->Catalogos'

define ('IDSCREEN',6617); // ID de pantalla
...
$descripcion_error[1] = "C&oacute;digo de subdelegaci&oacute;n ya existe en la Base de Datos";
...
$db = DB::connect($dsn);
if (DB::isError($db)) {
	echo "Error al intentar acceder a la Base de Datos. Avisar al administrador. ban_sdi_tas.<br>";
	die($db->getMessage());
}
...
if (DB::isError($result)) {
	$db->disconnect();
	echo "Error en script SQL: $sql<br>Avisar al administrador. ban_sdi_altas.<br>";
...
$id = nextid($screen->tabla, "idsubdelimss", $dsn);
$tpl->assign("id",$id);
