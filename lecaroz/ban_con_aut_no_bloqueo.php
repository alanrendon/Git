...
$tpl->assignInclude("body","./plantillas/ban/ban_con_aut_no_bloqueo.tpl");
$tpl->prepare();
...
// Analizar archivo, obtener movimientos
	...
	if (!/*(stristr($tipo_archivo,"text/plain") && $tamano_archivo < 5242880)*/1) {
		...
		// Si ya existe el archivo en el server, mandarlo a la página de resultados
			...
			// Crear cadena hash MD5
			header("location: ./ban_con_aut_no_bloqueo.php?codigo_error=1");
			die;
		}

		header("location: ./ban_con_aut_no_bloqueo.php?val_cue=1");
		die;
	}
}
// Validar cuentas
if (isset($_GET['val_cue'])) {
	...
	if ($result) {
		...
		header("location: ./ban_con_aut_no_bloqueo.php?val_cod=1");
		die;
	}
}
...
if (isset($_GET['val_cod'])) {
	...
	if ($count > 0) {
		...
		header("location: ./ban_con_aut_no_bloqueo.php?cod3=1");
		die;
	}
}
...
if (isset($_GET['cod3'])) {
	...
	if ($mov_ban) {
		...
		header("location: ./ban_con_aut_no_bloqueo.php?conciliar=1");
	}
}
...
if (isset($_GET['conciliar'])) {
	...
	$imss = ejecutar_script("SELECT * FROM mov_banorte WHERE fecha_con IS NULL AND num_cia > 0 AND cod_banco = 503 AND concepto = 'PAGO DE SUA IMSS' ORDER BY num_cia, fecha", $dsn);
	if ($imss) {
		$sql = '';
		$num_cia = NULL;
		foreach ($imss as $reg) {
	...
	*/
	...
	header("location: ./ban_con_aut_no_bloqueo.php?saldos=1");
	die;
}
...
if (isset($_GET['saldos'])) {
	...
	if (!$result)
		die(header('location: ./ban_con_aut_no_bloqueo.php?resultados=1'));
...
if (isset($_GET['resultados'])) {
	...
	if (!$mov_lib && !$mov_aut) die(header('location: ./ban_con_aut_no_bloqueo.php'));
