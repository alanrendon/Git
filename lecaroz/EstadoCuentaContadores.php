...
if (isset($_REQUEST['accion'])) {
	switch ($_REQUEST['accion']) {
		...
		case 'consultar':
			...
			if ( ! in_array($_SESSION['iduser'], array(1, 4, 26, 67, 2, 79))) {
				if ($_SESSION['tipo_usuario'] == 2) {
					$condiciones[] = 'ec.num_cia BETWEEN 900 AND 998';
				}
				else {
					$condiciones[] = 'ec.num_cia BETWEEN 1 AND 998';
				}
			}
			...
			if ((isset($_REQUEST['fecha1']) && $_REQUEST['fecha1'] != '')
				...
				if ((isset($_REQUEST['fecha1']) && $_REQUEST['fecha1'] != '')
					&& (isset($_REQUEST['fecha2']) && $_REQUEST['fecha2'] != '')) {
					$condiciones[] = 'ec.fecha_con BETWEEN \'' . $_REQUEST['fecha1'] . '\' AND \'' . $_REQUEST['fecha2'] . '\'';
				} else if (isset($_REQUEST['fecha1']) && $_REQUEST['fecha1'] != '') {
					$condiciones[] = 'ec.fecha_con >= \'' . $_REQUEST['fecha1'] . '\'';
				} else if (isset($_REQUEST['fecha2']) && $_REQUEST['fecha2'] != '') {
					$condiciones[] = 'ec.fecha_con = \'' . $_REQUEST['fecha2'] . '\'';
				}
			}
			...
			$tpl = new TemplatePower('plantillas/ban/EstadoCuentaContadoresReporte.tpl');
			$tpl->prepare();
				...
				foreach ($query as $row) {
					if ($num_cia != $row['num_cia']) {
						...
						if (isset($_REQUEST['depositos'])
							...
							&& (!isset($_REQUEST['folios']) || $_REQUEST['folios'] == '')
							&& (!isset($_REQUEST['pros']) || $_REQUEST['pros'] == '')
							&& (!isset($_REQUEST['gastos']) || $_REQUEST['gastos'] == '')
							&& (!isset($_REQUEST['importes']) || $_REQUEST['importes'] == '')
							&& (!isset($_REQUEST['concepto']) || $_REQUEST['concepto'] == '')) {
							$condiciones = array();
					...
					$tpl->assign('totales.depositos', $depositos != 0 ? number_format($depositos, 2) : '--');
...
$tpl = new TemplatePower('plantillas/ban/EstadoCuentaContadores.tpl');
$tpl->prepare();
