<?php
// ALTA DE DELEGACIONES DEL IMSS
// Tabla 'catalogo_del_imss'
// Menu 'Bancos->Catalogos'

define ('IDSCREEN',6615); // ID de pantalla
...
$descripcion_error[1] = "C&oacute;digo de delegaci&oacute;n ya existe en la Base de Datos";
...
$db = DB::connect($dsn);
if (DB::isError($db)) {
	echo "Error al intentar acceder a la Base de Datos. Avisar al administrador. ban_dim_tas.<br>";
	die($db->getMessage());
}
...
if (DB::isError($result)) {
	$db->disconnect();
	echo "Error en script SQL: $sql<br>Avisar al administrador. ban_dim_altas.<br>";
...
$id = nextid($screen->tabla, "iddelimss", $dsn);
$tpl->assign("id",$id);
