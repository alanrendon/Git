...
//	echo "prueba de pan con pasteles y usando DB";
...
//----------------------CODIGO PARA SACAR EL PASTEL ENTREGADO
				$facturas_pago_anticipado=$db->query("select sum(cuenta) as cuenta, sum(base) as base from venta_pastel where fecha_entrega='".$fechas[$i]['fecha_total']."' and cuenta > 0 and fecha != '".$fechas[$i]['fecha_total']."' and num_cia=".$companias[$j]['num_cia']);
				$facturas_posteriores=$db->query("select sum(cuenta) as cuenta, sum(base) as base from venta_pastel where fecha='".$fechas[$i]['fecha_total']."' and resta is null and cuenta > 0 and dev_base is null and num_cia=".$companias[$j]['num_cia']." and fecha_entrega > '".$fechas[$i]['fecha_total']."'");
...
//				$pastel=$db->query("select venta_pastel + abono_pastel as pastel from total_panaderias where num_cia=".$companias[$j]['num_cia']." and fecha = '".$fechas[$i]['fecha_total']."'");
			
				$total_pasteles = number_format($facturas_pago_anticipado[0]['cuenta'],2,'.','') - number_format($facturas_pago_anticipado[0]['base'],2,'.','') - number_format($facturas_posteriores[0]['cuenta'],2,'.','') - number_format($facturas_posteriores[0]['base'],2,'.','') - number_format($gasto_cancelacion[0]['sum'],2,'.',''); 
//----------------------	
...
//----------------------CODIGO PARA SACAR EL PASTEL ENTREGADO
				$facturas_pago_anticipado=$db->query("select sum(cuenta) - sum(base) as total from venta_pastel where fecha_entrega='".$fechas[$i]['fecha_total']."' and cuenta > 0 and fecha != '".$fechas[$i]['fecha_total']."' and num_cia=".$companias[$j]['num_cia']);
				$facturas_posteriores=$db->query("select sum(cuenta) - sum(base) as total from venta_pastel where fecha='".$fechas[$i]['fecha_total']."' and resta is null and cuenta > 0 and dev_base is null and num_cia=".$companias[$j]['num_cia']." and fecha_entrega > '".$fechas[$i]['fecha_total']."'");
				$gasto_cancelacion=$db->query("select sum(importe) from movimiento_gastos where fecha='".$fechas[$i]['fecha_total']."' and num_cia=".$companias[$j]['num_cia']." and codgastos=115 and concepto like 'CANCELACION%PASTEL'");
				$pastel=$db->query("select venta_pastel + abono_pastel as pastel from total_panaderias where num_cia=".$companias[$j]['num_cia']." and fecha = '".$fechas[$i]['fecha_total']."'");
			
				$total_pasteles = number_format($facturas_pago_anticipado[0]['total'],2,'.','') - number_format($facturas_posteriores[0]['total'],2,'.','') - number_format($gasto_cancelacion[0]['sum'],2,'.',''); 
//----------------------	

				$sql="SELECT * from total_panaderias where num_cia=".$companias[$j]['num_cia']." and fecha='".$fechas[$i]['fecha_total']."'";
				$efectivos_tabla=$db->query($sql,$dsn);
