...
{
	echo "GenerarDatosPagosV2.php Ver 2.15.9.8";
	echo "\nCopyright (c) 2015, Lecaroz";
	echo "\n\nModo de empleo: php GenerarDatosPagosV2.php [OPCIONES]";
...
(
	...
	FROM
		facturas sf
		LEFT JOIN facturas_pagadas sfp ON (
			sfp.num_proveedor = sf.num_proveedor
			AND sfp.num_fact = sf.num_fact
			AND sfp.fecha = sf.fecha
		)
		LEFT JOIN estado_cuenta sec ON (
			sec.num_cia = sfp.num_cia
			AND sec.cuenta = sfp.cuenta
			AND sec.folio = sfp.folio_cheque
			AND sec.fecha = sfp.fecha_cheque
		)
	WHERE
		{$condiciones_string}
	GROUP BY
	...
	f.id"))
{
	...
	foreach ($datos_pago as $i => $dp)
	{
		if ($id_pago != $dp['id_pago'])
		{
			if ($id_pago != NULL)
			{
				...
				if ( ! $no_cargar)
				{
					...
					if ($response === FALSE)
					{
						$db->query("UPDATE facturas SET ts_carga_pago_conta_error = 'Webservice no disponible', ts_carga_pago_conta = NULL WHERE id IN (" . implode(', ', $facs_id) . ")");
						...
						if ( ! $nderr && $ob_err >= $max_ob_err)
						{
							...
							if ($ids)
							{
								...
								$db->query("UPDATE facturas SET ts_carga_pago_conta = NOW(), ts_carga_pago_conta_error = NULL WHERE id IN (" . implode(', ', $ids) . ")");
							}
					...
					else
					{
						...
						{
							$db->query("UPDATE facturas SET ts_carga_pago_conta_error = 'Imposible decodificar respuesta de openbravo: {$response}', ts_carga_pago_conta = NULL WHERE id IN (" . implode(', ', $facs_id) . ")");
						...
						else if ($ob_data->status < 0)
						{
							...
							$db->query("UPDATE facturas SET ts_carga_pago_conta_error = '{$msg}', ts_carga_pago_conta = NULL WHERE id IN (" . implode(', ', $facs_id) . ")");
			...
			echo "\n[{$ts}](DD) Banco pago: " . ($dp['banco'] == 1 ? 'BANORTE' : ($dp['banco'] == 1 ? 'SANTANDER' : 'DESCONOCIDO'));
		...
		echo "\n[{$ts}](DD) Importe factura: {$dp['importe_factura']}";
		...
		$datos['facturas'][] = array(
			...
			'importe'	=> get_val($dp['importe_factura'])
		);
		...
		if ($dp['carga_factura'] == 'f' || $forzar_carga)
		{
			...
			$datos_fac = array(
				'cabecera'	=> array (
					...
					'descuento'				=> 0,
					'ieps'					=> get_val($query_fac[0]['ieps']),
					'iva'					=> get_val($query_fac[0]['iva']),
					'importe_retencion_isr'	=> get_val($query_fac[0]['retencion_isr']),
					'importe_retencion_iva'	=> get_val($query_fac[0]['retencion_iva']),
					'total'					=> get_val($query_fac[0]['total'])
				),
				'detalle'	=> array()
			);
			...
			if ($detalles = $db->query("SELECT
				...
				WHERE
					d.num_proveedor = {$query_fac[0]['num_pro']}
					AND d.num_fact = '{$query_fac[0]['num_fact']}'
					AND d.fecha = '{$query_fac[0]['fecha']}'
					AND regalado = FALSE"))
			{
			...
			{
				$tax = $query_fac[0]['iva'] != 0 && $query_fac[0]['retencion_iva'] != 0 && $query_fac[0]['retencion_isr'] != 0 ? 'HONORARIOS/ARRENDAMIENTOS' : ($query_fac[0]['iva'] != 0 && $query_fac[0]['retencion_iva'] != 0 ? 'IVA + RET 4%' : ($query_fac[0]['retencion_isr'] != 0 ? (abs(round($df['retencion_isr'] * 100 / $query_fac[0]['importe'], 2)) == 10 ? 'ARRENDAMIENTO HABITACION' : 'R35%') : ($query_fac[0]['iva'] != 0 ? 'IVA' : 'IVA 0')));

				$datos_fac['detalle'][] = array(
					'clave'			=> "G{$query_fac[0]['codigo']}",
			...
			if ( ! $no_cargar)
			{
				...
				if ($response === FALSE)
				{
					$db->query("UPDATE facturas SET ts_carga_conta_error = 'Webservice no disponible', ts_carga_conta = NULL WHERE id = {$dp['id_factura']}");
				...
				else
				{
					...
					if ( ! isset($ob_data->status))
					{
						$db->query("UPDATE facturas SET ts_carga_conta_error = 'Imposible decodificar respuesta de openbravo: {$response}', ts_carga_conta = NULL WHERE id = {$dp['id_factura']}");
					...
					else if ($ob_data->status < 0)
					{
						...
						$db->query("UPDATE facturas SET ts_carga_conta_error = '{$msg}', ts_carga_conta = NULL WHERE id = {$dp['id_factura']}");
					...
					else
					{
						...
						$db->query("UPDATE facturas SET ts_carga_conta = NOW(), ts_carga_conta_error = NULL WHERE id = {$dp['id_factura']}");
					}
				}
			}
			else
...
function generar_xml_datos($datos, $guardar_xml = FALSE)
{
	...
	if ($guardar_xml)
	{
		...
		$nombre_xml = "carga-pagos-{$datos['pago']['id_pago']}-{$datos['pago']['num_cia_pago']}-{$datos['pago']['cuenta_pago']}-{$datos['pago']['folio_pago']}.xml";
