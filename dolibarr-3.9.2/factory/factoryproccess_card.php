...
if (empty($reshook))
{
	...
	//print_r($_POST);
		...
		print '<script>location.href="factoryProces_list.php";</script>';
	...
	if ($action == 'confirm_delete')
	{
		...
		if ($result > 0)
		{
			...
			header("Location: ".dol_buildpath('/factory/factoryProces_list.php',1));
			exit;
		}
		else
		{
...
// Part to show record
if ( $action == 'view' || $action == 'delete' || $action == 'cerrar' || $action == 'cerrarTrat')
{
	print load_fiche_titre($langs->trans("Detalle proceso"));
	...
	print '<br/>';
		...
		if ($user->rights->factory->maqui && $dat->STATUS==1)
		{
		...
		if ($user->rights->factory->maqui && $dat->STATUS==3)
		{
...
if($action2=="Interrumpir"){
	...
	print '<fieldset>';
		print '<br/>';
		print load_fiche_titre($langs->trans("Interrupción del proceso"));	
		print '<form method="post" action="'.$_SERVER["PHP_SELF"].'?idProp='.$idPro.'&idProd='.$idProd.'&action=view">';			
		print '<table class="border centpercent"  width="100%">';						
			print '<tr>';
