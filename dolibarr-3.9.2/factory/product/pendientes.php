...
if ($id || $ref) {
	if ($result) {
		...
		$sql = "SELECT f.rowid as idorden, fd.fk_product as idprod, label, stock, fd.qty_planned as cant FROM ".MAIN_DB_PREFIX."factory f ";
		...
		if( $num > 0 ) {
			...
			print '<div width="100%" align="right" style="padding:10px">
						<a href="pdf/pendientes.php?id='.$id.'" class="butAction" target="_blank">Ver PDF</a>
						<a href="resumen.php?id='.$id.'" id="ver_resumen" class="butAction">Ver resumén</a>
					</div>';
			...
			print '<td>Orden de producción</td>';
