<?php
	...
	$sql = "SELECT f.rowid as idorden, fd.fk_product as idprod, p.ref as ref_prod, label, SUM(stock) stock, SUM(fd.qty_planned) cant FROM ".MAIN_DB_PREFIX."factory f ";
		$sql .= "LEFT JOIN ".MAIN_DB_PREFIX."factorydet fd ON fd.fk_factory = f.rowid ";
		$sql .= "LEFT JOIN ".MAIN_DB_PREFIX."product p ON p.rowid = fd.fk_product WHERE stock <= 0 GROUP BY idprod";
	$res = $db->query($sql);
	$num = $db->num_rows($res);
	$contador = 1;
