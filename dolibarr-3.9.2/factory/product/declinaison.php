...
/**
 *  \file       htdocs/factory/product/declinaison.php
 *  \ingroup    serem
			...
			if ($ret > 0)
			{
				...
				if ($resvariantsql)
				{
					...
					if ($numvar > 0 )
					{
						...
						$nbpossible=1;
						foreach($tblElemVariant as $lgncomponent)
						{
							$nbelement=count(explode(":",$lgncomponent[2]))-1;	
							if ($nbelement >0)
							$nbpossible=$nbpossible*$nbelement;
						}

						for($k=0; $k < $nbpossible; $k++)
						{
							// on récupère l'élément variable dans le tableau
							$sznewref="";
							foreach($tblElemVariant as $key => $lgncomponent)
							{
								$sznewref.=getelementVariant($key)."_";
								// on passe à l'élément suivant
								$tblElemVariant[$key][0]++;
							}
							if (btest) print "key=".$key."=".$sznewref."<br>";

							// ensuite on incrémente le dernier et on remonte en récursif
							incrementevariant($key);

							$listNewProd[$k]=substr($sznewref, 0, -1);
						}
						...
						foreach($listNewProd as $productelement)
						{
							...
							if ($retnewref)
							{
								$numnewref = $db->num_rows($retnewref);
								if ($numnewref > 0)
								{
									//print $newref." a modifier<br>";
							...
							$sql.= " value (".$objectelement->id.", ".$conf->global->FACTORY_CATEGORIE_CATALOG.")";	
							...
							if ($restraduction)
							{
								...
								if ($btest) print "nbtrads=".$numtrad."<br>" ;
								if ($numtrad > 0 )
								{
									// on crée un tableau avec les variants
									$j = 0;
