 ...
 *  \ingroup    product
 *  \brief      Page de d�finition de la fabrication
 */
...
if ( $action == 'add_prod' && $cancel <> $langs->trans("Cancel") && ($user->rights->produit->creer || $user->rights->service->creer))
{
	...
	for($i=0;$i<$_POST["max_prod"];$i++)
	{
		...
		if($_POST["prod_id_chk".$i] != "")
		{
			...
			//if($factory->add_component($id, $_POST["prod_id_".$i], $_POST["prod_qty_".$i], $pmp2, $price2, $_POST["prod_id_globalchk".$i], $_POST["descComposant".$i], $_POST["treatment_".$i]) > 0)
...
{	// on r�cup�re la fabrication de la composition virtuelle
...
if ($id || $ref)
{
	if ($result)
	{
		...
		// pour connaitre les produits compos� du produits
		$prodsfather = $factory->getFather(); //Parent Products
		...
		if ($action == 'edit' || $action == 'search' || $action == 're-edit' )
		{
			...
			if ($action == 'search')
			{
				...
				//print '<th class="liste_titre" align="right">'.$langs->trans("Trat").'</th>';
				...
				if ($resql)
				{
					...
					{	
						...
						$sqlt = 'SELECT p.rowid, p.ref, p.label';
						$sqlt.= ' FROM '.MAIN_DB_PREFIX.'product as p';				
						$sqlt.= ' WHERE p.ref like "tra%"';
						$sqlt.= " AND p.fk_product_type =1 ";				
						$sqlt.= " ORDER BY p.ref ASC";
...
/*
						print '<td align="right">';
							print '<select name="treatment_'.$i.'" >';
								...
								while ($objt = $db->fetch_object($resqlt)) {
									print '<option name="treatment_opt_'.$i.'" value='.$objt->rowid.'>'.$objt->ref.' - '.$objt->label.'</option>';
								}								
							print '</select>';
						print '</td>';*/					
...
if ($action == '' && $bproduit)
{
	...
	if ($user->rights->factory->creer )
	{
		...
		if ($conf->stock->enabled && $object->status_buy ==0)
		{
			...
			//print '<a class="butAction" href="'.DOL_URL_ROOT.'/factory/product/reporteExcelFormatCotizacion.php?id='.$productid.'" target="_blank">Excel Formato Cotizacion</a>';							
