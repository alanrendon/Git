...
if (! empty($conf->global->FACTORY_ADDON)) {
	if($conf->global->FACTORY_ADDON=='._mod_babouin'){
		...
		if($conf->global->FACTORY_ADDON=='._mod_mandrill'){
			...
			dol_include_once("/core/modules/factory/".$conf->global->FACTORY_ADDON.".php");
		}
	}
...
llxHeader('',$langs->trans("CardFactory"),'','','','',$morejs,'',0,0);
...
dol_fiche_head($head, 'factoryreport', $titre, 0, $picto);
	...
	$statut_entrpot = ($factory->fk_statut_entrpot == 1) ? 'Validado' : 'Sin validar';
	print '<tr><td>Estatus Almacén</td><td colspan=3>'.$statut_entrpot.'</td></tr>';
	...
	print '<tr><td>No. de serie</td>';
	if( !empty($factory->no_serie) ) {
		print '<td colspan=3>'.$factory->no_serie.'</td>';
	}
	else if ( $factory->fk_statut == 1 ) {
		print '<td colspan=3>';
		print '<table>';
		print '<tr><td colspan=2 valign="top">Define número inicial: <input type="text" name="numero_inicial_consecutivo" size="2" /> <input type="button" class="button" name="generar" value=" Generar números de serie" /></td></tr>';
		...
		print '<td colspan=3>Sin asignar<td>';
	}
	print '</tr>';
	print '</td></tr>';
	print '</table>';
...
//	$factory->id = $product->id;
	...
	if( $factory->fk_statut > 1 ) {
		...
		print '<a href="product/pdf/orden.php?id='.$id.'" id="ver_detalle" class="butAction" target="_blank">Producción finalizada (PDF)</a>';
		if( $factory->fk_statut_entrpot == 0 ) {
			print '<a href="#" id="'.$id.'" class="butAction validar_almacen">Validación de Almacén</a>';
		}
		print '</div>';
	}
