...
if ($id > 0 || ! empty($ref))
{
	if ($object->fetch($id,$ref) > 0)
	{
		...
		if (! empty($projectstatic->socid)) $projectstatic->fetch($projectstatic->socid);
...
if ( $action == 'add_prod' && $cancel <> $langs->trans("Cancel") && $user->rights->factory->creer )
{
	...
	for($i=0;$i<$_POST["max_prod"];$i++)
	{
		...
		{
			if($factory->add_componenttask($id, $_POST["prod_id_".$i], $_POST["prod_qty_".$i], $_POST["pmp_".$i], $_POST["price_".$i] ) > 0)
			{
				$action = '';
			}
			else
...
{	// on modifie les prix 
	...
	if (count($prods_arbo) > 0)
	{
		foreach($prods_arbo as $value)
			$factory->updatefactorytaskprices($id, $value['id'], GETPOST("prod_pmp_".$value['id']),GETPOST("prod_price_".$value['id']));
	}
	$action="";
}
if ($action == 'updateqty')
{	// on modifie les quantités 
	...
	{
		foreach($prods_arbo as $value)
			$factory->updatefactorytaskqty($id, $value['id'], GETPOST("qtyused_".$value['id']),GETPOST("qtydeleted_".$value['id']));
	}
	$action="";
}
...
if (count($prods_arbo) > 0)
{
	...
	print '<td colspan="5" align="center">Cant. Prevista</td>';
	print '<td colspan="5" align="center">Cant. Consumida</td>';
	print '</tr>';
	print '<tr class="liste_titre">';
	...
	foreach($prods_arbo as $value)
	{
		...
		if ($user->rights->factory->showprice )
		{
			...
			/*Consumido*/
			print '<td align="right">'.price($pmp).'</td>'; // display else vwap or else latest purchasing price
			print '<td align="right">'.price($pmp*$value['qtyused']).'</td>'; // display total line
			print '<td align="right">'.price($price).'</td>';
			print '<td align="right">'.price($price*$value['qtyused']).'</td>';
			print '<td align="right">'.price(($price-$pmp)*$value['qtyused']).'</td>';
			/*Consumido*/
			...
			/*Consumido*/
			$mntTotCon=$mntTotCon+$productstatic->price*$value['qtyused'];
			$pmpTotCon=$pmpTotCon+$pmp*$value['qtyused'];
			/*Consumido*/
		}
		print '</tr>';
	...
	if ($user->rights->factory->showprice )
	{
		...
		/*Consumido*/
		print '<td ></td>';
		print '<th align="right" >'.price($pmpTotCon).'</th>';
		print '<td ></td>';
		print '<th align="right" >'.price($mntTotCon).'</th>';
		print '<th align="right" >'.price($mntTotCon-$pmpTotCon).'</th>';
		/*Consumido*/
	}
	print '</tr>';
	print '</table>';
...
if ($action == 'adjustprice')
{
	...
	print '<form action="'.DOL_URL_ROOT.'/factory/project/factorytask.php?id='.$id.'" method="post">';
...
if ($action == 'adjustquantity')
{
	...
	foreach($prods_arbo as $value)
	{
		...
		print '<td align="right"><input type="text" size="5" name="qtyused_'.$value['id'].'" value="'.$qtyused.'">';
		print '<td align="right"><input type="text" size="5" name="qtydeleted_'.$value['id'].'" value="'.$value['qtydeleted'].'">';
...
if ($action == 'edit' || $action == 'search' || $action == 're-edit' )
{
	...
	print '<form action="'.DOL_URL_ROOT.'/factory/project/factorytask.php" method="post">';
...
if ($action == '' )
{
	...
	if ($user->rights->factory->creer )
	{
		...
		if ($conf->stock->enabled )
		{
			print '<a class="butAction" href="'.DOL_URL_ROOT.'/factory/project/factorytask.php?action=edit&withproject=1&id='.$id.'">'.$langs->trans("EditComponent").'</a>';

			if ($compositionpresente)
			{
				// gestion des prix
				print '<a class="butAction" href="'.DOL_URL_ROOT.'/factory/project/factorytask.php?action=getdefaultprice&withproject=1&id='.$id.'">'.$langs->trans("GetDefaultPrice").'</a>';
				print '<a class="butAction" href="'.DOL_URL_ROOT.'/factory/project/factorytask.php?action=adjustprice&withproject=1&id='.$id.'">'.$langs->trans("AdjustPrice").'</a>';

				if ($projectstatic->statut == 1) // saisie des quantités consommés uniquement si le projet est valide
					print '<a class="butAction" href="'.DOL_URL_ROOT.'/factory/project/factorytask.php?action=adjustquantity&withproject=1&id='.$id.'">'.$langs->trans("AdjustQty").'</a>';

				if ($projectstatic->statut == 2) // actualisation de la formule du produit mis en description
					print '<a class="butAction" href="'.DOL_URL_ROOT.'/factory/project/factorytask.php?action=moveproduct&withproject=1&id='.$id.'">'.$langs->trans("UseCompositionInProduct").'</a>';
				...
				if($conf->global->MAIN_MODULE_EQUIPEMENT && $projectstatic->statut == 1)
				{
					...
					print '<a class="butAction" href="'.DOL_URL_ROOT.'/factory/project/factorytask.php?action=selectequipement&withproject=1&id='.$id.'">'.$langs->trans("SelectEquipement").'</a>';
				}


				if ($projectstatic->statut > 0) // transfert des produits sur une autre tache
				print '<a class="butAction" href="'.DOL_URL_ROOT.'/factory/project/factorytask.php?action=movetoanothertask&withproject=1&id='.$id.'">'.$langs->trans("MoveCompositionToAnotherTask").'</a>';
...
// use it when on the core
	...
	function select_tasks($socid=-1, $selected='', $htmlname='taskid', $maxlength=16, $open_only=0, $show_empty=1)
	{
		...
		if ($tasksListId !== false) $sql.= " AND t.rowid IN (".$tasksListId.")";
		if ($socid == 0) $sql.= " AND (p.fk_soc=0 OR p.fk_soc IS NULL)";
		if ($open_only) $sql.= " AND t.progress < 100";
