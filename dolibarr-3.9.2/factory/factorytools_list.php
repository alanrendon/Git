...
/**
 *   	\file       root/factorytools_list.php
 *		\ingroup    root
...
dol_include_once('/root/class/factorytools.class.php');
...
$hookmanager->initHooks(array('factorytoolslist'));
...
't.fk_operator'=>array('label'=>$langs->trans("Fieldfk_operator"), 'checked'=>1),
...
't.comment'=>array('label'=>$langs->trans("Fieldcomment"), 'checked'=>1),
...
$sql.= " FROM ".MAIN_DB_PREFIX."factory_tools as t";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)) $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."factory_tools_extrafields as ef on (u.rowid = ef.fk_object)";
$sql.= " WHERE 1 = 1";
//$sql.= " WHERE u.entity IN (".getEntity('mytable',1).")";

if ($search_fk_operator) $sql.= natural_search("fk_operator",$search_fk_operator);
...
if ($search_comment) $sql.= natural_search("comment",$search_comment);
...
if ($search_comment != '') $params.= '&amp;search_comment='.urlencode($search_comment);
...
if (! empty($arrayfields['t.comment']['checked'])) print '<td class="liste_titre"><input type="text" class="flat" name="search_comment" value="'.$search_comment.'" size="10"></td>';
...
if (! empty($arrayfields['t.fk_operator']['checked'])) print '<td>'.$obj->fk_operator.'</td>';
if (! empty($arrayfields['t.fk_product']['checked'])) print '<td>'.$obj->fk_product.'</td>';
if (! empty($arrayfields['t.type']['checked'])) print '<td>'.$obj->type.'</td>';
if (! empty($arrayfields['t.status']['checked'])) print '<td>'.$obj->status.'</td>';
if (! empty($arrayfields['t.comment']['checked'])) print '<td>'.$obj->comment.'</td>';
