...
if ($search_fk_operator != '') $params.= '&amp;search_fk_operator='.urlencode($search_fk_operator);
    ...
    print '<div align="right">';		
		print '<a class="button" href="factoryTreatment_card.php?action=create">Asignar Tratamiento </a>';
	print '</div>';
	print '<br/>';
...
print '<td class="liste_titre" >';	
		print '<select class="flat" name="search_status">
			...
			<option value=5>Inicializado</option>
			<option value=7>Interrumpido</option>
			<option value=6>Tratamiento Finalizado</option>
    ...
    while ($i < $num)
    {
        ...
        if ($obj)
        {
			...
			if (! empty($arrayfields['r.fk_operator']['checked'])){
				print '<td>';
					if (!empty($obj->refOp)) {
            ...
            // Action column
            print '<td><a href="factoryTreatment_card.php?action=view&idProp='.$obj->fk_propal.'&idProd='.$obj->fk_product.'" class="button">Ver</a></td>';
    		print '</tr>';
