...
if($action=='updateAlm'){
	...
	if(strcmp($val,'No asignado')==0){
		$string="insert into ".MAIN_DB_PREFIX."const (name, entity, value) values('".$name."',".$conf->entity.", ".$alm.")";
		$res=$db->query($string);
	}else{			
		$string="update ".MAIN_DB_PREFIX."const set value=".$alm." where name='".$name."' and  entity = ".$conf->entity;		
		$res=$db->query($string);
	}	
...
if($action=='updateTC'){
	...
	if($n>0){
		$string="update ".MAIN_DB_PREFIX."const set value=".$tc." where name='POS_TC' and  entity = ".$conf->entity;
		
		$res=$db->query($string);
	}else{
		$string="insert into ".MAIN_DB_PREFIX."const (name, entity, value) values('POS_TC',".$conf->entity.", ".$tc.")";
		$res=$db->query($string);
	}
}
 ...
 */
print '<br>';
print_titre($langs->trans("Tipo de cambio"));

$sql = "SELECT name, value";
$sql.= " FROM ".MAIN_DB_PREFIX."const";
$sql.= " WHERE name = 'POS_TC'";
$sql.= " AND entity = ".$conf->entity;
$resql=$db->query($sql);
$n=$db->num_rows($resql);
...
print '<form method="post" action ="'.$_SERVER['PHP_SELF'].'?action=updateTC&val='.$n.'" >';
print '<table class="noborder" width="100%">';
print '<tr class="liste_titre">';
print '<td>'.$langs->trans("Name").'</td>';
print '<td>'.$langs->trans("Description").'</td>';
print '<td align="center" width="60">'.$langs->trans("value").'</td>';
print '<td align="center" width="60">'.$langs->trans("Modificar").'</td>';
print "</tr>\n";
print '<tr >';
print '<td>TC</td>';
print '<td>Valor actual del tipo de cambio (TC)</td>';
print '<td align="center" width="60"><input type="text" name="newTC" value="'.$valorTC.'" /></td>';
print '<td align="center" width="60"><input type="submit" value="Actualizar" /></td>';
print "</tr>\n";
print '</table>';
print '</form>';
 ...
 */
print '<br>';
print_titre($langs->trans("Almacenes"));

	$sql = "SELECT name, value, rowid from
				".MAIN_DB_PREFIX."const
				where name like 'FACT_ALM_%' 
...
//				echo $sql;
	...
	if($n>0){		
		while ($obj= $db->fetch_object($resql)) {
			$resp=substr($obj->name, 9, 2);			
			if(strcmp($resp, 'MP')==0){
				$sql2='SELECT label from llx_entrepot where rowid='.$obj->value.';';
	...
	print '<table class="noborder" width="100%">';
		print '<tr class="liste_titre">';
			...
			print '<td align="center">'.$langs->trans("Almacenes").'</td>';
			print '<td align="center">'.$langs->trans("Modificar").'</td>';
		print "</tr>\n";
		print '<tr >';
			print '<td>ALMACENMP</td>';
			print '<td>Almacen de Materias Primas</td>';
			print '<td><b>'.$alm_mp.'</b></td>';
			print '<form method="post" action ="'.$_SERVER['PHP_SELF'].'?action=updateAlm&type=MP&val='.$alm_mp.'" >';
				print '<td align="center">';
					print '<select name="almMP">';
					...
					print '</select>';
				print '</td>';
				print '<td align="center"><input type="submit" value="Actualizar" /></td>';
			print '</form>';				
		print "</tr>\n";
		print '<tr >';
			print '<td>ALMACENHI</td>';
			print '<td>Almacen Herramientas e Insumos</td>';
			print '<td><b>'.$alm_hi.'</b></td>';
			print '<form method="post" action ="'.$_SERVER['PHP_SELF'].'?action=updateAlm&type=HI&val='.$alm_hi.'" >';
				print '<td align="center">';
					print '<select name="almHI">';
						print '<option>...Seleccione</option>';
						foreach ($list as $obj) {
							print '<option value="'.$obj->rowid.'">'.$obj->label.'</option>';
						}
		...
		print "</tr>\n";
		print '<tr >';
			print '<td>ALMACENPT</td>';
			print '<td>	Almacen Productos Terminados</td>';
			print '<td><b>'.$alm_pt.'</b></td>';
			print '<form method="post" action ="'.$_SERVER['PHP_SELF'].'?action=updateAlm&type=PT&val='.$alm_pt.'" >';
				print '<td align="center">';
					print '<select name="almPT">';
						print '<option>...Seleccione</option>';
						foreach ($list as $obj) {
							print '<option value="'.$obj->rowid.'">'.$obj->label.'</option>';
						}
