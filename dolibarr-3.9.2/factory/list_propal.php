...
$id_line2=GETPOST("id_line2");
$id_line3=GETPOST("id_line3");

// Nombre de ligne pour choix de produit/service predefinis
$NBLINES=4;
$action=GETPOST("action");
$id_prod=GETPOST("id_prod");
$id_pro=GETPOST("id_pro");
...
if ($action=="confirm_deleteline" && $confirm == 'yes') {
	$objectstatic3=new Propal($db);
	$objectstatic3->fetch($id_pro);
	$result = $objectstatic3->deleteline($id_line2);

	setEventMessages( "Registro eliminado satisfactoriamente" , "");
	header('Location: ' . $_SERVER["PHP_SELF"] . '?id=' . $object->id);
	exit();
}

if ($action=="add_line_prod") {
	$object_line=new Propal($db);
	$object_line->fetch($id_line3);
	$producto2 = new Product($db);

	$producto2->fetch($_POST["select_clave"]);
	$cant=GETPOST("qty","int");
	if ($cant>0) {
		$iva=GETPOST("tva","int");
		$price=$producto2->price*$cant;


		$res=$object_line->addline(
			"", 
			$producto2->price, 
			$cant, 
			$iva, 
		...
		if ($res) {
			setEventMessages("Producto agregado satisfactoriamente", "");
		}else{
			setEventMessages("Error al agregar producto","", 'errors');
		}
	}else{
		setEventMessages("Inserte una cantidad valida","", 'errors');
	}
}
...
if ($id_line>0) {
	$sql.= ' LEFT JOIN llx_propaldet as v on v.fk_propal=p.rowid';
}
...
if ($id_line>0) {
	$sql.= ' AND v.fk_product='.$id_line." GROUP BY p.rowid";
}
...
if ($result)
{
    ...
    if ($action=="del_line") {
		$objectstatic2=new Propal($db);
		$objectstatic2->fetch($id_pro);
		$prod_prueba2= new Product($db);
		$prod_prueba2->fetch($id_prod);
		$formconfirm = $form->formconfirm($_SERVER["PHP_SELF"] . '?id_prod=' . $id_prod . '&id_pro=' . $id_pro. '&id_line2=' . $id_line2, $langs->trans('DeleteProductLine'), $langs->trans('ConfirmDeleteProductLine')."<br><br>Presupuesto: ".$objectstatic2->getNomUrl(1)."<br>Producto: ".$prod_prueba2->getNomUrl(1), 'confirm_deleteline', '', 0, 1,180);
		print $formconfirm;
	}
	...
	$moreforfilter.='<div class="divsearchfield">';
	$moreforfilter.='Producto: ';
	$moreforfilter.=select_dol_products(GETPOST("id_line"), 'id_line',1);
	$moreforfilter.='</div>';
	...
	while ($i < min($num,$limit))
	{
		...
		print '<tr '.$bc[$var].'>';
			...
			print '<td align="center" class="input_label" style="cursor:pointer;" var="'.$i.'"><img class="imgalign" id="img2-'.$i.'" src="img/2down.png" rot="1"></td>';
		...
		$sql2='
			...
			FROM
				llx_propaldet AS a
			INNER JOIN llx_product as b on a.fk_product=b.rowid
			WHERE
				a.fk_propal ='.$objp->propalid;

		$result2=$db->query($sql2);
		print '<tr id="fade2-'.$i.'" class="fader" style="display: none;">';
		print '<td colspan=10 align="right" style="padding: 0px 0px 5px 0px;">';
			print '<table  width="100%" style="border:1px solid rgba(0,0,0,0.5);">';
				print '
					<tr class="liste_titre nodrag nodrop">
						<td class="linecoldescription">Descripción</td>
						<td class="linecolvat" align="right" width="50">IVA</td>
						<td class="linecoluht" align="right" width="80">P.U.</td>
						<td class="linecolqty" align="right" width="50">Cant.</td>
						<td class="linecolht" align="right" width="50">Importe</td>
						<td class="linecolht" align="right" width="50">Eliminar</td>
		...
		if ($result2)
		{
			while ($obj = $db->fetch_object($result2)) {
				...
				$prod_prueba->fetch($obj->producto);
				print '<tr class="impair drag drop">';
					print '<td class=" nowrap">';
						echo $prod_prueba->getNomUrl(1);
					print '</td>';
					print '<td class=" nowrap" align="right">';
						echo number_format($obj->tva_tx,0)."%";
					print '</td>';
					print '<td class=" nowrap" align="right">';
						echo price($obj->subprice);
					print '</td>';
					print '<td class=" nowrap" align="right">';
						echo number_format($obj->qty,0);
					print '</td>';
					print '<td class=" nowrap" align="right">';
						echo price($obj->total_ttc);
					print '</td>';
					print '<td class=" nowrap" align="center">';
						if ($objp->fk_statut==0) {
							echo '<a href="'.$_SERVER['PHP_SELF'].'?action=del_line&id_prod='.$obj->producto.'&id_pro='.$objp->propalid.'&id_line2='.$obj->rowid.'">
								'.img_picto("Eliminar","delete").'
							</a>';
						}else{
							echo img_picto("Eliminar","delete","style='opacity: 0.6;'");
						}
		...
		if ($objp->fk_statut==0) {	
				print '<tr class=" pair">';
					print '<form action="'.$_SERVER['PHP_SELF'].'?id_line3='.$objp->propalid.'&action=add_line_prod" method="POST">';
		 

					print '<td colspan=2 ><b>Inserte la clave especifica del proyecto</b></td>';
					print '<td>';
					$sql='SELECT
						SUBSTR(a.ref, 1, 8) as clave,a.rowid
					FROM
						llx_product AS a
					...
					<select class="flat" id="select_clave" name="select_clave">';
						while ($obj = $db->fetch_object($query)) {
							print '<option value="'.$obj->rowid.'">'.$obj->clave.'</option>';
						}
					print '
					</select>';
					print '</td>
					<td align="center">'.
						$form->load_tva("tva", "16", $mysoc, '').'
					</td>
					<td align="center">
						<input type="text" size="2" name="qty" id="qty">
					</td>
					<td >
