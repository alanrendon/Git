...
if($action=='addtarealine'){
	...
	$tareahra=GETPOST('tareahra');
	$tareamin=GETPOST('tareamin');
	if(is_numeric($tareahra)){
	}else{
		$tareahra=0;
	}
	if(is_numeric($tareamin)){
	}else{
		$tareamin=0;
	}
	$sql="INSERT INTO ".MAIN_DB_PREFIX."factory_groupdet (fk_groupt,etiqueta,horas,minutos) 
			VALUES (".$idr.",'".$db->escape($tareaname)."',".$tareahra.",".$tareamin.")";
	$rs=$db->query($sql);
	print "<script>window.location.href='grupotareas.php'</script>";
}
if($action=='addsubtarealine'){
	$idr=GETPOST('idr');
	$idt=GETPOST('idt');
	$tareaname=GETPOST('tareaname');
	$tareahra=GETPOST('tareahra');
	...
	$sql="INSERT INTO ".MAIN_DB_PREFIX."factory_groupdet (fk_groupt,etiqueta,fk_tareapadre,horas,minutos) 
			VALUES (".$idr.",'".$db->escape($tareaname)."',".$idt.",".$tareahra.",".$tareamin.")";
	$rs=$db->query($sql);
	print "<script>window.location.href='grupotareas.php'</script>";
}
	...
	if($nr>0){
		while($rq=$db->fetch_object($rs)){
			print "<tr class='pair'>";
				...
				print "</td>";
			print "</tr>";
			$sql2="SELECT  rowid,fk_groupt,etiqueta,fk_tareapadre,horas,minutos FROM ".MAIN_DB_PREFIX."factory_groupdet WHERE fk_groupt=".$rq->rowid;
			$rs2=$db->query($sql2);
			$nr2=$db->num_rows($rs2);
			if($nr2>0){
				while ($rq2=$db->fetch_object($rs2)){
					print "<tr>";
						...
						print "<td>";
							print "Duracion: ";
							print $rq2->horas.":".$rq2->minutos;
						print "</td>";
						print "<td>";
							print "<a href='grupotareas.php?action=deltarea&idr=".$rq->rowid."&idt=".$rq2->rowid."'>".img_delete()."</a>";
						print "</td>";
						print "<td width='20%'>";
							print "<form method='POST' action='grupotareas.php?action=addsubtarea&idr=".$rq->rowid."&idt=".$rq2->rowid."'><input type='submit' value='Agregar Subtarea'></form>";
						print "</td>";
					print "</tr>";
					if($action=='addsubtarea' && GETPOST('idt')==$rq2->rowid){
						print "<tr>";
						print "<td colspan='4' align='center'><form action='grupotareas.php?action=addsubtarealine&idr=".$rq->rowid."&idt=".$rq2->rowid."' method='POST'>";
						print "Etiqueta Subtarea: ";
						print " <input type='text' name='tareaname' size='50' required>";
						print " Hrs: <input type='text' name='tareahra' size='2' required>";
						print " Min: <input type='text' name='tareamin' size='2' required>";
						print " <input type='submit' value='Guardar'>";
						print "</form></td>";
						print "</tr>";
					}
