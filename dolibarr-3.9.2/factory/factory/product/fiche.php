...
if ($id || $ref)
{
	if ($result)
	{
		...
		if ($action == 'build' || $action == 'createof')
		{
			...
			print '<form action="'.DOL_URL_ROOT.'/factory/product/fiche.php?id='.$id.'" method="post">';
			...
			print '</td></tr>';
			print '<tr>';
			print '<td align=center><input type="submit" class="button" name="verifyof" value="'.$langs->trans("VerifyQty").'"></td>';
			if ($action=='createof' && GETPOST("nbToBuild") > 0)
				print '<td  align=center><input type="submit" class="button" name="createofrun" value="'.$langs->trans("LaunchOF").'"></td>';
