...
if ($action == 'generateref')
{
	...
	$sql = 'SELECT distinct p.rowid, p.ref, p.label, p.price, p.price_ttc, p.price_base_type, p.fk_product_type, p.tva_tx,';
	$sql.= ' fcc.price_level, fcc.init_price, fcc.computemode, fcc.computevalue, fcc.multiplyby';
	...
	if ($resql)
	{
		...
		while ($i < $num)
		{
			...
			if ($resqlcomp)
			{
				...
				// Add new prices
				$now=dol_now();
				$sql = "INSERT INTO ".MAIN_DB_PREFIX."product_price(price_level, date_price, fk_product, fk_user_author, price, price_ttc, price_base_type, tosell, tva_tx, ";
				$sql.= " recuperableonly, localtax1_tx, localtax2_tx, price_min, price_min_ttc, price_by_qty, entity) ";
				$sql.= " VALUES(".$obj->price_level .", '".$db->idate($now)."', ".$obj->rowid.", ".$user->id.", ".$price.", ".$price_ttc.", '".$obj->price_base_type."', 1, ".$obj->tva_tx.", ";
 ...
 */
	...
	print '<input type="submit" id="launch_generate" name="launch_generate" value="'.$langs->trans("LaunchChangePrice").'" class="button" />';
	print '</div>';
