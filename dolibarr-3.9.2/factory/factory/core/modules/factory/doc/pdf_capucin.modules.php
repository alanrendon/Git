...
class pdf_capucin extends ModeleFactory
{
	...
	function __construct($db)
	{
		...
		$this->marge_haute=10;
		$this->marge_basse=10;
		$this->posqtyplanned=120;
		$this->posqtyused=150;
		$this->posqtydeleted=180;
		...
		if ($conf->factory->dir_output)
		{
			...
			if (file_exists($dir))
			{
				...
				for ($i = 0; $i < $nblines; $i++)
				{
					...
					$pdf->writeHTMLCell(0, 3, $this->marge_gauche +5, $curY, $txt, 0, 1, 0);
					
					$pdf->SetXY($this->posqtyplanned -1, $curY);
					$pdf->MultiCell(20,8,price($objectligne['qtyplanned']),0,'R',0);

					$pdf->SetXY($this->posqtyused -1 , $curY);
					$pdf->MultiCell(20,8,price($objectligne['qtyused']),0,'R',0);

					$pdf->SetXY($this->posqtydeleted -1, $curY);
					$pdf->MultiCell(20,8,price($objectligne['qtydeleted']),0,'R',0);
				...
				$pdf->writeHTMLCell(0, 3, $this->marge_gauche+5, $tab_top+$tab_height-5,$outputlangs->transnoentities("NbOfComponent")." : ".$nblines, 0, 1, 0,true,"L");	
	...
	function _tableau(&$pdf, $tab_top, $tab_height, $nexY, $outputlangs)
	{
		...
		$pdf->SetXY($this->marge_gauche+5, $tab_top+1);
		$pdf->MultiCell(50,8,$outputlangs->transnoentities("ListOfComponents"),0,'L',0);
		$pdf->SetXY($this->posqtyplanned, $tab_top+1);
		$pdf->MultiCell(20,8,$outputlangs->transnoentities("QtyNeed"),0,'R',0);
		$pdf->SetXY($this->posqtyused, $tab_top+1);
		$pdf->MultiCell(20,8,$outputlangs->transnoentities("QtyConsummedOF"),0,'R',0);
		$pdf->SetXY($this->posqtydeleted, $tab_top+1);
		$pdf->MultiCell(20,8,$outputlangs->transnoentities("QtyLosedOF"),0,'R',0);
		...
		$pdf->line($this->marge_gauche+4, $tab_top, $this->marge_gauche+4, $tab_top + $tab_height);		
	...
	function _pagehead(&$pdf, $object, $showaddress, $outputlangs)
	{
		...
		{
			pdf_watermark($pdf,$outputlangs,$this->page_hauteur,$this->page_largeur,'mm',$conf->global->FACTORY_DRAFT_WATERMARK);
		}
		...
		$pdf->SetXY($posx,$posy);
		$pdf->SetFont('','', $default_font_size);
		$datestart = dol_print_date($object->date_start_planned,"day",false,$outputlangs,true) . ($object->date_start_made ? " / " . dol_print_date($object->date_start_made,"day",false,$outputlangs,true):'');
		$pdf->MultiCell(120, 3, $outputlangs->transnoentities("DateStartPlannedMade")." : " . $datestart, '', 'L');
		$posy+=7;
		$pdf->SetXY($posx,$posy);
		$dateend = dol_print_date($object->date_end_planned,"day",false,$outputlangs,true) . ($object->date_end_made?" / " . dol_print_date($object->date_end_made,"day",false,$outputlangs,true):'');
		...
		$durationFactory= convertSecondToTime($object->duration_planned, 'allhourmin'). ($object->duration_made ? " / " . convertSecondToTime($object->duration_made, 'allhourmin') : ''); 
		$pdf->MultiCell(100, 3, $outputlangs->transnoentities("DurationPlannedMade")." : ".$durationFactory, '', 'L');
