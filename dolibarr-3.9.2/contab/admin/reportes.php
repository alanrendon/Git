...
$h2 = 0;
$head2[$h2][0]="reportes.php?mod=1"; // Url de la page affichée quand on clique sur l'onglet
$head2[$h2][1]="Agrupacion del Balance General"; // Titre de l'ongLet
$head2[$h2][2]="Agrupacion del Balance General";
$h2++;
$head2[$h2][0]="reportes.php?mod=2"; // Url de la page affichée quand on clique sur l'onglet
$head2[$h2][1]="Agrupacion del Estado de Resultados"; // Titre de l'ongLet
...
dol_fiche_head($head2, $mod-1, 'Reportes', 0, '');


if($mod==1){
	if($user->rights->contab->ccreportes){
		include "config_group_bg.php";
	}else{
	...
	if($user->rights->contab->ccreportes){
		include "config_group_er.php";
	}else{
		print '<div class="error">Acceso denegado.<br>Intenta acceder a una página, área o funcionalidad de un módulo desactivado o sin una sesión auntenticada o no permitida a su usuario</div>';
	}
}elseif ($mod==3 || $mod==4) {
	include "config_group_active.php";
}