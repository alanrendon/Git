...
class cuentas_registradas extends conexion {
	...
	public function get_grupos() {
         $grupo = array();
		$result = $this->db->query("SELECT * FROM ".PREFIX."contab_grupos WHERE tipo_edo_financiero = 2");
         if($result){
...
foreach ( $arreglo as $key => $value ) {
    ...
    if(isset($grupo_padre) && isset($value[0])  && isset($b[$value[2]]) && isset($c[$value[3]])){
            ...
            $res .= '<li><a href="edit.php?id='.$key.'&tipo=2" class="edit-link" title="Editar"><i class="fa fa-file-text" style="color: #FFC300"></i></a></li>';
