...
class productos_cuentas extends conexion {
    ...
    $cuenta = array();
		$result = $this->db->query("SELECT * FROM ".PREFIX."contab_cuentas_rel WHERE fk_type = 3");
	...
	public function get_producto($rowid) {
		$result = $this->db->query("SELECT ref,label FROM ".PREFIX."product WHERE rowid = ".$rowid);
		$row = mysqli_fetch_array($result, MYSQLI_ASSOC);
		return $row['ref'].' - '.$row['label'];
	}
