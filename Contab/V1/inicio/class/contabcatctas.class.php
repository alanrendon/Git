...
class Contabcatctas //extends CommonObject
{
    ...
    {	
    	...
    	$sql.= " FROM ".MAIN_DB_PREFIX."contab_sat_ctas c, ";
    	$sql.= " ".MAIN_DB_PREFIX."contab_polizas p, ";
    	$sql.= " ".MAIN_DB_PREFIX."contab_polizasdet pd ";
    	if ($anio > 0 && $mes > 0) {
			$sql.= " WHERE p.mes = ".$mes." AND p.anio = ".$anio." AND ";
    	} else {
    		$sql .= " WHERE 1 AND ";
    	}
    ...
    function fetch2($id)
    {
	    ...
	    $sql.= " FROM ".MAIN_DB_PREFIX."contab_sat_ctas as t ";
	    $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."contab_cat_ctas as s ";
	    $sql.= " ON t.fk_sat_cta = s.rowid ";
	    $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."contab_sat_ctas d ";
	    	...
	    	/* $sql .= " WHERE
						CASE WHEN Locate('.', '$cta', Locate('.', '$cta', Locate('.', '$cta') + 1) + 1) = 0 THEN 
							...
							ELSE 
								...
								END
						END"; */
    		$sql .= " WHERE INSTR(t.cta, '$cta') > 0 OR LEFT('$cta', LENGTH(t.cta)) = t.cta AND t.entity = ".$conf->entity." ORDER BY t.cta DESC LIMIT 1 ";
    	}
    ...
    function fetch_by_CodAgr($codagr)
    {
    	...
    	$sql = "SELECT * ";
    	$sql.= " FROM ".MAIN_DB_PREFIX."contab_sat_ctas c ";
    	$sql.= " INNER JOIN ".MAIN_DB_PREFIX."contab_cat_ctas s ";
    ...
    {
    	global $langs,$conf;
    	$sql = "SELECT c.rowid, c.cta, c.descta, c.fk_sat_cta, c.subctade, s.rowid as s_rowid ";
    	$sql.= " FROM ".MAIN_DB_PREFIX."contab_sat_ctas c ";
    	$sql .= " INNER JOIN ".MAIN_DB_PREFIX."contab_cat_ctas s ";
    	$sql .= " ON c.fk_sat_cta = s.rowid ";
    ...
    function fetch_array_by_dependede($sctade=0, $a=array()) {
    	...
    	global $conf;
    	$sql = "Select rowid From ".MAIN_DB_PREFIX."contab_sat_ctas c ";
...
//     				$this->subctade = $obj->subctade;
	...
	function reindexar() {
		...
		$resql = $this->db->query("SELECT * FROM ".MAIN_DB_PREFIX."contab_sat_ctas ORDER BY entity, rowid");
		if($resql){
			...
			$this->db->query("INSERT INTO ".MAIN_DB_PREFIX."contab_sat_ctas_tmp (entity, cta, descta, fk_sat_cta, subctade, import_key) SELECT entity, cta, descta, fk_sat_cta, subctade, '' FROM ".MAIN_DB_PREFIX."contab_sat_ctas ORDER BY entity, rowid");
			...
			$this->db->query("INSERT INTO ".MAIN_DB_PREFIX."contab_sat_ctas (entity, cta, descta, fk_sat_cta, subctade, import_key) SELECT entity, cta, descta, fk_sat_cta, subctade, '' FROM ".MAIN_DB_PREFIX."contab_sat_ctas_tmp ORDER BY entity, rowid");
	...
	function update_depende_de() {
		$sql = "SELECT min(rowid) - 1 as minid, entity FROM ".MAIN_DB_PREFIX."contab_sat_ctas WHERE entity > 1 GROUP BY entity";
		if ($res = $this->db->query($sql)) {
			while ($obj = $this->db->fetch_object($res)) {
				$minid = $obj->minid;
				$ent = $obj->entity;
				$sql = "UPDATE ".MAIN_DB_PREFIX."contab_sat_ctas SET subctade = subctade + ".$minid." WHERE subctade > 0 AND entity = ".$ent;
