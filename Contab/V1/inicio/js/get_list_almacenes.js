(function($){
  	...
  	$.get( "../get/get_almacenes.php", function( data ) {
        $("#almacen").html(data);
    ...
    $( "#guardar_almacen" ).on( "click", function(e) {
		e.preventDefault();
		$("#mensaje").html('');
		var form = $('#form_almacen').serialize();
		console.log(form);
		$.ajax( {
			type: "POST",
			url: '../get/../get/carga_cuenta_rel.php',
			data: form,
			dataType: "Json",
	...
	$.get( "../get/get_almacen_cuentas.php", function( data ) {
		$( "#listado_almacen" ).html(data).fadeIn('slow');
		$( '#datatable' ).DataTable({
            "aoColumnDefs": [
	...
	$( document ).on( "click", ".delete-link", function() {
		...
		if (confirm("¿Está seguro de eliminar la relación de este almacén?")) {
			$.ajax({
				type: "POST",
