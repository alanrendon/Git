(function($){
  	...
  	$.get( "../get/get_servicios.php", function( data ) {
        $("#servcios").html(data);
    ...
    $( "#guardar_servicio" ).on( "click", function(e) {
		e.preventDefault();
		$("#mensaje").html('');
		var form = $('#form_servicio').serialize();
		console.log(form);
		$.ajax( {
			type: "POST",
	...
	$.get( "../get/get_servicios_cuentas.php", function( data ) {
		$( "#listado_servicios" ).html(data).fadeIn('slow');
		$( '#datatable' ).DataTable({
            "aoColumnDefs": [
	...
	$( document ).on( "click", ".delete-link", function() {
		...
		if (confirm("¿Está seguro de eliminar la relación de este servicio?")) {
			$.ajax({
				type: "POST",
