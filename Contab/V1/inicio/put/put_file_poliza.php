...
if (isset($_REQUEST['action']) && $_REQUEST['action'] == 'carga') {
    ...
    if ($id_poliza['tipol_l']=='E' && isset($_POST['id_typeoperation']) && isset($_POST['operation_societe']) && $tipo_compare == 'xml') {
    ...
    if (isset($_POST['id_typeoperation']) && $_POST['id_typeoperation']==2) {

        if (!isset($_POST['rfc_ext']) && !isset($_POST['id_ext']) && !isset($_POST['nombre_ext'])) {
            exit( json_encode( array( 'mensaje'=>'Hacen falta los datos del extranjero.')));
        }else{

            $rfc         = addslashes(trim($_POST['rfc_ext']));
            $id_ext      = addslashes(trim($_POST['id_ext']));
            $nombre_ext  = addslashes(trim($_POST['nombre_ext']));
            $newfilename ='';
            $crg         = $polizas->insert_info_docto($_REQUEST['idp'], '', '', $tipo,'',$operacion,$rfc, $id_ext, $nombre_ext);

            exit( json_encode( array( 'mensaje'=>'El archivo se cargo exitosamente.')));     
        }
         
    }else if ($continue && move_uploaded_file($_FILES['docto']['tmp_name'], $fichero_subido)) {
        ...
        $crg = $polizas->insert_info_docto($_REQUEST['idp'], $carpeta, $newfilename, $tipo,$timbreFDUuid,$operacion,$rfc, $id_ext, $nombre_ext);
