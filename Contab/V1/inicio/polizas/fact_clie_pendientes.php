            ...
            <div id="dv_tabla">
                <form id="frm_AsignarPoliza" data-parsley-validate class="form-horizontal form-label-left">
                    ...
                    <table class="table table-striped jambo_table bulk_action" id="datatable">
                        ...
                        <tbody>
                            <?php   $a_factures = $facpen->getFacPolizasPend(); ?>
                            <?php foreach ($a_factures as $key => $a_facture): ?>
                                    <?php $a_polrel = $facpen->pol_relacionadas($a_facture['rowid']); ?>
                                    <?php $a_facture['dateo'] = (isset($a_facture['dateo']) && $a_facture['dateo'] != null) ? $a_facture['dateo'] : 'N/A'; ?>
                                    <tr>
                                        <td>
                                            <?php echo $a_facture['facnumber'] ?>
...
function getFacturaInfo() {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $("#txt_concepto").val('' + $("#td_facnumber").val());
...
function f_get_partidas_facture(facturasid){
    ...
    $.ajax({
        url: "../get/get_asientos_facture.php",
        type: 'POST',
        data: {idFacture: facturasid, tipo:1, societe: valor},
        dataType: 'html',
        async: false,
        success: function(data) {
            ...
            $('#iva_debe_pendiente').val($('#tva').val());
...
function AgregarPoliza() {
    ...
    if (confirm("¿Está seguro de agregar la póliza?")) {
        $.ajax({
            ...
            type: 'POST',
            data: $("#frm_AltaPoliza").serialize() + '&tipoSociedad=1',
            dataType: 'json',
            success: function(data) {
                if (data.mensaje) {
                    alert(data.mensaje);
...
function ignorar_facturas() {
    if (confirm("¿Está seguro ingorar las facturas seleccionadas?")) {
        $.ajax({
            ...
            type: 'POST',
            data: $("#frm_AsignarPoliza").serialize() + '&tipo=1',
            dataType: 'json',
            success: function(data) {
                if (data.mensaje) {
                    alert(data.mensaje);
