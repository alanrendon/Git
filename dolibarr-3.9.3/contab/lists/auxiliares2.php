...
<table class="noborder" style="width: 100%">
		<tr class="liste_titre">
          ...
          <input type="hidden" name="cuenta2" value="<?=$cuenta2?>">
          <input type="checkbox" name="chk_todas_2" <?=$ck2?> onchange="this.form.submit()">Solo cuentas con saldo 
		...
		<tr class="liste_titre">
			...
			<td style="width: 10%;text-align: right;">Saldo Final</td>
		</tr>
<?php 
$mm = sprintf("%02d", $mes);
...
while($rs=$db->fetch_object($rq)){
  ...
  FROM ".MAIN_DB_PREFIX."contab_cat_ctas as d 
  LEFT JOIN (SELECT b.cuenta,sum(b.debe) as debe,sum(b.haber) as haber FROM ".MAIN_DB_PREFIX."contab_polizas as a INNER JOIN
  			".MAIN_DB_PREFIX."contab_polizasdet as b on a.rowid=b.fk_poliza WHERE (CONCAT(anio,LPAD(mes,2,'0'))<=CONCAT('$anio','$mm') AND CONCAT(anio,LPAD(mes,2,'0'))>=CONCAT('$anio1','$mm1')) 
  			AND entity=".$conf->entity."  GROUP BY b.cuenta) c ON d.cta=c.cuenta 
  ...
  // 	$mini+=$minicial;
  	...
  	$sumsalini+=$saldini;
  	$mactua+=$mact;
  if ($rs->debe==0 AND $rs->haber==0 AND $saldini==0 AND $mact==0 AND $todos2=="SI") {
