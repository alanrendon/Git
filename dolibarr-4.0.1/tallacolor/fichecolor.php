...
if (empty($reshook))
{
	...
	// Update a product or service
    if ($action == 'updatecolor' && ($user->rights->produit->creer || $user->rights->service->creer))
    {
    	/*if (GETPOST('cancel'))
        {
            $action = '';
        ...
        else
        {
            if ($object->fetch($id,$ref))
            {
				...
				$object->ref                = $id;
                $object->libelle            = GETPOST('libelle');
				$object->updateColor($object->id, $user);
    ...
    if ($action == 'confirm_delete' && $confirm == 'yes')
    {
        ...
        {
            $result = $object->deleteColor($id);
        }

        if ($result > 0)
        {
            header('Location: '.DOL_URL_ROOT.'/tallacolor/listecolor.php?delColor='.urlencode($id));
            exit;
        }
        else
        {
...
else
{
	...
	//print $id.'---'.$ref."---".$action;
        ...
        if ($action == 'edittalla' && ($user->rights->produit->creer || $user->rights->service->creer))
        {
            ...
            // Label
            print '<tr><td class="fieldrequired">Talla: </td><td colspan="2"><input name="libelle" size="40" value="'.$object->muestraColor($user,$id).'"></td></tr>';
/*
            // Status
...
*/
        ...
        else
        {
           ...
           // print $form->showrefnav($object,'ref','',1,'ref');
            ...
            // Label
            print '<tr><td>'.$langs->trans("Label").'</td><td colspan="2">'.$object->muestraColor($user,$id).'</td>';
/*
            $nblignes=8;
...
*/
    ...
    {
        header("Location: listecolor.php");
        exit;
    }
}
...
if ($action == 'deletecolor' && empty($conf->use_javascript_ajax))
{
    print $form->formconfirm("fichetalla.php?id=".$object->id,"Eliminar color","Eliminar Color","confirm_delete",'',0,"action-delete");
}
...
if ($action == '' || $action == 'view')
{
	...
	//print $id;
    $object_is_used = $object->isColorUsed($id);
    ...
    || ($object->type == 1 && $user->rights->service->supprimer))
    {
        if (empty($object_is_used) && (! isset($object->no_button_delete) || $object->no_button_delete <> 1))
        {
            if (! empty($conf->use_javascript_ajax))
            {
                ...
                print $form->formconfirm("fichecolor.php?id=".$id,$langs->trans("DeleteProduct"),$langs->trans("ConfirmDeleteProduct"),"confirm_delete",'',0,"action-delete");
            }
            else
            {
                print '<a class="butActionDelete" href="'.$_SERVER["PHP_SELF"].'?action=deletecolor&amp;id='.$id.'">'.$langs->trans("Delete").'</a>';
            }
        }
        else
        {
