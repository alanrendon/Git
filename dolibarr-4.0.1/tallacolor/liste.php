...
else
{
    ...
    $sql = 'SELECT DISTINCT p.rowid, p.ref, p.label, p.llx_product, llx_talla, llx_color, p.barcode, p.price, p.price_ttc, p.price_base_type,';
    $sql.= ' p.fk_product_type, p.tms as datem,';
    $sql.= ' p.duration, p.tosell, p.tobuy, p.seuil_stock_alerte,';
    $sql.= ' MIN(pfp.unitprice) as minsellprice';
    //Talla Color
    $sql.= ' ,p.llx_product, p.llx_talla, p.llx_color';
...
// multilang
    ...
    $sql.= " GROUP BY p.rowid, p.ref, p.label, p.llx_product, p.llx_talla, p.llx_color, p.barcode, p.price, p.price_ttc, p.price_base_type,";
    ...
    if ($resql)
    {
    	...
    	else
    	{
    		...
    		// Talla Color
    		print_liste_field_titre($langs->trans("Talla"), $_SERVER["PHP_SELF"], "p.llx_talla",$param,"","",$sortfield,$sortorder);
    		print_liste_field_titre($langs->trans("Color"), $_SERVER["PHP_SELF"], "p.llx_color",$param,"","",$sortfield,$sortorder);    		    		
    		...
    		while ($i < min($num,$limit))
    		{
                ...
                // Talla Color
    			print '<td>'.dol_trunc($product_static->muestraTalla($objp->id,$objp->llx_talla),40).'</td>';
    			
    			print '<td>'.dol_trunc($product_static->muestraColor($objp->id ,$objp->llx_color),40).'</td>';
