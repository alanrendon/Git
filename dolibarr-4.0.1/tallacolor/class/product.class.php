 ...
 */
class ProductTC extends CommonObject
{
	...
	/**
	 *	Insert product into database ---> productos ya combinados con talla y color
	 *
	 *	@param	User	$user     		User making insert
	 *  @param	int		$notrigger		Disable triggers
	 *	@return int			     		Id of product/service if OK or number of error < 0
	 */
	function insertaColor($user){
				
		//insertar datos
			$sql = "INSERT INTO ".MAIN_DB_PREFIX."color (id,color)";
    				$sql.= " VALUES(";
    				$sql.= " '".$this->ref."'";
    				$sql.= ", '".$this->color."'";
    				$sql.= " )";
		...
		if ($result)
		{
			$this->error='Color agregado';
		}else{
			$this->error='Error al agregar color '.$sql;
		}
	}
	//
	function muestraTalla($user,$id){
				
		//insertar datos
			$sql = "SELECT * FROM ".MAIN_DB_PREFIX."talla WHERE";
    				$sql.= " id = '".$id."'";
		$result = $this->db->query($sql);
	...
	//
	function muestraColor($user,$id){
				
		//insertar datos
			$sql = "SELECT * FROM ".MAIN_DB_PREFIX."color WHERE";
    				$sql.= " id = '".$id."'";
		$result = $this->db->query($sql);
		while ($obj = $this->db->fetch_object($resql))
		{
	...
	function insertaTalla($user){
				
		//insertar datos
			$sql = "INSERT INTO ".MAIN_DB_PREFIX."talla (id,talla)";
    				$sql.= " VALUES(";
    				$sql.= " '".$this->ref."'";
    				$sql.= ", '".$this->talla."'";
    				$sql.= " )";
		...
		if ($result)
		{
			$this->error='Talla agregada';
		}else{
			$this->error='Error al agregar talla '.$sql;
		}
	}
	
	function createTallaColor($user,$notrigger=0)
	{
		global $conf, $langs;
	...
	if (is_array($array)){
		foreach ($array as &$valor) {
			...
			$sql.= " FROM ".MAIN_DB_PREFIX."product";
			$sql.= " WHERE entity IN (".getEntity('product', 1).")";
			$sql.= " AND ref = '" .$this->ref."-".$this->talla."-".$valor."'";
			...
			if ($result)
			{
				...
				if ($obj->nb == 0)
				{
					...
					$sql.= ", canvas";
					$sql.= ", finished";
					$sql.= ", llx_product";//---> Campo ref del producto padre
					$sql.= ", llx_talla";//---> Campo id talla
					$sql.= ", llx_color";//---> Campo id color
					$sql.= ") VALUES (";
					$sql.= $this->db->idate($now);
					$sql.= ", ".$conf->entity;
					$sql.= ", '".$this->db->escape($this->ref)."-".$this->talla."-".$valor."'";
					$sql.= ", ".(! empty($this->ref_ext)?"'".$this->db->escape($this->ref_ext)."'":"null");
					$sql.= ", ".price2num($price_min_ht); //->public 'price_min' => string '9.00000000' (length=10)
					$sql.= ", ".price2num($price_min_ttc); //->public 'price_min_ttc' => string '10.44000000' (length=11)
					$sql.= ", ".(! empty($this->libelle)?"'".$this->db->escape($this->libelle)."'":"null");
					$sql.= ", ".$user->id;
					$sql.= ", ".$this->type;
					$sql.= ", ".price2num($price_ht);//->public 'price' => string '10.00000000' (length=11)
					$sql.= ", ".price2num($price_ttc);//->public 'price_ttc' => string '11.60000000' (length=11)
					$sql.= ", '".$this->price_base_type."'";//->public 'price_base_type' => string 'HT' (length=2)
					$sql.= ", ".$this->tva_tx;
					$sql.= ", ".$this->status;
					$sql.= ", ".$this->status_buy;
					$sql.= ", '".$this->canvas."'";
					$sql.= ", ".$this->finished;
					$sql.= ", '".$this->ref."'"; // Referencia producto padre
					$sql.= ", '".$this->talla."'"; // Id talla
					$sql.= ", '".$valor."'"; // Id color
					$sql.= ")";
					...
					if ( $result )
					{
						...
						{
						$error++;
						$this->error='ErrorFailedToUpdateRecord'.$sql;
						}
						}
						else
						{
					...
					{
						$error++;
						$this->error=$this->db->lasterror()."Sql".$sql."usr: ".$this->type;
					}
				}
				else
				{
	 ...
	 */
	function updateTalla($id, $user, $notrigger=false, $action='update')
	{
		global $langs, $conf;
		...
		$sql = "UPDATE ".MAIN_DB_PREFIX."talla";
		$sql.= " SET talla = '".$this->libelle."'";
		$sql.= " WHERE id = '".$this->ref."'";
	//print $sql;
	...
	function updateColor($id, $user, $notrigger=false, $action='update')
	{
		global $langs, $conf;
		...
		$sql = "UPDATE ".MAIN_DB_PREFIX."color";
		$sql.= " SET color = '".$this->libelle."'";
		$sql.= " WHERE id = '".$this->ref."'";
	//print $sql;
		dol_syslog(get_class($this)."update sql=".$sql);
		$resql=$this->db->query($sql);
	...
	function isTallaUsed($id)
    {
        ...
        //{
            ...
            $nb=0;
            $sql = "SELECT COUNT(*) as nb from ".MAIN_DB_PREFIX."product";
            $sql.= " WHERE llx_talla = '".$id."'";
			//print $sql;
            $resql=$this->db->query($sql);
            if ($resql)
            {
	...
	function deleteTalla($id)
	{
		...
		{
			$objectisused = $this->isTallaUsed($id);
			if (empty($objectisused))
			{
                ...
                if (! $error)
                {
    				$sqlz = "DELETE FROM ".MAIN_DB_PREFIX."talla";
    				$sqlz.= " WHERE id = '".$id."'";
	...
	function deleteColor($id)
	{
		...
		if ($user->rights->produit->supprimer)
		{
			...
			if (empty($objectisused))
			{
                ...
                if (! $error)
                {
    				$sqlz = "DELETE FROM ".MAIN_DB_PREFIX."color";
    				$sqlz.= " WHERE id = '".$id."'";
                    dol_syslog(get_class($this).'::delete sql='.$sql, LOG_DEBUG);
    				$resultz = $this->db->query($sqlz);
       				if ( ! $resultz )
	...
	function fetch($id='',$ref='',$ref_ext='')
	{
		...
		$sql.= " llx_product, llx_talla, llx_color";		
		$sql.= " FROM ".MAIN_DB_PREFIX."product";
		...
		if ( $resql )
		{
			if ($this->db->num_rows($resql) > 0)
			{
				...
				$this->llx_product              = $obj->llx_product;
				$this->llx_talla                = $obj->llx_talla;
				$this->llx_color                = $obj->llx_color;								
	...
	function getNomUrl($withpicto=0,$option='',$maxlength=0)
	{
		...
		if ($option == 'supplier')
		{
			$lien = '<a href="'.DOL_URL_ROOT.'/tallacolor/fournisseurs.php?id='.$this->id.'">';
			$lienfin='</a>';
		}
        else if ($option == 'stock')
        {
            $lien = '<a href="'.DOL_URL_ROOT.'/tallacolor/stock/product.php?id='.$this->id.'">';
            $lienfin='</a>';
        }
        else if ($option == 'composition')
        {
			$lien = '<a href="'.DOL_URL_ROOT.'/tallacolor/composition/fiche.php?id='.$this->id.'">';
			$lienfin='</a>';
        }
        else
		{
			$lien = '<a href="'.DOL_URL_ROOT.'/tallacolor/fiche.php?id='.$this->id.'">';
			$lienfin='</a>';
		}
		$newref=$this->ref;
	...
	//---------> URL Colores
function getNomUrlColor($withpicto=0,$option='',$maxlength=0)
	{
		global $langs;

		$result='';

		$lien = '<a href="'.DOL_URL_ROOT.'/tallacolor/fichecolor.php?id='.$this->id.'">';
		//$lien = '<a href="#">';
		$lienfin='</a>';
				$newref=$this->ref;
		if ($maxlength) $newref=dol_trunc($newref,$maxlength,'middle');
...
function getNomUrlTalla($withpicto=0,$option='',$maxlength=0)
	{
		global $langs;

		$result='';

		$lien = '<a href="'.DOL_URL_ROOT.'/tallacolor/fichetalla.php?id='.$this->id.'">';
		//$lien = '<a href="#">';
		$lienfin='</a>';
				$newref=$this->ref;
		if ($maxlength) $newref=dol_trunc($newref,$maxlength,'middle');
	...
	function load_stock()
	{
		...
		if(DOL_VERSION>='4'){
			$sql = "SELECT ps.reel, ps.fk_entrepot, gl.pmp";
			$sql.= " FROM ".MAIN_DB_PREFIX."product_stock as ps";
			$sql.= ", ".MAIN_DB_PREFIX."entrepot as w, ".MAIN_DB_PREFIX."product as gl";
			$sql.= " WHERE w.entity IN (".getEntity('warehouse', 1).")";
			$sql.= " AND w.rowid = ps.fk_entrepot AND ps.fk_product=gl.rowid";
...
//print $sql;
	...
	function is_photo_available($sdir)
	{
		...
		if(DOL_VERSION>="3.8"){
			$pdir=get_exdir($this->id, 2, 0, 0, $objProd, 'product'). $this->id ."/photos/";
		}else{
