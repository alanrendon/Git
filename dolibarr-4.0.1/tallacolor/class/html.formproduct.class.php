...
class FormProduct
{
	...
	//Imprime el combobox para talla ---->
	function select_talla($talla)
	{
		print $this->load_talla($talla);
	}
	//Imprime el checkbox para color ---->
	function select_color($talla, $prod)
	{
		print $this->load_color($talla, $prod);
	}
	//Imprime el producto con su color y talla generado ---->
	function select_prodTallaColor($talla, $prod)
	{
		print $this->load_prodTallaColor($talla, $prod);
	}
	/**
	...
	//Funcion que genera talla ---->
	function load_talla($talla)
	{
		$return='';

		$return.= '<select class="flat" name="talla" onchange="this.form.submit()" >';
		$return.= '<option value=""> -Seleccione Talla- </option>';
		$sql = 'SELECT * FROM '.MAIN_DB_PREFIX.'talla';
	...
	//Funcion que genera color ---->
	function load_color($talla, $prod)
	{
		$return='';
		if($talla != ''){
		//$sql = 'SELECT * FROM '.MAIN_DB_PREFIX.'color';
		$sql = " SELECT c.id id, c.color color FROM ".MAIN_DB_PREFIX."color c";
		//$sql .= " LEFT JOIN ".MAIN_DB_PREFIX."product p";
		//$sql .= " ON c.id = p.llx_color AND p.llx_talla = '".$talla."'";
		//$sql .= " AND p.llx_product='".$prod."'";
		//$sql .= " WHERE p.llx_product IS NULL";
		//print $sql;
		$resql = $this->db->query($sql);
		while ($obj = $this->db->fetch_object($resql))
		{
			$return.= ' <input type="checkbox" name="color[]" value="'.$obj->id.'">'.$obj->color;//.'<br>';
		}
		}
		return $return;//.$sql;
	}
	//Funcion que muestra productos talla color---->
	function load_prodTallaColor($talla, $prod)
	{
		$return='';
		//if($talla != ''){
		//$sql = "SELECT * FROM ".MAIN_DB_PREFIX."color";
		$sql = " SELECT p.label prod, c.color color, t.talla talla";
		$sql .= " FROM ".MAIN_DB_PREFIX."product p JOIN ".MAIN_DB_PREFIX."color c";
		$sql .= " ON p.llx_color = c.id";
		$sql .= " JOIN ".MAIN_DB_PREFIX."talla t ON p.llx_talla = t.id";
		$sql .= " WHERE p.llx_product = '".$prod."'";
		//echo $sql;break;
		
		$resql = $this->db->query($sql);
		while ($obj = $this->db->fetch_object($resql))
		{
			$return.= ' Producto: '.$obj->prod.' Talla: '.$obj->talla.' Color: '.$obj->color.'<br>';
		}
		return $return;
	}
}
