...
if (empty($reshook))
{
    ...
    // Add a product or service --------> Color
    if ($action == 'addcolor' && ($user->rights->produit->creer || $user->rights->service->creer))
    {
        $error=0;

        if (! GETPOST('color'))
        {
            $mesg='<div class="error">'.$langs->trans('ErrorFieldRequired',$langs->transnoentities('Color')).'</div>';
            $action = "createcolor";
            $error++;
        }
        if (empty($ref))
        {
            $mesg='<div class="error">'.$langs->trans('ErrorFieldRequired',$langs->transnoentities('Referencia')).'</div>';
            $action = "createcolor";
            $error++;
        }
        ...
        {
            $object->ref                = $ref;
			$object->color            = GETPOST('color');
			
            $id = $object->insertaColor($user);
	...
	// Add a product or service --------> Talla
    if ($action == 'addtalla' && ($user->rights->produit->creer || $user->rights->service->creer))
    {
        $error=0;

        if (! GETPOST('talla'))
        {
            $mesg='<div class="error">'.$langs->trans('ErrorFieldRequired',$langs->transnoentities('Talla')).'</div>';
            $action = "createtalla";
            $error++;
        }
        if (empty($ref))
        ...
        {
            $object->ref                = $ref;
			$object->talla            = GETPOST('talla');
			
            $id = $object->insertaTalla($user);
...
else
{
    ...
    // -----------------------------------------
    if ($action == 'createcolor' && ($user->rights->produit->creer || $user->rights->service->creer))
    {
        //WYSIWYG Editor
        ...
        print '<form action="tallacolor.php" method="post">';
        print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
        print '<input type="hidden" name="action" value="addcolor">';
       ...
       dol_htmloutput_mesg($mesg);
        ...
        $tmpcode='';
		/*if (! empty($modCodeProduct->code_auto))
			$tmpcode=$modCodeProduct->getNextValue($object,$type);*///'.$langs->trans("Ref").'
        print '<td class="fieldrequired" width="20%">Ref. Color</td><td><input name="ref" size="40" maxlength="32" value="">';
        if ($_error)
        {
        ...
        print '<tr><td class="fieldrequired">Color: </td><td><input name="color" size="40" value=""></td></tr>';
...
*/
  ...
  //      print '<tr><td>'.$langs->trans("CustomCode").'</td><td><input name="customcode" size="10" value="'.GETPOST('customcode').'"></td></tr>';
	...
	//Agregadooooo ------------>
	if ($action == 'createtalla' && ($user->rights->produit->creer || $user->rights->service->creer))
    {
        //WYSIWYG Editor
        require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
        ...
        print '<form action="tallacolor.php" method="post">';
        print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
        print '<input type="hidden" name="action" value="addtalla">';
        print '<input type="hidden" name="type" value="'.$type.'">'."\n";
		/*if (! empty($modCodeProduct->code_auto))*/
			print '<input type="hidden" name="code_auto" value="1">';
       ...
       dol_htmloutput_mesg($mesg);
		...
		/*if (! empty($modCodeProduct->code_auto))
			$tmpcode=$modCodeProduct->getNextValue($object,$type);*///'.$langs->trans("Ref").'
        print '<td class="fieldrequired" width="20%">Ref. Talla</td><td><input name="ref" size="40" maxlength="32" value="">';
        if ($_error)
        {
            print $langs->trans("RefAlreadyExists");
        }
        ...
        // Label '.$langs->trans("Label").'
        print '<tr><td class="fieldrequired">Talla: </td><td><input name="talla" size="40" value=""></td></tr>';
