...
require_once DOL_DOCUMENT_ROOT.'/tallacolor/lib/product.lib.php';
...
if (empty($reshook))
{
    ...
    if ($action == 'confirm_delete' && $confirm == 'yes')
    {
        ...
        if ($result > 0)
        {
            header('Location: '.DOL_URL_ROOT.'/tallacolor/liste.php?delprod='.urlencode($object->ref));
            exit;
        }
        else
        {
...
else
{
    ...
    else if ($id || $ref)
    {
        ...
        elseif ($action == 'editcolortalla' && ($user->rights->produit->creer || $user->rights->service->creer))
        {
            ...
            print '<input type="hidden" name="price_min" value="'.$object->price_min.'">';
            print '<input type="hidden" name="price_base_type" value="'.$object->price_base_type.'">';
            print '<input type="hidden" name="tva_tx" value="'.$object->tva_tx.'">';
			...
			//Color ---->
            print '<tr><td>Color: </td><td colspan="2">';
            ...
            else
            {
            	...
            	// PRIX
            	print '<tr><td>'.$langs->trans("SellingPrice").'</td>';
            	$object->price = $object->price*1;
            	if(($object->price*1) == 0){$object->price = '';}else{$object->price=$object->price*1;}
            	...
            	// MIN PRICE
            	print '<tr><td>'.$langs->trans("MinPrice").'</td>';
            	if(($object->price_min*1) == 0){$object->price_min = '';}else{$object->price_min=$object->price_min*1;}
            	print '<td><input name="price_min" size="10" value="'.$object->price_min.'">';
            	print '</td></tr>';
        ...
        else
        {
            ...
            //var_dump($object);
            //Talla
            print '<tr><td>'.$langs->trans("Talla").'</td><td colspan="2">'.$object->muestraTalla($object->id,$object->llx_talla).'</td>';
            //Color
            print '<tr><td>'.$langs->trans("Color").'</td><td colspan="2">'.$object->muestraColor($object->id ,$object->llx_color).'</td>';
            // Customs code
