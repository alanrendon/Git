...
// multilang
	...
	$sqlc = " SELECT id, talla FROM ".MAIN_DB_PREFIX."talla";
    dol_syslog("sql=".$sqlc);
    $resql = $db->query($sqlc);
    if ($resql)
    {
    	...
    	if (GETPOST('delTalla'))	dol_htmloutput_mesg("Talla eliminada",GETPOST('delTalla'));
    	...
    	else
    	{
    		...
    		print_liste_field_titre("Talla", $_SERVER["PHP_SELF"], "p.label",$param,"","",$sortfield,$sortorder);
    		...
    		while ($i < min($num,$limit))
    		{
    			...
    			$product_static->type = $objp->talla;
    			print $product_static->getNomUrlTalla(1,'',24);
    			print "</td>\n";

    			// Label
    			print '<td>'.dol_trunc($objp->talla,40).'</td>';
