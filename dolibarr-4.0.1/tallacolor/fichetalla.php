...
require_once DOL_DOCUMENT_ROOT.'/tallacolor/class/html.formproduct.class.php';
require_once DOL_DOCUMENT_ROOT.'/core/class/extrafields.class.php';
require_once DOL_DOCUMENT_ROOT.'/tallacolor/lib/talla.lib.php';
...
$object = new ProductTC($db);
//$objectX = $object->muestraColor($user);
...
if (empty($reshook))
{
	...
	// Add a product or service
    if ($action == 'addtallacolor' && ($user->rights->produit->creer || $user->rights->service->creer))
    {
        $error=0;
        ...
        if (! $error)
        {
            ...
            $id = $object->createTallaColor($user);

            if ($id > 0)
            {
		$mesg='<div class="error">'.$langs->trans($object->error).'</div>';
                header("Location: ".$_SERVER['PHP_SELF']."?id=".$id."&action=editcolortalla&talla=".$object->talla);
                exit;
            }
            else
            {
	...
	// Update a product or service
    if ($action == 'updatetalla' && ($user->rights->produit->creer || $user->rights->service->creer))
    {
    	/*if (GETPOST('cancel'))
        {
            $action = '';
        ...
        else
        {
            if ($object->fetch($id,$ref))
            {
				...
				$object->ref                = $id;
                $object->libelle            = GETPOST('libelle');
				$object->updateTalla($object->id, $user);
                    //if ($object->updateTalla($object->id, $user) > 0)
                    //{
                        $action = 'view';
						$mesg = $object->error;
                    /*}
    ...
    if ($action == 'confirm_delete' && $confirm == 'yes')
    {
        ...
        {
            $result = $object->deleteTalla($id);
        }

        if ($result > 0)
        {
            header('Location: '.DOL_URL_ROOT.'/tallacolor/listetalla.php?delTalla='.urlencode($id));
            exit;
        }
        else
        {
...
else
{
	...
	//print $id.'---'.$ref."---".$action;
        ...
        if ($action == 'edittalla' && ($user->rights->produit->creer || $user->rights->service->creer))
        {
            //WYSIWYG Editor
            require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
            ...
            print_fiche_titre('Modificar Talla'.' '.''.' : '.$object->ref, "");
            ...
            print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
            print '<input type="hidden" name="action" value="updatetalla">';
            print '<input type="hidden" name="id" value="'.$id.'">';
            print '<input type="hidden" name="canvas" value="'.$object->canvas.'">';
            print '<table class="border allwidth">';

            // Ref
            print '<tr><td width="15%" class="fieldrequired">'.$langs->trans("Ref").'</td><td colspan="2">'.$id.'</td></tr>';

            // Label
            print '<tr><td class="fieldrequired">Talla: </td><td colspan="2"><input name="libelle" size="40" value="'.$object->muestraTalla($user,$id).'"></td></tr>';
/*
            // Status
...
*/
        ...
        // Fiche en mode edition --------> Agregar combinacion de colores
        elseif ($action == 'editcolortalla' && ($user->rights->produit->creer || $user->rights->service->creer))
        {
            //WYSIWYG Editor
            require_once DOL_DOCUMENT_ROOT.'/core/class/doleditor.class.php';
            ...
            print '<form action="'.$_SERVER['PHP_SELF'].'" method="POST">'."\n";
            print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
            print '<input type="hidden" name="action" value="addtallacolor">';// Cambiar update por insert para agregar nuevos productos con talla color
            print '<input type="hidden" name="id" value="'.$object->id.'">';
			print '<input type="hidden" name="type" value="'.$object->type.'">'."\n";
            print '<input type="hidden" name="canvas" value="'.$object->canvas.'">';
            print '<table class="border allwidth">';
            ...
            // Nature
			dol_htmloutput_errors("Nature".$object->type);
            if($object->type!=1)
            {
                print '<tr><td>'.$langs->trans("Nature").'</td><td colspan="2">';
                $statutarray=array('-1'=>'&nbsp;', '1' => $langs->trans("Finished"), '0' => $langs->trans("RowMaterial"));
			...
			// Talla
            print '<tr><td>Talla: </td><td colspan="2">';
            print $formproduct->select_talla($talla);
            print '</td></tr>';
			
			//Color ---->
            print '<tr><td>Color:'.$object->ref.' </td><td colspan="2">';
			print $formproduct->select_color($talla, $object->ref);
			print '</td></tr>';
	
				//Producto Tallacolor ---->
            print '<tr><td>Producto TallaColor: </td><td colspan="2">';
			print $formproduct->select_prodTallaColor($talla, $object->ref);
			print '</td></tr>';
            // Note
            print '<tr><td valign="top">'.$langs->trans("NoteNotVisibleOnBill").'</td><td colspan="2">';
        ...
        else
        {
            ...
            // Ref
            print '<td width="15%">'.$langs->trans("Ref").'</td><td colspan="'.(2+(($showphoto||$showbarcode)?1:0)).'">'.$id;
           // print $form->showrefnav($object,'ref','',1,'ref');
            ...
            // Label
            print '<tr><td>'.$langs->trans("Label").'</td><td colspan="2">'.$object->muestraTalla($user,$id).'</td>';
/*
            $nblignes=8;
...
*/
    ...
    {
        header("Location: listetalla.php");
        exit;
    }
}
...
if ($action == 'deletetalla' && empty($conf->use_javascript_ajax))
{
    print $form->formconfirm("fichetalla.php?id=".$object->id,$langs->trans("DeleteProduct"),$langs->trans("ConfirmDeleteProduct"),"confirm_delete",'',0,"action-delete");
}
...
if ($action == '' || $action == 'view')
{
    ...
    {
        if (! isset($object->no_button_edit) || $object->no_button_edit <> 1) print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?action=edittalla&amp;id='.$id.'">'.$langs->trans("Modify").'</a>';
	...
	//print $id;
    $object_is_used = $object->isTallaUsed($id);
//print $object_is_used;
    ...
    || ($object->type == 1 && $user->rights->service->supprimer))
    {
        if (empty($object_is_used) && (! isset($object->no_button_delete) || $object->no_button_delete <> 1))
        {
            if (! empty($conf->use_javascript_ajax))
            {
                ...
                print $form->formconfirm("fichetalla.php?id=".$id,$langs->trans("DeleteProduct"),$langs->trans("ConfirmDeleteProduct"),"confirm_delete",'',0,"action-delete");
            }
            else
            {
                print '<a class="butActionDelete" href="'.$_SERVER["PHP_SELF"].'?action=deletetalla&amp;id='.$id.'">'.$langs->trans("Delete").'</a>';
            }
        }
        else
        {
