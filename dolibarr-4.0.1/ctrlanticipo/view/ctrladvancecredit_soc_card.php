...
if (empty($reshook))
{
	...
	if ($action == 'add_credit' && $confirm=="yes")
	{
		...
		if (! $error)
		{
			...
			if ($result > 0 && $result2 > 0)
			{
				...
				$urltogo=$backtopage?$backtopage:dol_buildpath('/ctrlanticipo/view/ctrladvancecredit_soc_card.php?id='.$id,1);
				header("Location: ".$urltogo);
				exit;
			}
			{
	...
	if ($action == 'confirm_delete')
	{
		...
		while ($cur!=NULL) {
			...
			$split_left->statut;
			$split_right->statut;
			if (($split_left->statut==2 || $split_left->statut==3 || $split_left->fk_user_asign>0 || $split_right->statut==2 || $split_right->statut==3 || $split_right->fk_user_asign>0 )  ) {
				$error++;
		...
		if (! $error)
		{
			...
			if ($result > 0)
			{
				...
				header("Location: ".dol_buildpath('/ctrlanticipo/view/ctrladvancecredit_soc_card.php?id='.$id,1));
				exit;
			}
			else
			{
...
if ($id && (empty($action) || $action == 'view' || $action == 'delete' || $action == 'split' || $action == 'add_credit' || $action=='asign' || $action=="asign_step2" ))
{
	...
	print '<table class="border centpercent">';
		...
		INNER JOIN llx_ctrl_advance_provider AS b on b.rowid = a.fk_advance
		WHERE
			a.statut = 1
		AND (b.fk_soc = ".$anticipo->fk_soc." OR a.fk_soc = ".$anticipo->fk_soc.") 	";
		...
		if ($resql) {
			...
			if ($num>0) {
				while ($objp = $db->fetch_object($resql) ) {
			            ...
			            print '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href="ctrladvancecredit_soc_card.php?id='.$id.'&cid='.$objp->rowid.'&action=asign">'.img_picto('','edit').' '.$langs->trans('ctrl_view_credit_reasign').'</a>';
			            print '</td>';
			            ...
			            if ($objp->fk_brother_credit>0) {
							...
							if ($objp->fk_parent>0 ) {
								print '<a href="ctrladvancecredit_soc_card.php?id='.$id.'&cid='.$objp->rowid.'&action=delete">'.img_picto('','delete').'</a>';
							}else{
