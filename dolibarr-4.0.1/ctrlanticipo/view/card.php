...
if (isset($_POST["admin"]) ) {
    if ($_POST["credit"]==-1) {
        ...
        $credit=new Ctrladvancecredit($object->db);
        $credit->fetch($_POST["credit"]);
        header('Location: ../../ctrlanticipo/view/ctrladvancecredit_card.php?id='.$credit->fk_advance);
    }
...
if (isset($_POST["add_credit"]) ) {
    if ($_POST["credit"]==-1) {
        ...
        $credit->fetch($_POST["credit"]);
        $tot_prin=$total;
        $total=$total+$credit->total_import;

        $error=0;
        if ($total>$object->total_ttc) {
        	setEventMessages($langs->trans('ctrl_facture_sum_plus'), NULL, 'errors');
        }elseif($total==$object->total_ttc){
        	$object->statut=2;
        	$object->paye=1;
        	$object->update($user);
        	$error=$credit->add_fact($object->id,$user);
        }else{
        	$error=$credit->add_fact($object->id,$user);
        }
        if ($error>0) {
...
if ($cid>0 && $aid>0 ) {
    $credit=new Ctrladvancecredit($object->db);
    $res=$credit->fetch($aid);
    if ($res) {
    	$result=$credit->delete_link($user,$cid,$id);

    	if ($result) {
    		$credit->statut=1;
			$credit->update($user);
    		setEventMessages($langs->trans("ctrl_facture_delete_reasign"), null, 'mesgs');
...
else
{
    if ($id > 0 || ! empty($ref))
    {
    	...
    	$sql="
        SELECT
            SUM(b.import) as imp, SUM(b.total_import) as tot,COUNT(*) as con
        FROM
            llx_ctrl_advance_provider AS a
        INNER JOIN llx_ctrl_advance_credit as b on a.rowid=b.fk_advance";
        if ($conf->global->MAIN_MODULE_MULTIDIVISA==1) {
	        $sql.=' INNER JOIN llx_multidivisa_divisas as c on c.rowid=a.fk_mcurrency ';
        }
        $sql.='
        WHERE
            b.statut=1 AND
            ((a.fk_soc = '.$object->socid.' AND b.fk_soc = '.$object->socid.') OR (a.fk_soc != '.$object->socid.' AND b.fk_soc = '.$object->socid.'))';

        if ($conf->global->MAIN_MODULE_MULTIDIVISA==1) {
        	$sql3="SELECT divisa FROM ".MAIN_DB_PREFIX."multidivisa_facture_fourn WHERE fk_object='".$object->id."'";
			$ra=$db->query($sql3);
			$rb=$db->fetch_object($ra);
            $sql.=' AND c.code="'.$rb->divisa.'"  ';
        }
        ...
        if ($resql) {
            ...
            if ($numrows>0) {
                ...
                if ($obj->con>0 && $object->fk_statut==1) {
                	print "
	                    <tr>
	                        <td>".$langs->trans("ctrl_facture_add_field")."</td>
	                        <td colspan=4>
	                        <form action='".$_SERVER["PHP_SELF"]."?id=".$object->id."' method='POST'>
	                        ".$langs->trans("ctrl_facture_advance_credit")." $".price($obj->tot);
	                        $sql=' 
	                        SELECT
	                            CONCAT(a.ref," (",FORMAT(b.import,2)," Sin IVA - ",FORMAT(b.total_import,2)," IVA Incluido)") as title,b.rowid as id
	                        FROM
	                            llx_ctrl_advance_provider AS a
	                        INNER JOIN llx_ctrl_advance_credit as b on a.rowid=b.fk_advance';
	                        ...
	                        if ($conf->global->MAIN_MODULE_MULTIDIVISA==1) {
								...
								$ra=$db->query($sql3);
								$rb=$db->fetch_object($ra);
	                            $sql.=' AND c.code="'.$rb->divisa.'" AND ';
	                        }
	                        $sql.=' ((a.fk_soc = '.$object->socid.' AND b.fk_soc = '.$object->socid.') OR (a.fk_soc != '.$object->socid.' AND b.fk_soc = '.$object->socid.'))';
	                        //$sql.='(b.fk_soc = '.$object->socid.' OR a.fk_soc = '.$object->socid.')  ';
	                        //echo $sql;
	                        print "&nbsp;&nbsp;&nbsp;
	                        <select name='credit'>";
	                        ...
	                        if ($resql) {
	                            ...
	                            if ($numrows>0) {
	                                ...
	                                while ($i<$numrows) {
	                                    $obj =$db->fetch_object($resql);
	                                    if ($credit==$obj->id) {
	                                        print '<option value="'.$obj->id.'" selected >'.$obj->title.'</option>';
	                                    }else{
	                                       print '<option value="'.$obj->id.'"  >'.$obj->title.'</option>'; 
	                                    }
	                        ...
	                        print '</select>
	                            &nbsp;&nbsp;&nbsp;
	                            <input type="submit" class="button" name="add_credit" value="Aplicar">
	                            &nbsp;&nbsp;&nbsp;&nbsp;
	                            <input type="submit" name="admin" style="background-color:transparent !important; border:none; cursor:pointer; color: rgb(0,0,120);" value="( '.$langs->trans("ctrl_view_manage_credit").' )">
	                        </form>
	                        ';
	                  print "</td>
	                    </tr>";
        ...
        if ($result)
        {
            ...
            if ($object->paye == 0)
            {
			            ...
			            if ($num>0) {
			            	while ($i<$num) {
				            	...
				            	<tr>
				            		<td colspan="'.$nbcols.'" align="right">'.$langs->trans('ctrl_note_advance').' '.$anticipo->getNomUrl(1).'</td>
				            		<td align="right"><b>'.price($objp->total_import).'</b></td>
				            		<td><a href="'.$_SERVER["PHP_SELF"].'?id='.$id.'&aid='.$objp->fk_source.'&cid='.$objp->rowid.'">'.img_picto('','delete').'</a></td>
				            	</tr>';
				            	$tot_creditos+=$objp->total_import;
				            	$i++;
			            	}
			            }
                ...
                $resteapayer = $object->total_ttc - $totalpaye - $tot_creditos;
        ...
        if ($action != 'presend')
        {
			...
			if (empty($reshook))
			{
	            ...
	            {
	                print '<a class="butAction" href="../../fourn/facture/paiement.php?facid='.$object->id.'&amp;action=create'.($object->fk_account>0?'&amp;accountid='.$object->fk_account:'').'">'.$langs->trans('DoPayment').'</a>';	// must use facid because id is for payment id not invoice
	            }
