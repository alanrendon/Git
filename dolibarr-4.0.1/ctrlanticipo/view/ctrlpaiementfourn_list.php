...
llxHeader('', $langs->trans('ctrl_list_advance_payment'));
...
{*/
    ...
    $sql.= " p.statut, p.num_paiement,";
    $sql.= " c.code as paiement_code,";
    $sql.= " r.fk_soc as socid,";
    $sql.= " s.fk_account as bid";
    // Add fields for extrafields
    foreach ($extrafields->attribute_list as $key => $val) $sql.=",ef.".$key.' as options_'.$key;
    // Add fields from hooks
    $parameters=array();
    ...
    $sql.=$hookmanager->resPrint;
    $sql.= " 
        FROM llx_c_paiement AS c INNER JOIN llx_ctrl_paiementfourn AS p ON p.fk_paiement = c.id
        INNER JOIN llx_ctrl_paiementfourn_facturefourn as q on q.fk_paiementfourn=p.rowid
        INNER JOIN llx_ctrl_advance_provider as r on r.rowid=q.fk_facturefourn
        LEFT JOIN llx_bank as s on s.rowid=p.fk_bank
        LEFT JOIN llx_societe as f on r.fk_soc=f.rowid
    ";
    ...
    // Search criteria
    if ($search_account>0) {
        $sql.= " AND s.fk_account= ".$search_account." ";
    }
    ...
    if ($search_company) {
        $sql.= " AND f.nom like '%".$search_company."%' ";
    }
...
if ($resql)
{
    ...
    $param='';
        ...
        print_barre_liste($langs->trans("ctrl_paiment_advance_list"), $page, $_SERVER["PHP_SELF"],$param,$sortfield,$sortorder,'',$num, $nbtotalofrecords,'title_accountancy.png', 0, '', '', $limit);
        ...
        print '</td>';
        print '<td align="left">';
        //print $form->select_company($search_company,"search_company","","1");
