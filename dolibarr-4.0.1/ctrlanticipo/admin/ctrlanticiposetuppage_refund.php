...
{
    if (dolibarr_set_const($db, "SOCIETE_CODECTRLREFUND_ADDON",$value,'chaine',0,'',$conf->entity) > 0)
    {
...
$head=array(
    array(DOL_URL_ROOT.'/ctrlanticipo/admin/ctrlanticiposetuppage.php', $langs->trans("ctrl_ref_admin_advance"), 'advance_ref' ),
     array($_SERVER["PHP_SELF"], $langs->trans("ctrl_ref_admin_refund"), 'refund_ref' ),
...
dol_fiche_head($head,'refund_ref', $langs->trans("ctrl_menu_list"), 0, 'service');

$dirsociete=array_merge(array('/ctrlanticipo/core/modules/'),$conf->modules_parts['societe']);

// Module to manage customer/supplier code

print load_fiche_titre($langs->trans("ctrl_conf_titre_refund"),'','');
...
foreach ($dirsociete as $dirroot)
{
    ...
    if (is_resource($handle))
    {
        ...
        {
            if (substr($file, 0, 15) == 'mod_coderefund_' && substr($file, -3) == 'php')
            {
                ...
                if ($conf->global->SOCIETE_CODECTRLREFUND_ADDON == "$file")
                {
