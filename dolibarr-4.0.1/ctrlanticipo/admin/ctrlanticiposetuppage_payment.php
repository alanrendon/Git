...
{
    if (dolibarr_set_const($db, "SOCIETE_CODECTRLPAYMENT_ADDON",$value,'chaine',0,'',$conf->entity) > 0)
    {
...
$head=array(
    array(DOL_URL_ROOT.'/ctrlanticipo/admin/ctrlanticiposetuppage.php', $langs->trans("ctrl_ref_admin_advance"), 'advance_ref' ),
    array(DOL_URL_ROOT.'/ctrlanticipo/admin/ctrlanticiposetuppage_refund.php', $langs->trans("ctrl_ref_admin_refund"), 'refund_ref' ),
    array($_SERVER["PHP_SELF"], $langs->trans("ctrl_ref_admin_payment_ref"), 'payment_ref' )
);

dol_fiche_head($head,'payment_ref', $langs->trans("ctrl_menu_list"), 0, 'service');

$dirsociete=array_merge(array('/ctrlanticipo/core/modules/'),$conf->modules_parts['societe']);

// Module to manage customer/supplier code

print load_fiche_titre($langs->trans("ctrl_conf_titre_payment"),'','');
...
foreach ($dirsociete as $dirroot)
{
    ...
    if (is_resource($handle))
    {
        ...
        while (($file = readdir($handle))!==false )
        {
            ...
            if (substr($file, 0, 12) == 'mod_codepay_' && substr($file, -3) == 'php')
            {
                ...
                if ($conf->global->SOCIETE_CODECTRLPAYMENT_ADDON == "$file")
                {
