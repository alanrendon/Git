 ...
 */
class mod_codepay_elephant extends ModeleThirdPartyCode
{
	...
	function info($langs)
	{
		...
		$texte.= '<input type="hidden" name="param1" value="COMPANY_ELEPHANT_MASK_PAYMENT">';
		...
		$texte.= '<td align="right">'.$form->textwithpicto('<input type="text" class="flat" size="24" name="value1" value="'.$conf->global->COMPANY_ELEPHANT_MASK_PAYMENT.'"'.$disabled.'>',$tooltip,1,1).'</td>';
	...
	function getNextValue($objsoc=0,$type=-1)
	{
		...
		// Get Mask value
		$mask = '';
		if (isset($conf->global->COMPANY_ELEPHANT_MASK_PAYMENT)) {
			$mask = $conf->global->COMPANY_ELEPHANT_MASK_PAYMENT;
		}else{
			return '';
		}
		...
		$numFinal=get_next_value($db,$mask,'ctrl_paiementfourn',$field,$where,'',$now,'next',false);
	...
	function verif($db, &$code, $soc, $type)
	{
		...
		else
		{
			// Get Mask value
			$mask = '';
			$mask = empty($conf->global->COMPANY_ELEPHANT_MASK_PAYMENT)?'':$conf->global->COMPANY_ELEPHANT_MASK_PAYMENT;
			if (! $mask)
			{
	...
	{
		$sql = "SELECT ref FROM llx_ctrl_paiementfourn";
		$sql.= " WHERE ref = '".$code."'";
	...
	function getToolTipp($langs,$soc,$type)
    {
        ...
        if ($type == 0 || $type == -1)
        {
            ...
            $s.=$langs->trans("NextValue").($type == -1?" ".$langs->trans("ctrl_view_pay"):'').': <b>'.$nextval.'</b><br>';
        }
        return $s;
    }
