 ...
 */
class mod_codeconsultas_monkey extends ModeleThirdPartyCode
{
	...
	function info($langs)
	{
		return "Devuelve un número bajo el formato nnnnnnn para los códigos de consultas creadas, donde nnnnnnn sera un contador secuencial sin ruptura y sin volver a 0.";
	}
	...
	{
		return str_replace($this->prefixcustomer,"",$this->getNextValue("",0));
	}
	...
	function getNextValue($objsoc=0,$type=-1)
	{
		...
		if ($max >= (pow(10, 4) - 1)) $num=$max+1;	// If counter > 9999, we do not format on 4 chars, we take number as it is
			...
			if ($type == 0 )
			{
				$num = sprintf("%07s",$max+1);
			}else{
				$num = sprintf("%01s",$max+1);
			}
		}
	...
	function getToolTipp($langs,$soc,$type)
    {
        ...
        if ($type == 0 || $type == -1)
        {
            ...
            $s.=$langs->trans("NextValue").($type == -1?' ('."Consulta".')':'').': <b>'.str_replace($this->prefixcustomer,"",$nextval).'</b><br>';
        }
