 ...
 */
class mod_codeconsultas_elephant extends ModeleThirdPartyCode
{
	...
	function info($langs)
	{
		...
		$texte.= '<input type="hidden" name="param1" value="COMPANY_ELEPHANT_MASK_CONSULTA">';
		$texte.= '<input type="hidden" name="param2" value="COMPANY_ELEPHANT_MASK_BORRADOR">';
		...
		$texte.= '<td align="right">'.$form->textwithpicto('<input type="text" class="flat" size="24" name="value1" value="'.$conf->global->COMPANY_ELEPHANT_MASK_CONSULTA.'"'.$disabled.'>',$tooltip,1,1).'</td>';
	...
	function getNextValue($objsoc=0,$type=-1)
	{
		...
		// Get Mask value
		$mask = '';
		if ($type==0) $mask = $conf->global->COMPANY_ELEPHANT_MASK_CONSULTA;
		if ($type==1) $mask = $conf->global->COMPANY_ELEPHANT_MASK_BORRADOR;
		if (! $mask)
		{
		...
		$numFinal=get_next_value($db,$mask,'consultas',$field,$where,'',$now,'next',false);
	...
	function verif_prefixIsUsed()
	{
		...
		$mask = $conf->global->COMPANY_ELEPHANT_MASK_CONSULTA;
		if (preg_match('/\{pre\}/i',$mask)) return 1;

		$mask = $conf->global->COMPANY_ELEPHANT_MASK_BORRADOR;
		if (preg_match('/\{pre\}/i',$mask)) return 1;
	...
	function verif($db, &$code, $soc, $type)
	{
		...
		else
		{
			// Get Mask value
			$mask = '';
			if ($type==0) $mask = empty($conf->global->COMPANY_ELEPHANT_MASK_CONSULTA)?'':$conf->global->COMPANY_ELEPHANT_MASK_CONSULTA;
			if ($type==1) $mask = empty($conf->global->COMPANY_ELEPHANT_MASK_BORRADOR)?'':$conf->global->COMPANY_ELEPHANT_MASK_BORRADOR;
			if (! $mask)
			{
				$this->error='NotConfigured';
				return '';
	...
	{
		$sql = "SELECT code_client FROM llx_consultas";
	...
	function getToolTipp($langs,$soc,$type)
    {
        ...
        if ($type == 0 || $type == -1)
        {
            ...
            $s.=$langs->trans("NextValue").($type == -1?' ('."Consulta".')':'').': <b>'.$nextval.'</b><br>';
        }
        return $s;
    }
