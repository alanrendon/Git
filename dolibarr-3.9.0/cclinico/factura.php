...
if (empty($reshook))
{
	...
	else if ($action == 'setrevenuestamp' )
	{
		$object->fetch($id);
	...
	else if ($action == 'setremisepercent' )
	{
		$object->fetch($id);
		$result = $object->set_remise($user, $_POST['remise_percent']);
	}

	else if ($action == "setabsolutediscount" )
	{
	...
	// Classify "paid"
	else if ($action == 'confirm_paid' && $confirm == 'yes')
	{
		$object->fetch($id);
		$result = $object->set_paid($user);
		if ($result<0) setEventMessages($object->error, $object->errors, 'errors');
	} // Classif "paid partialy"
	else if ($action == 'confirm_paid_partially' && $confirm == 'yes' )
	{
		$object->fetch($id);
	...
	// Convertir en reduc
	else if ($action == 'confirm_converttoreduc' && $confirm == 'yes' )
	{
		$object->fetch($id);
		$object->fetch_thirdparty();
	...
	*/
			...
			if (! $error)
			{
				...
				if ($id > 0) {
					if (!empty($aid)) {
						...
						$consulta->fetch($aid);
						$actualizar_tercero=$consulta->actualizar_tercero($object->socid);
						if ($actualizar_tercero>0) {
							$consulta->vincular_consulta_factura($id,$user);
						}
	...
	elseif ($action == 'updateligne'  && ! GETPOST('cancel'))
	{
	...
	else if ($action == 'updatealllines'  && $_POST['all_percent'] == $langs->trans('Modifier'))
	{
		if (!$object->fetch($id) > 0) dol_print_error($db);
	...
	else if ($action == 'updateligne'  && $_POST['cancel'] == $langs->trans('Cancel')) {
...
if ($conf->global->CLINICO_PROD>0 && GETPOST('facid')>0) {
	$line_ext = new FactureLigne($db);
	
    require_once DOL_DOCUMENT_ROOT.'/core/lib/product.lib.php';
    $produ = new Product($db);
    $produ->fetch($conf->global->CLINICO_PROD);

    $object->addline($produ->label,$produ->price,1,0,0,0,$conf->global->CLINICO_PROD);
    //array_push ($object->lines,$line_ext);
}
...
if ($action == 'create')
{
	...
	if (!empty($tercero)) {
		print '<td colspan="2">';
		$soc2 = new Societe($db);
		$result=$soc2->fetch($tercero);
		print $soc2->getNomUrl(1);
		print '<input type="hidden" name="socid" value="' . $soc2->id . '">';
		// Outstanding Bill
	...
	print '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
	print '<a style="text-decoration:none;" type="button" class="button" href="consultas_card.php?aid='.$aid.'" >' . $langs->trans("Cancel") . '</a>';
	print '</div>';
...
else if ($id > 0 || ! empty($ref))
{
	...
	if (! empty($conf->global->FACTURE_CHANGE_THIRDPARTY) && $action != 'editthirdparty' && $object->brouillon )
	...
	print '</td>';
	if ($object->type != Facture::TYPE_CREDIT_NOTE && $action != 'editinvoicedate' && ! empty($object->brouillon) )
	...
	print '</td>';
	if ($object->type != Facture::TYPE_CREDIT_NOTE && $action != 'editpaymentterm' && ! empty($object->brouillon) )
	...
	print '<td>';
	if (($action != 'editbankaccount')  && ! empty($object->brouillon))
	...
	$resql1=$db->query("SELECT a.fk_consulta FROM llx_facturas_consulta as a WHERE a.fk_factura=".$object->id);
		    ...
		    if ($resql1)
		    {
		        ...
		        if ($num2>0)
		        {
		            ...
		            $consulta->fetch($obj2->fk_consulta);
	...
	if (! empty($conf->global->INVOICE_US_SITUATION))
	{
		if ($object->situation_cycle_ref && $object->statut == 0) {
			...
			if (! empty($conf->margin->enabled) && empty($user->societe_id))
			{
				...
				if ((! empty($conf->global->DISPLAY_MARGIN_RATES) || ! empty($conf->global->DISPLAY_MARK_RATES)) ) {
	...
	if ($object->statut == 0  && $action != 'valid' && $action != 'editline' && ($object->is_first() || !$object->situation_cycle_ref))
	{
		if ($action != 'editline')
		{
			$var = true;
	...
	if ($action != 'prerelance' && $action != 'presend' && $action != 'valid' && $action != 'editline')
	{
		...
		if (empty($reshook)) {
			...
			// Create payment
			if ($object->type != Facture::TYPE_CREDIT_NOTE && $object->statut == 1 && $object->paye == 0 ) {
				if ($objectidnext) {
					...
					if ($resteapayer == 0) {
						...
						print '<div class="inline-block divButAction"><a class="butAction" href="../compta/paiement.php?facid=' . $object->id . '&amp;action=create&amp;accountid='.$object->fk_account.'">' . $langs->trans('DoPayment') . '</a></div>';
					}
				}
			}
			...
			if ($object->type == Facture::TYPE_CREDIT_NOTE || $object->type == Facture::TYPE_DEPOSIT) {
				...
				// For credit note
				if ($object->type == Facture::TYPE_CREDIT_NOTE && $object->statut == 1 && $object->paye == 0  && $object->getSommePaiement() == 0) {
					print '<div class="inline-block divButAction"><a class="butAction" href="' . $_SERVER["PHP_SELF"] . '?facid=' . $object->id . '&amp;action=converttoreduc">' . $langs->trans('ConvertToReduc') . '</a></div>';
				}
				// For deposit invoice
				if ($object->type == Facture::TYPE_DEPOSIT && $object->paye == 1 && $resteapayer == 0  && empty($discount->id))
				{
					print '<div class="inline-block divButAction"><a class="butAction" href="'.$_SERVER["PHP_SELF"].'?facid='.$object->id.'&amp;action=converttoreduc">'.$langs->trans('ConvertToReduc').'</a></div>';
				}
			}
			...
			// Classify paid
			if ($object->statut == 1 && $object->paye == 0 && (($object->type != Facture::TYPE_CREDIT_NOTE && $object->type != Facture::TYPE_DEPOSIT && $resteapayer <= 0) || ($object->type == Facture::TYPE_CREDIT_NOTE && $resteapayer >= 0))
				|| ($object->type == Facture::TYPE_DEPOSIT && $object->paye == 0 && $object->total_ttc > 0 && $resteapayer == 0  && empty($discount->id))
			)
			{
			...
			// Clone
			if (($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_PROFORMA) )
			{
			...
			if (($object->type == Facture::TYPE_STANDARD || $object->type == Facture::TYPE_DEPOSIT || $object->type == Facture::TYPE_PROFORMA) && $object->statut == 0 )
			{
			...
			if ( ($object->type == 5) && ($object->statut == 1 || $object->statut == 2)) {
