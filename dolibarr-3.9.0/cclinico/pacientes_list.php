...
$extralabels = $extrafields->fetch_name_optionals_label('pacientes');
...
$arrayfields=array(
    'p.lastname'=>array('label'=>$langs->trans("Lastname"), 'checked'=>1, 'enabled' => 1),
    ...
    'p.phone'=>array('label'=>$langs->trans("PhonePro"), 'checked'=>0),
    'p.phone_perso'=>array('label'=>$langs->trans("PhonePerso"), 'checked'=>0),
    'p.phone_mobile'=>array('label'=>$langs->trans("PhoneMobile"), 'checked'=>0),
    'p.fax'=>array('label'=>$langs->trans("Fax"), 'checked'=>0),
    'p.email'=>array('label'=>$langs->trans("Email"), 'checked'=>0),
    'p.skype'=>array('label'=>$langs->trans("Skype"), 'checked'=>0, 'enabled'=>(! empty($conf->skype->enabled))),
...
llxHeader('',$langs->trans("Code11"),'EN:Module_Third_Parties|FR:Module_Tiers|ES:M&oacute;dulo_Empresas');
...
$parameters=array();
$sql.= " FROM llx_pacientes as p";
if (is_array($extrafields->attribute_label) && count($extrafields->attribute_label)){
   $sql.= " LEFT JOIN llx_pacientes_extrafields as ef on (p.rowid = ef.fk_object)"; 
} 
...
{
    $sql .= " AND p.fk_user=".$db->escape($fk_user)." ";
}
...
//print $sql;
dol_syslog("cclinico/pacientes_list.php", LOG_DEBUG);
$result = $db->query($sql);
if ($result)
{
    ...
    print_barre_liste($titre, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, '', $num, $nbtotalofrecords,'title_generic.png');
    ...
    print '<div class="liste_titre liste_titre_bydiv centpercent">';
    print '<div class="divsearchfield">'.$langs->trans("Code12").':';
    print $object->select_dolusers($fk_user, 'fk_user', 1, '', $disable2, '', 0, $conf->entity, 0, 0, '', 0, '', 'maxwidth300');
    print '</div></div>';
    // Ligne des titres
    ...
    while ($i < min($num,$limit))
    {
        ...
        print '<a href="'.DOL_URL_ROOT.'/cclinico/agenda_card.php?aid='.$obj->cidp.'&action=create">'.img_object("Agendar","action").'</a>';
        print '</td>';
