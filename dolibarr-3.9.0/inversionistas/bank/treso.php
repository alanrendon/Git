...

{
	...
	dol_fiche_head($head,'tabinvtrans',$langs->trans("FinancialAccount"),0,'account');
	...
	$sql_inversionista="SELECT a.rowid FROM llx_union_bt as a WHERE a.fk_cuenta=".$_REQUEST["account"];



	$result_inversionista=$db->query($sql_inversionista);



	if ($db->num_rows($result_inversionista)==0) {
		$result = $db->query($sql);
		...
		$sql2.= " INNER JOIN llx_corte_mensual as a on a.fk_factura=ff.rowid";

		$sql2.= " WHERE ff.entity = ".$conf->entity;

		$sql2.= " AND ff.paye = 0 AND fk_statut = 1";	// Not paid

	    $sql2.= " AND a.fk_cuenta=".$acct->id; // Id bank account of supplier invoice
